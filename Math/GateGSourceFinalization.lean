import Math.GateGCurveSelection
import Math.GateGSourceCell
import Math.GateGUltrafilterSubsequence
...
theorem positiveCoordinateArc_of_eventual_source_relations
    ...
    (hroot :
      ∀ j,
        ∀ᶠ n in (GateGGermComponentScratch.sequenceUltrafilter :
          Filter ℕ),
  ...
  have hgood :
      ∀ᶠ n in
          (GateGGermComponentScratch.sequenceUltrafilter : Filter ℕ),
        good n := by
  ...
  have hpower :
    ...
    apply
      Math.GateGCurveSelection.hasAnalyticPowerCurveAt_signCell_of_algebraic_approach
          (sourcePolynomial
            (withPositiveCoordinatePolynomial P coordinate)
            (withPositiveCoordinateSignPattern τ) x₀)
          (sourceSignPattern
