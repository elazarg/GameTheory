import Math.GateGConvergence
import Math.GateGRootCoverage
import Math.GateGPositiveRoot
...
namespace Math
namespace GateGFiniteBranchCoverage

open GateGConvergenceScratch
open GateGRootCoverage
open GateGPositiveRootScratch
...
theorem exists_finite_analytic_branches_covering_sequence
    ...
    (hxyroot :
      ∀ i, GateGTerminationScratch.bivEvalAt
        Q (x i) (y i) = 0) :
  ...
  let Good (s : PowerSeries ℂ) (z : ℂ → ℂ) : Prop :=
    ...
    (fun a => (s.trunc n).eval a + a ^ n * z a) 0 =
      s.constantCoeff ∧
    (∀ᶠ a in 𝓝 (0 : ℂ),
      GateGTerminationScratch.bivEvalAt Qram a
  ...
  have hBranches :
      ∀ b ∈ branches,
        ...
        b 0 = 0 ∧
        ∀ᶠ a in 𝓝 (0 : ℂ),
          GateGTerminationScratch.bivEvalAt Qram a (b a) = 0 := by
    intro b hb
  ...
  have hBranchRoots :
      ∀ᶠ i in atTop, ∀ b ∈ branches,
        GateGTerminationScratch.bivEvalAt Qram
          (t i) (b (t i)) = 0 := by
      ...
      GateGTerminationScratch.bivEvalAt Qram
        ...
        have hevalHom :
          ...
          · intro a
            simp
          · simp
        dsimp [Qram, GateGTerminationScratch.bivEvalAt]
...
theorem
    ...
    ∃ (q : J → Polynomial (Polynomial ℝ))
        ...
        (t : ℕ → ℝ),
      (∀ j,
        q j ∈ GateGFactorCoverageScratch.irreducibleFactors (Q j) ∧
        Irreducible (q j) ∧
        0 < (q j).natDegree ∧
        q j ∣ Q j) ∧
      Tendsto
