...
This module is adapted from EconCSLib's objective-bound Farkas lemma
formalization.
...
* `farkas_lemma`: objective-bound Farkas lemma for finite weak inequality systems.
-/
...
def farkasAugB (_b : I → 𝕜) : FarkasAugRow I → 𝕜
  | Sum.inl _ => 0
  | Sum.inr false => 0
  | Sum.inr true => 1
...
omit [LinearOrder 𝕜] [IsStrictOrderedRing 𝕜] [Fintype I] in
...
theorem farkas_lemma (A : I → Fin n → 𝕜) (b : I → 𝕜) (c : Fin n → 𝕜) (d : 𝕜)
  ...
  · -- (→) primal bound implies dual certificate, via ToA on augmented system
      ...
      · -- t > 0: rescaling produces a feasible point with ⟨c, ·⟩ < d.
        ...
        have hx'_feas : ∀ i, b i ≤ ∑ j, A i j * x' j := by
          ...
          have hAx_div : (∑ j, A i j * x' j) = (∑ j, A i j * x j) / t := by
            ...
            refine Finset.sum_congr rfl ?_
            intro j _
            change A i j * x j / t = A i j * (x j / t)
            ring
    ...
    have hu'_b_decomp : (∑ row, u' row * farkasAugB b row) = u' (Sum.inr true) := by
      ...
      simp [farkasAugB_inl, farkasAugB_inr_false, farkasAugB_inr_true]
...
/-- **Farkas' Lemma** for an arbitrary finite column type. This is the same
objective-bound statement as `farkas_lemma`, transported along the canonical
equivalence with `Fin (Fintype.card J)`. -/
theorem farkas_lemma_fintype {J : Type*} [Fintype J]
    (A : I → J → 𝕜) (b : I → 𝕜) (c : J → 𝕜) (d : 𝕜)
    (hS : ∃ x : J → 𝕜, ∀ i, b i ≤ ∑ j, A i j * x j) :
    (∀ x : J → 𝕜, (∀ i, b i ≤ ∑ j, A i j * x j) → d ≤ ∑ j, c j * x j) ↔
    (∃ u : I → 𝕜, (∀ i, 0 ≤ u i)
      ∧ (∀ j, ∑ i, u i * A i j = c j)
      ∧ d ≤ ∑ i, u i * b i) := by
  classical
  let e := _root_.Fintype.equivFin J
  let A' : I → Fin (_root_.Fintype.card J) → 𝕜 := fun i k => A i (e.symm k)
  let c' : Fin (_root_.Fintype.card J) → 𝕜 := fun k => c (e.symm k)
  have hS' : IsFeasible A' b := by
    obtain ⟨x, hx⟩ := hS
    refine ⟨fun k => x (e.symm k), ?_⟩
    intro i
    change b i ≤ ∑ k, A' i k * x (e.symm k)
    have hsum :
        (∑ k, A' i k * x (e.symm k)) = ∑ j, A i j * x j := by
      simpa [A'] using (e.symm.sum_comp fun j : J => A i j * x j)
    rw [hsum]
    exact hx i
  constructor
  · intro hbound
    have hbound' : ∀ x' : Fin (_root_.Fintype.card J) → 𝕜,
        (∀ i, b i ≤ ∑ k, A' i k * x' k) → d ≤ ∑ k, c' k * x' k := by
      intro x' hx'
      let x : J → 𝕜 := fun j => x' (e j)
      have hx : ∀ i, b i ≤ ∑ j, A i j * x j := by
        intro i
        have hsum :
            (∑ k, A' i k * x' k) = ∑ j, A i j * x j := by
          simpa [A', x] using (e.symm.sum_comp fun j : J => A i j * x j)
        rw [← hsum]
        exact hx' i
      have hb := hbound x hx
      have hsum_obj : (∑ k, c' k * x' k) = ∑ j, c j * x j := by
        simpa [c', x] using (e.symm.sum_comp fun j : J => c j * x j)
      rw [hsum_obj]
      exact hb
    obtain ⟨u, hu_nonneg, hu_col, hu_bound⟩ :=
      (farkas_lemma A' b c' d hS').mp hbound'
    refine ⟨u, hu_nonneg, ?_, hu_bound⟩
    intro j
    simpa [A', c', e] using hu_col (e j)
  · intro hcert
    obtain ⟨u, hu_nonneg, hu_col, hu_bound⟩ := hcert
    have hcert' : ∃ u : I → 𝕜, (∀ i, 0 ≤ u i)
        ∧ (∀ k, ∑ i, u i * A' i k = c' k)
        ∧ d ≤ ∑ i, u i * b i := by
      refine ⟨u, hu_nonneg, ?_, hu_bound⟩
      intro k
      simpa [A', c'] using hu_col (e.symm k)
    have hbound' :=
      (farkas_lemma A' b c' d hS').mpr hcert'
    intro x hx
    let x' : Fin (_root_.Fintype.card J) → 𝕜 := fun k => x (e.symm k)
    have hx' : ∀ i, b i ≤ ∑ k, A' i k * x' k := by
      intro i
      have hsum :
          (∑ k, A' i k * x' k) = ∑ j, A i j * x j := by
        simpa [A', x'] using (e.symm.sum_comp fun j : J => A i j * x j)
