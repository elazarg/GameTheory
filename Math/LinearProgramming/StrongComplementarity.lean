...
/-!
# Finite Linear Programming Strong Complementarity

Reusable strict-complementarity predicates, biconditional consequences, and
the finite-column Goldman-Tucker existence theorem for the standard-form LP API.

The existence proof adapts EconCSLib's Farkas-based strong-complementarity
argument to the local `MinPrimalFeasible`/`MaxDualFeasible` definitions.
-/
...
/-- A standard-form primal-dual pair with zero gap and strictly positive
complementary sums:

* `(Ax - b)_i + y_i > 0` for each row;
* `x_j + (c - yA)_j > 0` for each column. -/
def IsStrongComplementaryPair
    (A : Row → Col → ℝ) (b : Row → ℝ) (c : Col → ℝ)
    (x : Col → ℝ) (y : Row → ℝ) : Prop :=
  MinPrimalFeasible A b x ∧
  MaxDualFeasible A c y ∧
  minPrimalValue c x = maxDualValue b y ∧
  (∀ i, 0 < minPrimalSlack A b x i + y i) ∧
  (∀ j, 0 < x j + maxDualSlack A c y j)

private theorem eq_zero_of_mul_eq_zero_of_pos_left {a b : ℝ}
    (hprod : a * b = 0) (ha : 0 < a) : b = 0 := by
  exact eq_zero_of_ne_zero_of_mul_left_eq_zero ha.ne' hprod

private theorem eq_zero_of_mul_eq_zero_of_pos_right {a b : ℝ}
    (hprod : a * b = 0) (hb : 0 < b) : a = 0 := by
  exact eq_zero_of_ne_zero_of_mul_right_eq_zero hb.ne' hprod

/-- At zero standard-form duality gap, strict primal slack forces the
corresponding dual variable to vanish. -/
theorem dual_eq_zero_of_minPrimalSlack_pos
    {A : Row → Col → ℝ} {b : Row → ℝ} {c : Col → ℝ}
    {x : Col → ℝ} {y : Row → ℝ}
    (hx : MinPrimalFeasible A b x) (hy : MaxDualFeasible A c y)
    (hgap : minPrimalValue c x = maxDualValue b y) {i : Row}
    (hpos : 0 < minPrimalSlack A b x i) :
    y i = 0 := by
  exact eq_zero_of_mul_eq_zero_of_pos_left
    (minPrimalSlack_mul_dual_eq_zero_of_gap_zero hx hy hgap i) hpos

/-- At zero standard-form duality gap, a strict dual variable forces the
corresponding primal slack to vanish. -/
theorem minPrimalSlack_eq_zero_of_dual_pos
    {A : Row → Col → ℝ} {b : Row → ℝ} {c : Col → ℝ}
    {x : Col → ℝ} {y : Row → ℝ}
    (hx : MinPrimalFeasible A b x) (hy : MaxDualFeasible A c y)
    (hgap : minPrimalValue c x = maxDualValue b y) {i : Row}
    (hpos : 0 < y i) :
    minPrimalSlack A b x i = 0 := by
  exact eq_zero_of_mul_eq_zero_of_pos_right
    (minPrimalSlack_mul_dual_eq_zero_of_gap_zero hx hy hgap i) hpos

/-- At zero standard-form duality gap, strict dual slack forces the
corresponding primal variable to vanish. -/
theorem primal_eq_zero_of_maxDualSlack_pos
    {A : Row → Col → ℝ} {b : Row → ℝ} {c : Col → ℝ}
    {x : Col → ℝ} {y : Row → ℝ}
    (hx : MinPrimalFeasible A b x) (hy : MaxDualFeasible A c y)
    (hgap : minPrimalValue c x = maxDualValue b y) {j : Col}
    (hpos : 0 < maxDualSlack A c y j) :
    x j = 0 := by
  exact eq_zero_of_mul_eq_zero_of_pos_left
    (maxDualSlack_mul_primal_eq_zero_of_gap_zero hx hy hgap j) hpos

/-- At zero standard-form duality gap, a strict primal variable forces the
corresponding dual slack to vanish. -/
theorem maxDualSlack_eq_zero_of_primal_pos
    {A : Row → Col → ℝ} {b : Row → ℝ} {c : Col → ℝ}
    {x : Col → ℝ} {y : Row → ℝ}
    (hx : MinPrimalFeasible A b x) (hy : MaxDualFeasible A c y)
    (hgap : minPrimalValue c x = maxDualValue b y) {j : Col}
    (hpos : 0 < x j) :
    maxDualSlack A c y j = 0 := by
  exact eq_zero_of_mul_eq_zero_of_pos_right
    (maxDualSlack_mul_primal_eq_zero_of_gap_zero hx hy hgap j) hpos

/-- In a strong complementary pair, row slack is positive exactly when the
corresponding dual variable is zero. -/
theorem IsStrongComplementaryPair.minPrimalSlack_pos_iff_dual_eq_zero
    {A : Row → Col → ℝ} {b : Row → ℝ} {c : Col → ℝ}
    {x : Col → ℝ} {y : Row → ℝ}
    (h : IsStrongComplementaryPair A b c x y) (i : Row) :
    0 < minPrimalSlack A b x i ↔ y i = 0 := by
  rcases h with ⟨hx, hy, hgap, hrow, _hcol⟩
  constructor
  · exact dual_eq_zero_of_minPrimalSlack_pos hx hy hgap
  · intro hy_zero
    have hsum := hrow i
    rwa [hy_zero, add_zero] at hsum

/-- In a strong complementary pair, a dual variable is positive exactly when the
corresponding row slack is zero. -/
theorem IsStrongComplementaryPair.dual_pos_iff_minPrimalSlack_eq_zero
    {A : Row → Col → ℝ} {b : Row → ℝ} {c : Col → ℝ}
    {x : Col → ℝ} {y : Row → ℝ}
    (h : IsStrongComplementaryPair A b c x y) (i : Row) :
    0 < y i ↔ minPrimalSlack A b x i = 0 := by
  rcases h with ⟨hx, hy, hgap, hrow, _hcol⟩
  constructor
  · exact minPrimalSlack_eq_zero_of_dual_pos hx hy hgap
  · intro hslack_zero
    have hsum := hrow i
    rwa [hslack_zero, zero_add] at hsum

/-- In a strong complementary pair, dual slack is positive exactly when the
corresponding primal variable is zero. -/
theorem IsStrongComplementaryPair.maxDualSlack_pos_iff_primal_eq_zero
    {A : Row → Col → ℝ} {b : Row → ℝ} {c : Col → ℝ}
    {x : Col → ℝ} {y : Row → ℝ}
    (h : IsStrongComplementaryPair A b c x y) (j : Col) :
    0 < maxDualSlack A c y j ↔ x j = 0 := by
  rcases h with ⟨hx, hy, hgap, _hrow, hcol⟩
  constructor
  · exact primal_eq_zero_of_maxDualSlack_pos hx hy hgap
  · intro hx_zero
    have hsum := hcol j
    rwa [hx_zero, zero_add] at hsum

/-- In a strong complementary pair, a primal variable is positive exactly when
the corresponding dual slack is zero. -/
theorem IsStrongComplementaryPair.primal_pos_iff_maxDualSlack_eq_zero
    {A : Row → Col → ℝ} {b : Row → ℝ} {c : Col → ℝ}
    {x : Col → ℝ} {y : Row → ℝ}
    (h : IsStrongComplementaryPair A b c x y) (j : Col) :
    0 < x j ↔ maxDualSlack A c y j = 0 := by
  rcases h with ⟨hx, hy, hgap, _hrow, hcol⟩
  constructor
  · exact maxDualSlack_eq_zero_of_primal_pos hx hy hgap
  · intro hslack_zero
    have hsum := hcol j
    rwa [hslack_zero, add_zero] at hsum

section FinColumns

variable {I : Type*} [Fintype I] {n : ℕ}

private theorem weak_duality_sum (A : I → Fin n → ℝ) (b : I → ℝ) (c : Fin n → ℝ)
    {x : Fin n → ℝ} (hxA : ∀ i, b i ≤ ∑ j, A i j * x j) (hxnn : ∀ j, 0 ≤ x j)
    {u : I → ℝ} (hu : MaxDualFeasible A c u) :
    ∑ i, u i * b i ≤ ∑ j, c j * x j := by
  have h := min_weak_duality (A := A) (b := b) (c := c) (x := x) (y := u)
    ⟨hxnn, hxA⟩ hu
  simpa [minPrimalValue, maxDualValue, dot, mul_comm] using h
...
def optAugA (A : I → Fin n → ℝ) (c : Fin n → ℝ) : OptAugRow I n → Fin n → ℝ
  | Sum.inl (Sum.inl i), j => A i j
  | Sum.inl (Sum.inr j'), j => if j = j' then 1 else 0
  | Sum.inr (), j => -c j

/-- Optimality-augmented RHS. -/
def optAugB (b : I → ℝ) (v : ℝ) : OptAugRow I n → ℝ
  | Sum.inl (Sum.inl i) => b i
  | Sum.inl (Sum.inr _) => 0
  | Sum.inr () => -v

omit [Fintype I] in
@[simp] theorem optAugA_inl_inl (A : I → Fin n → ℝ) (c : Fin n → ℝ)
    (i : I) (j : Fin n) :
    optAugA A c (Sum.inl (Sum.inl i)) j = A i j := rfl

omit [Fintype I] in
@[simp] theorem optAugA_inl_inr (A : I → Fin n → ℝ) (c : Fin n → ℝ)
    (j' j : Fin n) :
    optAugA A c (Sum.inl (Sum.inr j')) j = if j = j' then 1 else 0 := rfl

omit [Fintype I] in
@[simp] theorem optAugA_inr (A : I → Fin n → ℝ) (c : Fin n → ℝ) (j : Fin n) :
    optAugA A c (Sum.inr ()) j = -c j := rfl

omit [Fintype I] in
@[simp] theorem optAugB_inl_inl (b : I → ℝ) (v : ℝ) (i : I) :
    optAugB b v (Sum.inl (Sum.inl i) : OptAugRow I n) = b i := rfl

omit [Fintype I] in
@[simp] theorem optAugB_inl_inr (b : I → ℝ) (v : ℝ) (j' : Fin n) :
    optAugB b v (Sum.inl (Sum.inr j') : OptAugRow I n) = 0 := rfl

omit [Fintype I] in
@[simp] theorem optAugB_inr (b : I → ℝ) (v : ℝ) :
...
theorem optAug_feasible_iff (A : I → Fin n → ℝ) (b : I → ℝ) (c : Fin n → ℝ)
    (v : ℝ) (x : Fin n → ℝ) :
  ...
  · rintro ⟨hAx, hxnn, hcx⟩ idx
    ...
    · simp only [optAugB_inl_inr, optAugA_inl_inr, ite_mul, one_mul, zero_mul,
                 Fintype.sum_ite_eq']
      exact hxnn j'
    · change optAugB b v (Sum.inr ()) ≤ ∑ j, optAugA A c (Sum.inr ()) j * x j
...
theorem exists_row_strict_pair
    (A : I → Fin n → ℝ) (b : I → ℝ) (c : Fin n → ℝ) (v : ℝ)
    {x₀ : Fin n → ℝ}
    (hx₀A : ∀ i, b i ≤ ∑ j, A i j * x₀ j)
    (hx₀nn : ∀ j, 0 ≤ x₀ j) (hx₀_val : ∑ j, c j * x₀ j = v)
    {u₀ : I → ℝ} (hu₀ : MaxDualFeasible A c u₀) (hu₀_val : ∑ i, u₀ i * b i = v)
    (i₀ : I) :
    ∃ (x : Fin n → ℝ) (u : I → ℝ),
      (∀ i, b i ≤ ∑ j, A i j * x j) ∧ (∀ j, 0 ≤ x j) ∧
      MaxDualFeasible A c u ∧
      (∑ j, c j * x j = v) ∧ (∑ i, u i * b i = v) ∧
      0 < (∑ j, A i₀ j * x j - b i₀) + u i₀ := by
  classical
  -- Classical case split.
  by_cases hCaseA : ∃ x : Fin n → ℝ,
      (∀ i, b i ≤ ∑ j, A i j * x j) ∧ (∀ j, 0 ≤ x j) ∧
  ...
  · -- Case B: ∀ primal-optimal x, (Ax - b)_{i₀} ≤ 0 (forced = 0 by Ax ≥ b).
    ...
    have hCaseA' : ∀ x : Fin n → ℝ,
      ...
      have hcx_ge : v ≤ ∑ j, c j * x j := by
        have hwd := weak_duality_sum A b c hxA hxnn hu₀
        linarith [hu₀_val]
    ...
    set μ : I → ℝ := fun i => w (Sum.inl (Sum.inl i)) with hμ_def
    set ν : Fin n → ℝ := fun j' => w (Sum.inl (Sum.inr j')) with hν_def
    set lam : ℝ := w (Sum.inr ()) with hlam_def
    ...
    have sum_split : ∀ (f : OptAugRow I n → ℝ),
    ...
    · -- Sub-case B.1: lam > 0. Set u = (μ + e_{i₀}) / lam.
      ...
      · -- MaxDualFeasible
        ...
        · intro i
          have hsum_nn : 0 ≤ μ i + (if i = i₀ then 1 else 0) := by
            have : 0 ≤ (if i = i₀ then (1 : ℝ) else 0) := by
              split_ifs <;> simp
            linarith [hμ_nn i]
        ...
        · intro j
          ...
          have hkey : (∑ i, (μ i + (if i = i₀ then 1 else 0)) * A i j) ≤ lam * c j := by
            ...
            have hsel : (∑ i, (if i = i₀ then (1 : ℝ) else 0) * A i j) = A i₀ j := by
              simp
            ...
            rw [hsplit2, hreorg]
            linarith [hν_nn j]
          change (∑ i, (μ i + (if i = i₀ then 1 else 0)) / lam * A i j) ≤ c j
      ...
      · -- ⟨u, b⟩ = v: equals from ≥ v (Farkas) and ≤ v (weak duality).
        have hub_ge_v : v ≤ ∑ i, (μ i + (if i = i₀ then 1 else 0)) / lam * b i := by
          ...
          have hkey : (∑ i, (μ i + (if i = i₀ then 1 else 0)) * b i) ≥ lam * v := by
            have hsplit : (∑ i, (μ i + (if i = i₀ then 1 else 0)) * b i)
              ...
              rw [← Finset.sum_add_distrib]
              refine Finset.sum_congr rfl (fun i _ => ?_); ring
            have hsel : (∑ i, (if i = i₀ then (1 : ℝ) else 0) * b i) = b i₀ := by
              simp
        ...
        have hub_le_v : (∑ i, (μ i + (if i = i₀ then 1 else 0)) / lam * b i) ≤ v := by
          have hu'_du : MaxDualFeasible A c (fun i => (μ i + (if i = i₀ then 1 else 0)) / lam) := by
            refine ⟨?_, ?_⟩
            · intro i
              have hsum_nn : 0 ≤ μ i + (if i = i₀ then 1 else 0) := by
                have : 0 ≤ (if i = i₀ then (1 : ℝ) else 0) := by
            ...
            · intro j
              ...
              rw [show (∑ i, (μ i + (if i = i₀ then 1 else 0)) / lam * A i j)
                ...
                refine Finset.sum_congr rfl (fun i _ => ?_); ring]
              rw [div_le_iff₀ hlam_pos]; linarith
          have hwd := weak_duality_sum A b c hx₀A hx₀nn hu'_du
          linarith [hx₀_val]
        linarith
      · -- (Ax₀ - b)_{i₀} + u_{i₀} > 0.
        change 0 < (∑ j, A i₀ j * x₀ j - b i₀) + (μ i₀ + (if i₀ = i₀ then 1 else 0)) / lam
        have hu_i₀_pos : 0 < (μ i₀ + (if i₀ = i₀ then 1 else 0)) / lam := by
          have : μ i₀ + (if i₀ = i₀ then (1 : ℝ) else 0) ≥ 1 := by
    ...
    · -- Sub-case B.2: lam ≤ 0 (i.e., lam = 0 since lam ≥ 0).
      ...
      · -- MaxDualFeasible
        ...
        · intro i
          ...
          have h3 : 0 ≤ (if i = i₀ then (1 : ℝ) else 0) := by split_ifs <;> simp
          linarith
        · intro j
          ...
          have hsel : (∑ i, (if i = i₀ then (1 : ℝ) else 0) * A i j) = A i₀ j := by
            simp
          change (∑ i, (u₀ i + μ i + (if i = i₀ then 1 else 0)) * A i j) ≤ c j
          rw [hsplit, hsel]
          linarith
      · -- ⟨u', b⟩ = v.
        ...
        have hu'_du : MaxDualFeasible A c (fun i => u₀ i + μ i + (if i = i₀ then 1 else 0)) := by
          refine ⟨?_, ?_⟩
          · intro i
            have h1 : 0 ≤ u₀ i := hu₀.1 i
            have h2 : 0 ≤ μ i := hμ_nn i
          ...
          · intro j
            ...
            have hsplit : (∑ i, (u₀ i + μ i + (if i = i₀ then 1 else 0)) * A i j)
              ...
              rw [← Finset.sum_add_distrib, ← Finset.sum_add_distrib]
              refine Finset.sum_congr rfl (fun i _ => ?_); ring
            have hsel2 : (∑ i, (if i = i₀ then (1 : ℝ) else 0) * A i j) = A i₀ j := by
              simp
            change (∑ i, (u₀ i + μ i + (if i = i₀ then 1 else 0)) * A i j) ≤ c j
            rw [hsplit, hsel2]
            linarith
...
theorem exists_col_strict_pair
  ...
  · obtain ⟨x, hxA, hxnn, hcx, hstrict⟩ := hCaseA
    refine ⟨x, u₀, hxA, hxnn, hu₀, hcx, hu₀_val, ?_⟩
    have hu₀_col := hu₀.2 j₀
    change (∑ i, u₀ i * A i j₀) ≤ c j₀ at hu₀_col
    have hu₀_slack : 0 ≤ c j₀ - ∑ i, u₀ i * A i j₀ := by linarith
    linarith
  · push Not at hCaseA
    have hCaseA' : ∀ x : Fin n → ℝ,
        (∀ idx, optAugB b v idx ≤ ∑ j, optAugA A c idx j * x j) →
          (0 : ℝ) ≤ ∑ j, (if j = j₀ then (-1 : ℝ) else 0) * x j := by
      intro x hx
      obtain ⟨hxA, hxnn, hcx_le⟩ := (optAug_feasible_iff A b c v x).mp hx
      have hcx_ge : v ≤ ∑ j, c j * x j := by
        have hwd := weak_duality_sum A b c hxA hxnn hu₀
    ...
    have hCert :=
      (Math.LinearAlgebra.farkas_lemma (optAugA A c) (optAugB b v)
        (fun j => if j = j₀ then (-1 : ℝ) else 0) 0 hAug_feas).mp hCaseA'
    obtain ⟨w, hw_nn, hw_col, hw_b⟩ := hCert
    set μ : I → ℝ := fun i => w (Sum.inl (Sum.inl i)) with hμ_def
    set ν : Fin n → ℝ := fun j' => w (Sum.inl (Sum.inr j')) with hν_def
    set lam : ℝ := w (Sum.inr ()) with hlam_def
    ...
    have sum_split : ∀ (f : OptAugRow I n → ℝ),
      ...
      have hunit : (∑ u : Unit, f (Sum.inr u)) = f (Sum.inr ()) := by
        rw [show (Finset.univ : Finset Unit) = {()} from rfl, Finset.sum_singleton]
      rw [hunit]
    have hcol : ∀ j, (∑ i, μ i * A i j) + ν j - lam * c j = (if j = j₀ then (-1 : ℝ) else 0) := by
      intro j
      have h := hw_col j
      rw [sum_split] at h
      simp only [optAugA_inl_inl, optAugA_inl_inr, optAugA_inr, mul_ite, mul_one,
    ...
    · -- Sub-case B.1: lam > 0. Set u = μ / lam.
      ...
      · -- ⟨u, b⟩ = v.
        have hub_ge_v : v ≤ ∑ i, μ i / lam * b i := by
          ...
          rw [show (∑ i, μ i / lam * b i) = (∑ i, μ i * b i) / lam from by
            ...
            refine Finset.sum_congr rfl (fun i _ => ?_); ring]
          rw [le_div_iff₀ hlam_pos]; linarith
        have hu'_du : MaxDualFeasible A c (fun i => μ i / lam) := by
          refine ⟨fun i => div_nonneg (hμ_nn i) hlam_pos.le, ?_⟩
          intro j
          have hkey : (∑ i, μ i * A i j) ≤ lam * c j := by
            have hcol_j := hcol j
      ...
      · -- (c - u'ᵀA)_{j₀} > 0 + x₀_{j₀} ≥ 0.
        change 0 < x₀ j₀ + (c j₀ - ∑ i, μ i / lam * A i j₀)
    ...
    · -- Sub-case B.2: lam = 0.
      ...
      · -- MaxDualFeasible
        ...
        by_cases hjj : j = j₀
        · subst j
          have hcol_j' : (∑ i, μ i * A i j₀) + ν j₀ = -1 := by
            simpa using hcol_j
          have hμA_neg : (∑ i, μ i * A i j₀) ≤ -1 := by
            linarith [hν_nn j₀, hcol_j']
          have hsplit : (∑ i, (u₀ i + μ i) * A i j₀)
              = (∑ i, u₀ i * A i j₀) + (∑ i, μ i * A i j₀) := by
            rw [← Finset.sum_add_distrib]
            refine Finset.sum_congr rfl (fun i _ => ?_); ring
          change (∑ i, (u₀ i + μ i) * A i j₀) ≤ c j₀
          rw [hsplit]; linarith [h_u₀A]
        · have hcol_j' : (∑ i, μ i * A i j) + ν j = 0 := by
            simpa [hjj] using hcol_j
          have hμA_nn : (∑ i, μ i * A i j) ≤ 0 := by
            linarith [hν_nn j, hcol_j']
          have hsplit : (∑ i, (u₀ i + μ i) * A i j)
              = (∑ i, u₀ i * A i j) + (∑ i, μ i * A i j) := by
            rw [← Finset.sum_add_distrib]
      ...
      · -- ⟨u', b⟩ = v.
        ...
        have hu'_du : MaxDualFeasible A c (fun i => u₀ i + μ i) := by
          refine ⟨fun i => by linarith [hu₀.1 i, hμ_nn i], ?_⟩
          intro j
          have hcol_j := hcol j
          rw [hlam_zero] at hcol_j
      ...
      · -- (c - u'ᵀA)_{j₀} > 0.
        change 0 < x₀ j₀ + (c j₀ - ∑ i, (u₀ i + μ i) * A i j₀)
        have hcol_j₀ : (∑ i, μ i * A i j₀) = -1 - ν j₀ := by
          have h : (∑ i, μ i * A i j₀) + ν j₀ - lam * c j₀ = -1 := by
            simpa using hcol j₀
...
theorem exists_strong_complementary_pair
    (A : I → Fin n → ℝ) (b : I → ℝ) (c : Fin n → ℝ) (v : ℝ)
    (hN_pos : (0 : ℝ) < ((Fintype.card I + n : ℕ) : ℝ))
    {x₀ : Fin n → ℝ}
    (hx₀A : ∀ i, b i ≤ ∑ j, A i j * x₀ j)
    (hx₀nn : ∀ j, 0 ≤ x₀ j) (hx₀_val : ∑ j, c j * x₀ j = v)
    {u₀ : I → ℝ} (hu₀ : MaxDualFeasible A c u₀) (hu₀_val : ∑ i, u₀ i * b i = v) :
  ...
  -- Cast N to ℝ.
  set N : ℝ := ((Fintype.card I + n : ℕ) : ℝ) with hN_def
  ...
  have hN_card : (Fintype.card I : ℝ) + (n : ℝ) = N := by
  ...
  · rw [hcx_avg]
    ...
    rw [show ((Fintype.card I : ℝ) * v + n * v) = N * v from by
      rw [← hN_card]; ring]
    field_simp
  -- (5) ⟨u_avg, b⟩ = v.
  · rw [hub_avg]
  ...
  · intro i
    ...
    rw [show N * b i = (Fintype.card I : ℝ) * b i + (n : ℝ) * b i from by
      rw [← hN_card]; ring]
    ...
    have hR_sum_id : (∑ i', ((∑ j, A i j * xR i' j) - b i + uR i' i))
        = (∑ i', ∑ j, A i j * xR i' j) + (∑ i', uR i' i) - (Fintype.card I : ℝ) * b i := by
      ...
      rw [show (∑ _i' : I, b i) = (Fintype.card I : ℝ) * b i from by
        rw [Finset.sum_const, Finset.card_univ, nsmul_eq_mul]]
    have hC_sum_id : (∑ j', ((∑ j, A i j * xC j' j) - b i + uC j' i))
        = (∑ j', ∑ j, A i j * xC j' j) + (∑ j', uC j' i) - (n : ℝ) * b i := by
      ...
      rw [show (∑ j', ((∑ j, A i j * xC j' j) + uC j' i))
          = (∑ j', ∑ j, A i j * xC j' j) + (∑ j', uC j' i) from
            Finset.sum_add_distrib]
      rw [show (∑ _j' : Fin n, b i) = (n : ℝ) * b i from by
        rw [Finset.sum_const, Finset.card_univ, Fintype.card_fin, nsmul_eq_mul]]
  ...
  · intro j
    ...
    rw [show ((∑ i', xR i' j) + (∑ j'', xC j'' j)) / N
      ...
      field_simp; ring]
    apply div_pos _ hN_pos
    rw [show N * c j = (Fintype.card I : ℝ) * c j + (n : ℝ) * c j from by
      rw [← hN_card]; ring]
    have hR_term : ∀ i', 0 ≤ (xR i' j + (c j - ∑ i, uR i' i * A i j)) := fun i' => by
      have hdu := (huR_du i').2 j
      change (∑ i, uR i' i * A i j) ≤ c j at hdu
      linarith [hxR_nn i' j, hdu]
    have hC_term : ∀ j'', 0 ≤ (xC j'' j + (c j - ∑ i, uC j'' i * A i j)) := fun j'' => by
      have hdu := (huC_du j'').2 j
      change (∑ i, uC j'' i * A i j) ≤ c j at hdu
      linarith [hxC_nn j'' j, hdu]
    have hC_strict_j : 0 < (xC j j + (c j - ∑ i, uC j i * A i j)) := hC_strict j
    have hR_sum_id : (∑ i', (xR i' j + (c j - ∑ i, uR i' i * A i j)))
        = (∑ i', xR i' j) + (Fintype.card I : ℝ) * c j - (∑ i', ∑ i, uR i' i * A i j) := by
      ...
      rw [show (∑ i', (xR i' j + c j)) = (∑ i', xR i' j) + (∑ _i' : I, c j) from
            Finset.sum_add_distrib]
      rw [show (∑ _i' : I, c j) = (Fintype.card I : ℝ) * c j from by
        rw [Finset.sum_const, Finset.card_univ, nsmul_eq_mul]]
    have hC_sum_id : (∑ j'', (xC j'' j + (c j - ∑ i, uC j'' i * A i j)))
        = (∑ j'', xC j'' j) + (n : ℝ) * c j - (∑ j'', ∑ i, uC j'' i * A i j) := by
      ...
      rw [show (∑ j'', (xC j'' j + c j)) = (∑ j'', xC j'' j) + (∑ _j'' : Fin n, c j) from
            Finset.sum_add_distrib]
      rw [show (∑ _j'' : Fin n, c j) = (n : ℝ) * c j from by
        rw [Finset.sum_const, Finset.card_univ, Fintype.card_fin, nsmul_eq_mul]]
