...
import Math.GateGTermination
import Math.GateGFactorCoverage
...
namespace Math
namespace GateGConvergenceScratch
...
theorem exists_analytic_complexBranch_of_formalRoot_of_derivative_ne_zero
    ...
    ∃ (k n : ℕ) (A : Polynomial ℂ) (c : ℂ)
      ...
      A = s.trunc n ∧
      c =
        (GateGTerminationScratch.formalTail n s).constantCoeff ∧
      R =
        GateGTerminationScratch.normalizedBivPolynomialTaylorTransform
          Q A n k ∧
      AnalyticAt ℂ z 0 ∧
      z 0 = c ∧
      A.eval 0 = s.constantCoeff ∧
      (∀ x w : ℂ,
        GateGTerminationScratch.bivEvalAt Q x
            (A.eval x + x ^ n * w) =
          x ^ (n + k) *
            GateGTerminationScratch.bivEvalAt R x w) ∧
      AnalyticAt ℂ
      ...
      (∀ᶠ x in 𝓝 (0 : ℂ),
        GateGTerminationScratch.bivEvalAt Q x
          (A.eval x + x ^ n * z x) = 0) ∧
      (∀ᶠ w in 𝓝 ((0, c) : ℂ × ℂ),
        GateGTerminationScratch.bivEvalAt R w.1 w.2 = 0 ↔
          z w.1 = w.2) := by
  let f := bivPolynomialToPowerSeriesPolynomial Q
  let k := (f.derivative.eval s).order.toNat
  let n := k + 1
  let A := s.trunc n
  let tail := GateGTerminationScratch.formalTail n s
  let c := tail.constantCoeff
  let R :=
    GateGTerminationScratch.normalizedBivPolynomialTaylorTransform
      Q A n k
  obtain ⟨hA0, htransform, hface, hsimple⟩ :=
    GateGTerminationScratch.normalizedBivPolynomialTaylorTransform_has_simple_specialFiber
      Q s hroot hD
    ...
    exists_analytic_complexBivPolynomial_root_of_simple_specialFiber
      R c (by
        simpa [complexBivEval,
          GateGTerminationScratch.bivEvalAt] using hface)
      (by
        simpa [complexBivEval,
          GateGTerminationScratch.bivEvalAt] using hsimple)
  have hzroot' :
      ∀ᶠ x in 𝓝 (0 : ℂ),
        GateGTerminationScratch.bivEvalAt R x (z x) = 0 := by
    simpa [complexBivEval,
      GateGTerminationScratch.bivEvalAt] using hzroot
  have hzunique' :
      ∀ᶠ w in 𝓝 ((0, c) : ℂ × ℂ),
        GateGTerminationScratch.bivEvalAt R w.1 w.2 = 0 ↔
          z w.1 = w.2 := by
    simpa [complexBivEval,
      GateGTerminationScratch.bivEvalAt] using hzunique
  have hgamma :
      AnalyticAt ℂ
...
theorem
  ...
  let R :=
    GateGTerminationScratch.normalizedBivPolynomialTaylorTransform
      Q A n k
  obtain ⟨hA0, htransform, hface, hsimple⟩ :=
    GateGTerminationScratch.normalizedBivPolynomialTaylorTransform_has_simple_specialFiber_of_lt
...
theorem bivEvalAt_ramifyBivPolynomial
    ...
    GateGTerminationScratch.bivEvalAt
        (ramifyBivPolynomial p Q) t y =
      GateGTerminationScratch.bivEvalAt Q (t ^ p) y := by
  have hhom :
    ...
    · intro a
      simp
    · simp
  simp [GateGTerminationScratch.bivEvalAt,
...
theorem
    ...
    ∃ (q : J → Polynomial (Polynomial ℝ))
      ...
      (∀ j,
        q j ∈
            GateGFactorCoverageScratch.irreducibleFactors (Q j) ∧
        Irreducible (q j) ∧
        0 < (q j).natDegree ∧
        q j ∣ Q j) ∧
  ...
  obtain ⟨q, ns, hq, hlimsub, hqroot⟩ :=
    _root_.Math.GateGFactorCoverageScratch.exists_irreducibleFactorTuple_subsequence_isRoot
...
theorem exists_analytic_complexBivPolynomial_root_of_formalRoot
    ...
    ∃ (q : Polynomial (Polynomial ℂ)) (γ : ℂ → ℂ),
      ...
      γ 0 = s.constantCoeff ∧
      (∀ᶠ x in 𝓝 (0 : ℂ),
        GateGTerminationScratch.bivEvalAt q x (γ x) = 0) ∧
      (∀ᶠ x in 𝓝 (0 : ℂ),
        GateGTerminationScratch.bivEvalAt Q x (γ x) = 0) := by
  ...
  have hQeventually :
    ...
    change
      GateGTerminationScratch.bivEvalAt q x (γ x) = 0 at hx
    calc
      GateGTerminationScratch.bivEvalAt Q x (γ x) =
          GateGTerminationScratch.bivEvalAt (q * T) x (γ x) := by
            rw [hQT]
      _ =
          GateGTerminationScratch.bivEvalAt q x (γ x) *
            GateGTerminationScratch.bivEvalAt T x (γ x) := by
              simp [GateGTerminationScratch.bivEvalAt]
