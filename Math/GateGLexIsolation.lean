...
namespace Math
namespace GateGLexIsolationScratch
...
theorem exists_permanentMultipliers_of_separableLexRelations
    ...
    (hindependent :
      LinearIndependent ℝ
        (fun i : I =>
          GateGNormalLagrangeScratch.evalGradient
            (P i) x)) :
    ∃ Λ : Fin n → I → ℝ,
      ∀ (j : Fin n) (k : σ),
  ...
  have hlocal :
    ...
    · exact hlex j
  exact
    Math.GateGNormalLagrangeScratch.exists_permanentMultipliers_of_localExtrOn
      x P Q hlocal hindependent

end GateGLexIsolationScratch
end Math
