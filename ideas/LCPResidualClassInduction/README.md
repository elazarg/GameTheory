...
settled independently (Solan 1999), so the class is only interesting from four
players up.
...
| --- | --- | --- | --- | --- | --- |
| C1 | The open range of the finite-quitting conjecture is contained in `{M : M is Q, some proper principal minor of M is not Q}`. | `OPEN` | `I` | Finite quitting games, `n ≥ 4`; `ε`-equilibria for every `ε > 0`. | The conjecture. |
