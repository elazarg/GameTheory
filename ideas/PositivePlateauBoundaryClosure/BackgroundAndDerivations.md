...
| PB10 | The displayed standard-proper path axioms have been matched to a complete path-to-profile proof including the negative-solo Never fallback. | `CONDITIONAL` | `I+M` | The literature theorems have exact hypotheses — Ashkenazi-Golan–Krasikov–Rainer–Solan's absorption-path sequential-perfectness framework and Solan–Vieille (2001) Propositions 2.4/2.6's local-to-global alternative, both audited in [`SourceCorrections-QuittingAbsorptionPaths`](../../docs/uniform-equilibrium/references/SourceCorrections-QuittingAbsorptionPaths.md) — and Q132 contains only a citation-dependent sketch against them and no checked adapter. | R3 compiler. |
