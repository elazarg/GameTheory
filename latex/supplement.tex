...
\TT{Concepts/Equilibrium/ApproximateNash.lean}.  \TT{IsEpsilonNash $\varepsilon$}
...
def ActionPosteriorLocal            (O : ObsModelCore $\ldots$) : Prop\\
def PerStepPlayerRecall             (O : ObsModelCore $\ldots$) : Prop\\[3pt]
theorem ObsModelCore.kuhn\_behavioral\_to\_mixed\\
\bd (O : ObsModelCore $\ldots$) [$\ldots$] (hH : HorizonSeparation O) (hR : NoNontrivialInfoStateRepeat O) :\\
...
\bd (O : ObsModelCore $\ldots$) [$\ldots$] (hL : ActionPosteriorLocal O) :\\
...
\label{sec:supp:info-design}\label{def:SignalStructure}\label{def:BayesPlausible}\label{def:PersuasionProblem}

\Anchor{SignalStructure, BayesPlausible, PersuasionProblem}
\begin{leanbox}
...
def BayesPlausible (prior : PMF $\Omega$) ($\nu$ : PMF ($\Omega \times$ Msg)) : Prop :=\\
\bd $\nu$.map Prod.fst = prior\\[3pt]
theorem joint\_bayesPlausible (S : SignalStructure $\Omega$ Msg) (prior : PMF $\Omega$) :\\
\bd BayesPlausible prior (S.joint prior)\\[3pt]
