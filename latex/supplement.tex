...
content; the accompanying file/path reference identifies the
verbatim source, and declaration anchors link back to the local
summary.
...
\TT{Math/} &
  ...
  finite-carrier transport, KKM covers, unit-interval measure/cut
  lemmas, optimization primitives, and the standalone
  \TT{Math.Schauder.schauder_fixed_point} theorem. \\
\TT{Core/} &
  \DefRef{GameForm}, \DefRef{KernelGame}, the \DefRef{ProtocolMap}
  morphism vocabulary, simulations, isomorphisms, inert extensions,
  and protocol-level babbling/cheap-talk constructions. \\
\TT{Concepts/} &
  Solution concepts:\ Nash, dominance, best response, correlated /
  coarse correlated, Pareto, security, regret, approximate Nash,
  babbling equilibria, potential, zero-sum, team, symmetric,
  rationalizability, common knowledge, EU/preference properties,
  and solution-concept transport theorems. \\
\TT{Theorems/} &
  ...
  \DefRef{ObsModelCore} Kuhn theorems, learning-to-CCE results,
  the discounted folk theorem, finite vNM representation, Shapley
  uniqueness, Gale--Shapley stable matching, and the stable-matching
  lattice. \\
\TT{Languages/} &
...
\TT{Mechanism/} &
  Bayesian games, finite information design, mechanism design,
  the revelation principle, Myerson payments, social-choice fragments,
  and fair division. \\
\TT{Auctions/} &
  Vickrey, ReserveVickrey, FirstPrice, AllPay, VCG, and Knapsack. \\
\bottomrule
\end{tabular}
\end{center}
...
\TT{Math/FixedPoint/KKM.lean} proves \TT{kkm\_closed\_cover} and
\TT{kkm\_open\_cover} for covers of the standard simplex.
\TT{Math/MeasureTheory/UnitInterval.lean} supplies
\TT{unitInterval\_cut\_exists} and the subtype-pushforward/CDF
continuity lemmas used by divisible fair division.
\TT{Math/Fintype.lean}, \TT{Math/Fin.lean}, and
\TT{Math/Fintype/Transport.lean} package finite-cardinality and
\TT{Fin n} transport lemmas used to state finite-agent theorems over
...
\TT{deviateDistribution}, \TT{deviateDistributionFn} (push a
distribution through a deviation, by constant strategy or by a
function of the recommendation); \TT{constDeviateDistributionFn};
the bridge \TT{withUtility} adjoining a
...
\TT{withUtility} adjoins a chosen utility to a game form.  The
expected round-trip equalities hold with that chosen utility:
forgetting after adjoining recovers the game form, and adjoining a
kernel game's own utility after forgetting recovers the kernel game.
...
\TT{Concepts/Foundations/OfEUProperties.lean} record that
...
\Variants
\TT{KernelGame.Morphism} (\TT{Core/GameMorphism.lean}) is the
kernel-game analogue:\ a per-player strategy map preserving the
induced joint utility distribution \TT{udist}.  \TT{EUMorphism}
adds pointwise expected-utility preservation.  A
\TT{KernelGame.Simulation} is a kernel-game morphism (the names
coincide).  A \TT{KernelGame.Bisimulation} is a game isomorphism;
the EU-preserving version is \TT{EUGameIsomorphism}.  A
...
are proved as \TT{simp}-lemmas in \TT{Core/GameForm.lean} and
\TT{Core/GameMorphism.lean}; their content is the
...
of \TT{IsDeviationEqFor} for an explicit choice of deviation family
$\Delta$ over a type $U$ of deviating units.  We anchor at $\IsNash$
(\S\ref{sec:supp:isNash}) and $\IsCorrelatedEq$
(\S\ref{sec:supp:isCE}); other concepts are bunched.

\subsection{Deviation-family schema}
\label{sec:supp:deviation-schema}\label{def:NoProfitableDeviationFor}\label{def:DeviationFamily}\label{def:IsDeviationEqFor}

\noindent\TT{Concepts/Transport/Deviation.lean} and
\TT{Concepts/Equilibrium/GameFormSolutionConcepts.lean}
...
structure DeviationFamily (F : GameForm $\iota$) (U : Type) where\\
\bd Dev      : U $\to$ Type\\
\bd deviate  : PMF F.Profile $\to$ $\forall$ u : U, Dev u $\to$ PMF F.Profile\\[3pt]
def IsDeviationEqFor (F : GameForm $\iota$) \{U : Type\}\\
\bd (pref : U $\to$ PMF F.Outcome $\to$ PMF F.Outcome $\to$ Prop)\\
\bd ($\mu$ : PMF F.Profile) ($\Delta$ : DeviationFamily F U) : Prop :=\\
\bdd $\forall$ u : U, $\forall$ d : $\Delta$.Dev u,\\
\bdd \bd pref u (F.correlatedOutcome $\mu$) (F.correlatedOutcome ($\Delta$.deviate $\mu$ u d))\\[3pt]
noncomputable def constantDeviationProfileFamily  (F : GameForm $\iota$) : DeviationFamily F $\iota$\\
noncomputable def recommendationDeviationFamily    (F : GameForm $\iota$) : DeviationFamily F $\iota$\\
noncomputable def coalitionConstantDeviationFamily (F : GameForm $\iota$) : DeviationFamily F (Coalition $\iota$)
\end{leanbox}

\Mathematical
A \emph{deviation family} over a type $U$ of \emph{deviating units}
assigns to each unit $u$ a type $\mathrm{Dev}(u)$ of deviations and a
function transforming a status-quo profile distribution into a
post-deviation profile distribution given a chosen deviation.  Taking
$U = \Players$ recovers unilateral (single-player) deviations; taking
$U = \mathrm{Coalition}\,\Players$ (a nonempty finite set of players)
gives coalitional ones.  A profile distribution $\mu$ is a
$\Delta$-equilibrium for the per-unit preference $\Pref$ when no unit
has a profitable deviation:
\[
  \forall u \in U,\,\forall d \in \mathrm{Dev}(u).\quad
    \Pref_u\bigl(\GF.\mathrm{correlatedOutcome}(\mu),\,
                 \GF.\mathrm{correlatedOutcome}(\Delta.\mathrm{deviate}(\mu, u, d))\bigr).
\]
The three named families are $\mathcal{D}^{\mathrm{const}}$ (unilateral
deviation by a fixed strategy; \TT{constantDeviationProfileFamily}),
$\mathcal{D}^{\mathrm{dep}}$ (unilateral deviation as a function of the
recommendation; \TT{recommendationDeviationFamily}), and
$\mathcal{D}^{\mathrm{coal}}$ (a coalition's joint fixed-strategy
substitution; \TT{coalitionConstantDeviationFamily}), the first two over
$U = \Players$ and the last over $U = \mathrm{Coalition}\,\Players$.

A \emph{family morphism} \TT{DeviationFamily.Hom}~$m$ reindexes
deviating units along $m : U' \to U$, mapping each source deviation to a
target deviation of the image unit that induces the \emph{same}
post-deviation distribution; equilibrium transports along it
(\TT{DeviationFamily.Hom.isDeviationEqFor}).  The singleton-coalition
morphism \TT{constantToCoalitionHom} embeds the unilateral constant
family into the coalition constant family, making ``a unilateral
deviation is a singleton-coalition deviation'' a theorem.

\Textbook
The deviation-family schema is project-internal but the named families
are textbook:\ unilateral deviation by a fixed strategy (Nash, CCE),
recommendation-dependent deviation (Aumann's correlated
equilibrium~\citep{Aumann1974}), and coalitional deviation
(strong Nash / the core).  With the coalition Pareto-block aggregator
\TT{noParetoBlock} of the expected-utility preferences, strong Nash is
exactly the coalition-constant $\Delta$-equilibrium
(\TT{KernelGame.isStrongNash_iff_isDeviationEqFor}).

\subsection{Deviation-simulation transport}
\label{sec:supp:deviation-simulation}
\label{def:ViewFamily}\label{def:Realization}\label{def:Transport}\label{def:NashDeviationSimulation}

\noindent\TT{Concepts/Transport/} (general theory) and
\TT{Concepts/Transport/Corners.lean} (pure-profile Nash corner)

\Anchor{ViewFamily}
\begin{leanbox}
structure ViewFamily (F : GameForm $\iota$) (U : Type) ($\Omega$ : U $\to$ Type) where\\
\bd observe : $\forall$ u, F.Outcome $\to$ $\Omega$ u\\[3pt]
noncomputable def ViewFamily.claw (V : ViewFamily F U $\Omega$)\\
\bd (u : U) ($\mu$ : PMF F.Profile) : PMF ($\Omega$ u) :=\\
\bdd PMF.map (V.observe u) (F.correlatedOutcome $\mu$)\\[3pt]
def observedPref (V : ViewFamily F U $\Omega$)\\
\bd (prefOmega : $\forall$ u, PMF ($\Omega$ u) $\to$ PMF ($\Omega$ u) $\to$ Prop) :\\
\bd U $\to$ PMF F.Outcome $\to$ PMF F.Outcome $\to$ Prop :=\\
\bdd fun u d e $\Rightarrow$ prefOmega u (PMF.map (V.observe u) d) (PMF.map (V.observe u) e)
\end{leanbox}

\Mathematical
A \emph{view family} records, for each deviating unit $u$, a
deterministic observation $\mathrm{observe}\,u : \Outcome \to \Omega\,u$
of the game's outcome — what unit $u$ sees of realized play.  Its
\emph{observed correlated law} $\mathrm{claw}\,u\,\mu$ is the pushforward
of the correlated outcome under $\mu$ along that observation; the
pure-profile version $\mathrm{plaw}\,u\,\sigma$ uses $\K(\sigma)$ in
place of $\mathrm{correlatedOutcome}(\mu)$.  The induced preference
\TT{observedPref V prefOmega} compares native outcome laws only through
these per-unit pushforwards.  This is deliberately more general than
expected utility.  The EU corner is the \emph{utility-law view}
\TT{KernelGame.utilityView} (carrier $\Omega\,u = \RR$, observing the
outcome's utility), under which \TT{observedPref} at the expectation
preference \TT{expectationPref} is exactly the expected-utility
preference \TT{euPref} (\TT{KernelGame.observedPref_utilityView}); it
lives at the \KG\ level because a \GF\ carries no utility.  A
unit-independent \emph{global view} is \TT{ViewFamily.const}; coarsening
a view along a deterministic map is \TT{ViewFamily.garble}.

\begin{leanbox}
structure Realization (G H : GameForm $\iota$)\\
\bd (VG : ViewFamily G U $\Omega$) (VH : ViewFamily H U $\Omega$) where\\
\bd rel    : PMF G.Profile $\to$ PMF H.Profile $\to$ Prop\\
\bd law\_eq : $\forall$ \{$\mu_G$\} \{$\mu_H$\}, rel $\mu_G$ $\mu_H$ $\to$\\
\bdd $\forall$ u, VG.claw u $\mu_G$ = VH.claw u $\mu_H$\\[3pt]
def Realization.Simulates (R : Realization G H VG VH)\\
\bd ($\Delta_G$ : DeviationFamily G U) ($\Delta_H$ : DeviationFamily H U) : Prop :=\\
\bdd $\forall$ \{$\mu_G$\} \{$\mu_H$\}, R.rel $\mu_G$ $\mu_H$ $\to$\\
\bdd $\forall$ u (dH : $\Delta_H$.Dev u), $\exists$ dG : $\Delta_G$.Dev u,\\
\bdd \bd VG.claw u ($\Delta_G$.deviate $\mu_G$ u dG) = VH.claw u ($\Delta_H$.deviate $\mu_H$ u dH)\\[3pt]
structure Transport (G H : GameForm $\iota$)\\
\bd (VG : ViewFamily G U $\Omega$) (VH : ViewFamily H U $\Omega$)\\
\bd ($\Delta_G$ : DeviationFamily G U) ($\Delta_H$ : DeviationFamily H U)\\
\bd extends Realization G H VG VH where\\
\bd simulate : toRealization.Simulates $\Delta_G$ $\Delta_H$
\end{leanbox}

\Mathematical
A \emph{realization} is a law-preserving correspondence on profile
\emph{distributions}:\ a relation \TT{rel} whose related pairs induce
the same observed correlated law at every unit (\TT{law_eq}).  It is a
relation, not a map, because bridges may be existential or
many-to-many:\ a behavioral profile may be realized by several mixed
profiles, and a compiled trace profile may forget administrative
choices.  \emph{Backtranslation} is asserted separately:\
\TT{R.Simulates}~$\Delta_G$~$\Delta_H$ says every target-side deviation
in $\Delta_H$ is matched by a source-side deviation in $\Delta_G$
inducing the same observed law.  Because backtranslation is
existential, which family pairs a realization simulates cannot be read
off \TT{rel} and must be asserted; a single realization typically
simulates several pairs.  A \emph{transport} bundles a realization with
one chosen simulated pair.  Realizations and transports compose through
a shared middle game (\TT{Realization.comp}, \TT{Transport.comp},
\TT{Transport.compOfHom}); restricting the target family, enlarging the
source family, or coarsening the view all preserve simulation
(\TT{Simulates.mono_target}, \TT{Simulates.mono_source},
\TT{Simulates.garble}).

\begin{leanbox}
theorem Transport.target\_eq\_of\_source\_eq\\
\bd (T : Transport G H VG VH $\Delta_G$ $\Delta_H$) (hrel : T.rel $\mu_G$ $\mu_H$)\\
\bd (hEq : G.IsDeviationEqFor (observedPref VG prefOmega) $\mu_G$ $\Delta_G$) :\\
\bdd H.IsDeviationEqFor (observedPref VH prefOmega) $\mu_H$ $\Delta_H$\\[3pt]
theorem TransportBisimulation.eq\_iff\\
\bd (B : TransportBisimulation G H VG VH $\Delta_G$ $\Delta_H$) (hrel : B.rel $\mu_G$ $\mu_H$) :\\
\bdd G.IsDeviationEqFor (observedPref VG prefOmega) $\mu_G$ $\Delta_G$ $\leftrightarrow$\\
\bdd \bd H.IsDeviationEqFor (observedPref VH prefOmega) $\mu_H$ $\Delta_H$
\end{leanbox}

\Mathematical
There is a \emph{single} generic transfer theorem:\ along a transport
that simulates the family pair, a source deviation-family equilibrium
under observed preferences is a target one under the corresponding
observed preferences.  Every named solution-concept transport is an
instantiation of it — no concept keeps a bespoke transfer proof.  The
argument is per unit:\ backtranslate the target deviation
(\TT{simulate}) to a source one with equal observed law, then carry the
source inequality across by rewriting along the base \TT{law_eq} and the
backtranslation equality.  A \TT{TransportBisimulation} simulates the
pair in both directions and yields the iff (\TT{eq_iff}).

\begin{leanbox}
structure NashDeviationSimulation (G H : GameForm $\iota$) ($\Omega$ : Type) where\\
\bd viewG : ViewFamily G $\iota$ (fun \_ $\Rightarrow$ $\Omega$)\\
\bd viewH : ViewFamily H $\iota$ (fun \_ $\Rightarrow$ $\Omega$)\\
\bd rel   : G.Profile $\to$ H.Profile $\to$ Prop\\
\bd law\_eq : $\forall$ \{$\sigma$\} \{$\tau$\}, rel $\sigma$ $\tau$ $\to$ $\forall$ i, viewG.plaw i $\sigma$ = viewH.plaw i $\tau$\\
\bd simulate\_target\_deviation :\\
\bdd $\forall$ \{$\sigma$\} \{$\tau$\}, rel $\sigma$ $\tau$ $\to$ $\forall$ (who : $\iota$) (sH : H.Strategy who),\\
\bdd \bd $\exists$ sG : G.Strategy who,\\
\bdd \bd viewG.plaw who (Function.update $\sigma$ who sG) =\\
\bdd \bd \bd viewH.plaw who (Function.update $\tau$ who sH)\\[3pt]
theorem NashDeviationSimulation.target\_nash\_of\_source\_nash\\
\bd (S : NashDeviationSimulation G H $\Omega$) (hrel : S.rel $\sigma$ $\tau$)\\
\bd (hN : G.IsNashFor (observedPref S.viewG prefOmega) $\sigma$) :\\
\bdd H.IsNashFor (observedPref S.viewH prefOmega) $\tau$
\end{leanbox}

\Mathematical
The pure-profile, unilateral corner is where the theory reads most
naturally.  \TT{NashDeviationSimulation} fixes $U = \Players$ and a
single shared carrier $\Omega$ (a global view), relates \emph{pure}
profiles, and phrases \TT{law_eq} and \TT{simulate_target_deviation}
with the pure-profile law \TT{plaw} and \TT{Function.update}.  Nash
transport \TT{target_nash_of_source_nash} is the instantiation of the
generic transfer theorem at the constant-deviation family evaluated at a
point mass, since \TT{IsNashFor} unfolds to exactly that; the two-way
\TT{NashDeviationBisimulation} yields \TT{nash_iff}.  The correlated and
coarse-correlated corners are built from a pointwise profile map by
\TT{Transport.ofRecommendationProfileMap} and
\TT{Transport.ofConstantProfileMap}, and transported by
\TT{Transport.target_correlatedEq_of_source_correlatedEq} and
\TT{Transport.target_coarseCorrelatedEq_of_source_coarseCorrelatedEq}.
At the \KG\ level, with both views the utility-law view,
\TT{KernelGame.NashDeviationSimulation.target_isNash_of_source_isNash}
specializes to the \TT{eu}-based \TT{IsNash}.

\Faithfulness
No finiteness assumptions appear in these transport theorems; they use
only exact equality of pushed-forward \TT{PMF} laws, so finite-outcome
hypotheses remain localized to existence theorems and expected-value
lemmas.  The coalition instantiation
\TT{Transport.target_strongNashEq_of_source_strongNashEq} (via
\TT{observedPref_noParetoBlock_const}) transports strong-Nash
equilibria, and a serialization order is not a new structure but a
quantifier:\ an index type of orders and a shared-source family of
transports whose observed law is index-independent
(\TT{Transport.ofIndexIndependent}, \TT{transport_under_every_index},
\TT{claw_index_independent} in \TT{Transport/Oblivious.lean}).

\subsection{Babbling and cheap-talk extensions}
\label{sec:supp:babbling}
\label{def:CheapTalkExtension}\label{def:BabblingDeviationPreservingFor}

\begin{leanbox}
structure CheapTalkExtension (F : GameForm $\iota$) where\\
\bd Msg      : $\iota \to$ Type\\
\bd default  : $\forall$ i, Msg i\\[3pt]
def CheapTalkExtension.Strategy' (C : CheapTalkExtension F) (i : $\iota$) : Type :=\\
\bdd C.Msg i $\times$ (($\forall$ j : $\iota$, C.Msg j) $\to$ F.Strategy i)\\[3pt]
def BabblingDeviationPreservingFor (F H : GameForm $\iota$)\\
\bd (prefF : $\iota \to$ PMF F.Outcome $\to$ PMF F.Outcome $\to$ Prop)\\
\bd (prefH : $\iota \to$ PMF H.Outcome $\to$ PMF H.Outcome $\to$ Prop)\\
\bd (proj : $\forall$ i, H.Strategy i $\to$ F.Strategy i)\\
\bd (embed : $\forall$ i, F.Strategy i $\to$ H.Strategy i) : Prop
\end{leanbox}

\Mathematical
An observable cheap-talk strategy is a message together with a
contingent action plan, evaluated after all messages are observed.
The canonical babbling embedding sends the default message and
uses a constant action plan.  The projection of a unilateral
enriched deviation is the base action induced when all other
players babble at their defaults.

The preservation predicate is local around babbling profiles: it
does not require global equality of payoffs or outcome laws between
the base and enriched games.  This distinction matters because
observable cheap talk can create additional non-babbling
coordination equilibria.  The theorem proves only the standard
one-way implication:
\[
  \GF.\IsNashFor(\Pref,\Profile)
  \Longrightarrow
  \GF^{\mathrm{ct}}.\IsNashFor(\Pref,\mathrm{embed}(\Profile)).
\]

\Textbook
This is the babbling-equilibrium observation for cheap-talk games:
if all players ignore the message channel, every unilateral
deviation is payoff-equivalent to a base-game unilateral deviation.
The library states the result at the \TT{GameForm} level for
arbitrary preferences and then provides the EU-specialized
\TT{KernelGame} corollary.

\leancorr{\TT{Core/Babbling.lean} defines \TT{CheapTalkExtension},
\TT{proj}, \TT{embed}, and \TT{BabblingDeviationPreservingFor};
\TT{Concepts/Communication/Babbling.lean} proves \TT{babbling\_nashFor},
\TT{KernelGame.babbling\_nash}, and the observable cheap-talk
instance; \TT{Languages/NFG/CheapTalkExamples.lean} gives a Battle
of the Sexes example.}
...
(\TT{Concepts/Equilibrium/GameFormSolutionConcepts.lean}) generalizes \eqref{eq:supp:isNash} to
...
\item \TT{IsDominant(For)} / \TT{IsStrictDominant(For)} — the
  ...
  profile.
\item \TT{WeaklyDominatesReflexive(For)} /
  \TT{WeaklyDominatesWithStrictWitness} / \TT{StrictlyDominates(For)} —
  pairwise dominance as binary predicates on strategies, respectively meaning
  weak everywhere, weak everywhere with a strict witness, and strict
  everywhere.
\end{itemize}
...
ex-post (conditional) form is \TT{Concepts/Correlation/CorrelatedEqProperties.lean}'s
...
\TT{Concepts/Equilibrium/NashPareto.lean}, \TT{Concepts/Welfare/WelfareTheorems.lean},
\TT{Concepts/Welfare/PriceOfAnarchy.lean}.  \TT{ParetoDominatesFor} is the
...
the ratio-valued \TT{priceOfAnarchy} and
\TT{priceOfStability} definitions are packaged against a
nonempty-Nash hypothesis
(\citep{Koutsoupias1999, Roughgarden2005}).

\paragraph{Security and individual rationality.}\
\TT{Concepts/ZeroSum/SecurityStrategy.lean}, \TT{Concepts/Welfare/IndividualRationality.lean}.
...
rationality says the relevant profile payoff meets the player's
reservation/security threshold (Maschler–Solan–Zamir §5.4).

\paragraph{Regret.}\
\TT{Concepts/Correlation/Regret.lean}.  External regret is the raw
gain from a constant deviation; swap regret replaces the constant
deviation by a function of the recommendation.  The
regret-equilibrium duality says CCE is equivalent to all external
regret gains being nonpositive, and CE is equivalent to all swap
regret gains being nonpositive.

\paragraph{Approximate equilibria.}\
\TT{Concepts/Equilibrium/ApproximateNash.lean}.  \TT{Is}$\varepsilon$\TT{Nash}
...
\TT{Concepts/Potential/PotentialGame.lean}, \TT{PotentialFIP.lean},
...
(\citep{MondererShapley1996}).

\paragraph{Zero-sum, constant-sum, team, symmetric, evolutionary.}\
\TT{Concepts/ZeroSum/ZeroSum.lean}, \TT{ConstantSum.lean},
...
predicates are similar.  \TT{ZeroSumNash.lean}, \TT{ConstantSumNash.lean}
record payoff-sum and deviation identities for two-player zero- and
constant-sum Nash profiles.  \TT{IsESS} is the evolutionary-stability
condition --- a Nash condition together with a stability inequality ---
on a symmetric two-player game, with strict Nash sufficient for it
(\TT{isESS\_of\_strict\_nash})~\citep{MaynardSmithPrice1973}.

\paragraph{Rationalizability and dominance solvability.}\
\TT{Concepts/Dominance/} contains \TT{Rationalizability.lean},
\TT{DominanceSolvable.lean}, and \TT{DominanceSolvability.lean}.
\TT{Survives n} is the
predicate "survives $n$ rounds of iterated strict dominance
elimination," defined inductively;  \TT{IsRationalizable} is
survival for every round, i.e.\ the intersection over all $n$.
\TT{IsNash.isRationalizable} and
\TT{IsDominant.isRationalizable} are the expected implications.
\TT{Concepts/Dominance/DominanceSolvable.lean} records that the unique
surviving profile (when one exists) is Nash.

\paragraph{Common knowledge.}\
\TT{Concepts/Knowledge/CommonKnowledge.lean}.  Partition-based knowledge
...
agree.  \TT{Concepts/Knowledge/ApproximateCommonKnowledge.lean}
adds \TT{PBelief}, \TT{IsPEvident}, and \TT{CommonPBeliefAt},
bridges exact common knowledge to common $p$-belief, and proves
\TT{commonPBelief_posterior_reports_close}:\ common $p$-belief of
posterior reports implies $|r_i-r_j| \le 2(1-p)$.
...
\bd utility  : Outcome $\to$ $\iota \to \mathbb{R}$\\[3pt]
...
subclasses:\ \TT{PublicGoods.lean} (free-ride dominance and
...
Congestion games live in the dedicated top-level \TT{Congestion/}
directory (\TT{Basic}, \TT{Rosenthal} — exact potential
\citep{Rosenthal1973}, pure Nash existence, FIP, weak acyclicity —
\TT{AffinePoA} with the smoothness-based $5/2$ price-of-anarchy
bound, and the Pigou/Braess instances in \TT{Examples}).
...
\bd $\mid$ \TT{.terminal} z $\Rightarrow$ PMF.pure z\\
\bd $\mid$ \TT{.chance} \_ $\mu$ \_ next $\Rightarrow$ $\mu$.bind (fun b $\Rightarrow$ (next b).evalDist $\sigma$)\\
\bd $\mid$ \TT{.decision} (p := p) I next $\Rightarrow$ ($\sigma$ p I).bind (fun a $\Rightarrow$ (next a).evalDist $\sigma$)
\end{leanbox}
...
than deterministic-with-lift.  See chapter §10.6 for the
...
\label{sec:supp:maid}\label{def:MAID.toKernelGame}

\Anchor{MAID.toKernelGame}
\begin{leanbox}
structure Struct (Player : Type) (n : Nat) where\\
\bd kind       : Fin n $\to$ NodeKind Player  -- chance / decision / utility\\
\bd parents    : Fin n $\to$ Finset (Fin n)\\
\bd obsParents : Fin n $\to$ Finset (Fin n)\\
\bd Val        : Fin n $\to$ Type             -- finite node domains\\
\bd acyclic    : DAG.Acyclic ($\cdot \in$ parents $\cdot$)\\[3pt]
structure Sem (S : Struct Player n) where\\
\bd chanceCPD : (c : ChanceNode S) $\to$ Cfg S (S.parents c.val) $\to$ PMF (S.Val c.val)\\
\bd utilityFn : (p) $\to$ (u : UtilityNode S p) $\to$ Cfg S (S.parents u.val) $\to \mathbb{R}$\\[3pt]
noncomputable def toKernelGame (S : Struct Player n) (sem : Sem S) : KernelGame Player
\end{leanbox}
...
§5.3 for the rationale on why we accepted the proof obligations of
...
def MultiRoundGame.ViewDeterminesRound (G : MultiRoundGame n S V A Sig) : Prop\\[3pt]
...
\Variants
\TT{Languages/MultiRound/RepeatedGame.lean} gives the
infinite-horizon public-action specialization with identical rounds
and discounted payoffs; \TT{StochasticGame.lean}~\citep{Shapley1953stochastic}
specializes the transition to be itself stochastic and
state-conditioned.  \TT{AbsentMindedDriver.lean} gives a two-round
single-player protocol in which both rounds produce the same view,
proves failure of \TT{ViewDeterminesRound}, and records the
standard payoff inconsistency:
\TT{exAnteOptimalContinue = 2/3} but
\TT{decisionOptimalContinue exAnteOptimalContinue = 1/3}.
\TT{CompileObs.lean} and its linearizations
...
\begin{leanbox}
structure FOSG ($\iota$ W : Type) [DecidableEq $\iota$]\\
\bd (Act : $\iota \to$ Type) (PrivObs : $\iota \to$ Type) (PubObs : Type) where\\
\bd init             : W\\
\bd active           : W $\to$ Finset $\iota$\\
\bd availableActions : (w : W) $\to$ (i : $\iota$) $\to$ Set (Act i)\\
\bd terminal         : W $\to$ Prop\\
\bd transition       : $\ldots$\\
\bd reward, privObs, pubObs : $\ldots$\\
\bd terminal\_active\_eq\_empty : $\ldots$\\
\bd terminal\_no\_legal, nonterminal\_exists\_legal : $\ldots$\\[3pt]
noncomputable def FOSG.toKernelGame (G : FOSG \dots) (hNorm : G.HasNormalizedTerminalLaw) : KernelGame $\iota$\\
noncomputable abbrev FOSG.toKernelGameOfBoundedHorizon (G : FOSG \dots) (hBound : G.BoundedHorizon k) : KernelGame $\iota$
\end{leanbox}
...
noncomputable def productMixedToBehavioral (G : WGame) : $\ldots$\\
theorem behavioral\_realizes\_productMixed (G : WGame) : $\ldots$\\[3pt]
...
\bd (p : G.P) (hpr : PerfectRecall G $\varphi$ p)\\
\bd ($\mu_p$ : MixedStrategy G p) :\\
...
For a solvable W-model equipped with a configuration ordering, the full
...
formal theorem supplies solvability directly for the outcome map.  Under
that stronger explicit assumption, no separate causality hypothesis is
needed for the conclusion; this is stronger than the paper's presentation.
...
at each.  Soundness at the outcome-law level:\
\TT{nfgTree\_evalDist\_pure} shows the EFG's \TT{evalDist} on each
pure profile is the Dirac on the corresponding NFG outcome, so the
compiled kernel games are utility-distribution equivalent, not merely
EU-equivalent.
...
Soundness is at the utility-distribution level:\
\TT{toFOSG\_morphism} lifts each pure NFG strategy to the behavioral
strategy that plays it at the initial information state (and the
forced no-op afterwards), yielding a \TT{KernelGame.Morphism} into
the bounded-horizon compiled FOSG that preserves the joint utility
distribution at every profile (\TT{toFOSG\_udist\_eq}:\ the horizon-$1$
run law collapses to the point mass on the single one-step history
carrying the NFG payoff).
...
its decision nodes.  Soundness is at the utility-distribution level:\
\TT{maidToEFGAt\_bisimulation} is a full kernel-game bisimulation
whose outcome kernels agree on the nose
(\TT{maidToEFGAt\_outcomeKernel}), and \TT{buildTree\_perfectRecall}
shows perfect recall is preserved, which transfers the Kuhn
behavioral$\leftrightarrow$mixed equivalence to MAIDs.
...
(\TT{Concepts/Foundations/GameMorphism.lean} and
\TT{Concepts/Foundations/UtilityInvariance.lean}) states that a protocol map with
...
(\TT{Concepts/Transport/}, with the pure-profile Nash corner
and canonical-family corollaries in
\TT{Concepts/Transport/Corners.lean}) removes the need for
an outcome bijection.  It compares two games through view families into
per-unit observed carriers $\Omega$, relates profile distributions by
equality of the pushed-forward observed law at every deviating unit
(\TT{Realization}), and separately requires one-way simulation of
target-family deviations by source-family ones
(\TT{Realization.Simulates}); a \TT{Transport} bundles the two.  The
single generic transfer theorem \TT{Transport.target_eq_of_source_eq}
then transfers \TT{IsDeviationEqFor} for an arbitrary
\TT{DeviationFamily} from source to target, and
\TT{TransportBisimulation.eq_iff} gives an iff.  Nash, correlated,
coarse-correlated, and strong-Nash transport are all instantiations at
specific families.  These results are the operational content of bridges
whose target presentation contains extra trace data or serialized
administrative structure:\ a Nash equilibrium of the abstract source
remains Nash after compilation provided all compiled unilateral
deviations are observed already by some source-side unilateral
deviation.
...
finite non-empty strategy space per
...
$\prod_i \Delta(\Strat_i)$.  Brouwer's fixed-point theorem from the
standalone \TT{fixed-point-theorems-lean4} package is lifted to the
product simplex by the wrapper in
\TT{Concepts/Existence/ProductSimplexBrouwer.lean}, giving a fixed point
$\Profile^*$.  The algebraic step in
\TT{Concepts/Existence/NashExistenceMixed.lean} (section
...
\paragraph{Uniform-balance corollary.}\
The same mixed-extension gain API also records a small finite-strategy
criterion:\ \TT{uniformMixedProfile} is the independent uniform profile,
\TT{IsUniformMixedBalanced} says that every pure unilateral deviation
has zero gain at that profile, and
\TT{IsUniformMixedBalanced.uniformMixedProfile\_isNash} proves that
such a balanced uniform profile is Nash.  This captures the row/column
``balanced payoff matrix'' calculation used by Matching Pennies without
making any zero-sum or symmetry assumption.

\paragraph{Binary matching-pennies calculus.}\
\TT{Concepts/Mixed/BinaryMixed.lean} factors out the reusable part of the
Matching Pennies calculation.  \TT{BinaryActionLabels} labels each
of the two players by \TT{Bool}, and also labels each player's two pure
strategies by \TT{Bool}; the file proves the binary PMF complement
identity, expands mixed-extension EU over the four pure label profiles,
and defines the bundled witness
\TT{MatchingPenniesLike} (with Prop wrapper
\TT{IsMatchingPenniesLike}) for the semantic payoff pattern ``matcher
wins scale, mismatcher loses scale, opponent gets the negative.''  The theorem
\TT{MatchingPenniesLike.mixed\_nash\_iff\_half} then states, at the
\KG level, that a mixed profile is Nash iff both players put probability
$1/2$ on the action labeled \TT{true}.  Language-specific examples only
need to compile to \KG, provide labels, and prove the four payoff
equations.
...
§12.1 as a future direction.

\subsection{Correlated and coarse correlated equilibrium existence}
\label{sec:supp:ce-exists}\label{def:correlatedEq_exists}

\begin{leanbox}
theorem KernelGame.correlatedEq\_exists\_of\_bounded       (G : KernelGame $\iota$) [$\ldots$]
theorem KernelGame.correlatedEq\_exists                   (G : KernelGame $\iota$) [$\ldots$]
theorem KernelGame.coarseCorrelatedEq\_exists\_of\_bounded (G : KernelGame $\iota$) [$\ldots$]
theorem KernelGame.coarseCorrelatedEq\_exists             (G : KernelGame $\iota$) [$\ldots$]
\end{leanbox}
...
corollary $\Pref$-CE $\subseteq$ $\Pref$-CCE\@.  The
\TT{\_of\_bounded} statements require finite strategy spaces but
not finite outcomes.

\paragraph{Constructive route:\ no-regret learning.}\
The complementary constructive route — the main chapter's
learning-dynamics theorems — is formalized.  The time-average of a
play sequence with cumulative external regret $R$ over horizon $T$
is an $(R/T)$-coarse correlated equilibrium
(\TT{KernelGame.timeAverage\_isεCCE\_of\_regret\_le} in
\TT{Concepts/Learning/NoRegretToCCE.lean}), and multiplicative-weights
self-play supplies a concrete such sequence with an explicit
$\varepsilon$
(\TT{KernelGame.mwSelfPlay\_timeAverage\_isεCCE},
\TT{KernelGame.exists\_mwSelfPlay\_isεCCE} in
\TT{Concepts/Learning/MWSelfPlay.lean}), built on the game-free
multiplicative-weights regret bound
\TT{Math.OnlineLearning.mw\_externalRegret\_le}.  The same set is
reached geometrically by regret matching, via Blackwell
approachability
(\TT{Math.Approachability.blackwell\_approaches},
\TT{regretMatch\_approaches}).  Unlike the Brouwer route this is a
per-horizon $\varepsilon$-bound, not an exact existence statement:\
for a fixed learning rate the residual $\varepsilon$ does not vanish.
...
\bd  -- world state, transition kernel, per-player observation, $\ldots$\\[3pt]
def NoNontrivialInfoStateRepeat (O : ObsModelCore $\ldots$) : Prop\\
def HorizonSeparation                (O : ObsModelCore $\ldots$) : Prop\\
def ActionPosteriorLocal            (O : ObsModelCore $\ldots$) (i : $\iota$) : Prop\\
def PerStepPlayerRecall             (O : ObsModel $\ldots$) : Prop\\[3pt]
theorem ObsModelCore.kuhn\_behavioral\_to\_mixed\\
\bd (O : ObsModelCore $\ldots$) [$\ldots$] (hR : NoNontrivialInfoStateRepeat O)\\
\bd ($\beta$ : BehavioralProfile O) (k : Nat) :\\
\bd runDist O k $\beta$ = (behavioralToMixedJoint $\beta$).bind (runDistPure O k)\\[3pt]
theorem ObsModelCore.kuhn\_mixed\_to\_behavioral\_semantic\\
\bd (O : ObsModelCore $\ldots$) (hMass : StepMassInvariant O) (hFactor : StepSupportFactorization O)\\
\bd (hL : $\forall$ i, ActionPosteriorLocal O i) ($\mu$ : $\forall$ i, PMF (LocalStrategy O i)) (k : Nat) :\\
\bd $\exists$ $\beta$, runDist O k $\beta$ = (pmfPi $\mu$).bind (runDistPure O k)
\end{leanbox}
...
The predicates that govern the two directions:
\begin{itemize}\itemsep1pt
...
\item \emph{HorizonSeparation}:\ a structural separation of
  information states by horizon; it holds automatically on
  snapshot-refined models and is not a separate hypothesis of the
  $B \to M$ theorem.
\item \emph{ActionPosteriorLocal}:\ the conditional distribution
  of an action given an information state does not depend on
  information outside the player's recall.  Together with the
  step-level regularity conditions \TT{StepMassInvariant} and
  \TT{StepSupportFactorization}, it drives the $M \to B$ direction.
  Strictly weaker than perfect recall in the textbook sense.
\item \emph{PerStepPlayerRecall}:\ an \TT{ObsModel}-level recall
  property that implies \TT{ActionPosteriorLocal} and is automatic
  for snapshot-refined information states.
\end{itemize}
...
strictly weaker than full perfect recall.  See chapter §7.7 for
the rationale.
...
\subsection{Additional theorem packages}
\label{sec:supp:additional-theorems}\label{def:vnmAxioms_iff_exists_representsExpectedUtility}\label{def:optionalStableJoin_isStable}

\paragraph{Finite vNM representation.}
\TT{Concepts/Foundations/VNM/Basic.lean} defines lotteries as
\TT{PMF} values and proves the finite binary-mixture representation
theorem.
\begin{leanbox}
def RepresentsExpectedUtility (pref : Lottery $\Omega \to$ Lottery $\Omega \to$ Prop) (u : $\Omega \to \mathbb{R}$) : Prop\\
\bd := $\forall$ L M, pref L M $\leftrightarrow$ expectedValue u L $\ge$ expectedValue u M\\[3pt]
theorem vnmAxioms\_iff\_exists\_representsExpectedUtility\\
\bd [Finite $\Omega$] [Nonempty $\Omega$] (pref : Lottery $\Omega \to$ Lottery $\Omega \to$ Prop) :\\
\bd Completeness pref $\wedge$ Transitivity pref $\wedge$ Independence pref $\wedge$ Continuity pref\\
\bdd $\leftrightarrow$ $\exists$ u : $\Omega \to \mathbb{R}$, RepresentsExpectedUtility pref u
\end{leanbox}

\paragraph{Stable-matching lattice.}
\TT{Cooperative/GaleShapley/OptionalLattice.lean} proves that the
pointwise optional-partner join of two stable matchings is stable and
packages the resulting lattice.
\begin{leanbox}
theorem optionalStableJoin\_isStable : M.IsStable (M.optionalStableJoin (μ := μ) (ν := ν))\\[3pt]
noncomputable def lattice : Lattice M.StableMatching
\end{leanbox}
The hypotheses are finite sides, strict/injective rankings, and
reservation values separated from all opposite-side partner scores.
The result is not restricted to balanced or complete-acceptability
markets.
...
\begin{leanbox}
structure BayesianGame ($\iota$ : Type) where\\
\bd $\Theta$       : $\iota \to$ Type\\
\bd Act     : $\iota \to$ Type\\
...
\mathrm{Act}_i$ replacing the player's strategy globally.  The type
and action spaces are arbitrary; the only discreteness is the
\TT{PMF} prior.

\subsection{Finite information design: \texttt{Mechanism/InformationDesign.lean}}
\label{sec:supp:info-design}\label{def:SignalStructure}\label{def:HasPriorMarginal}\label{def:PersuasionProblem}

\Anchor{SignalStructure, HasPriorMarginal, PersuasionProblem}
\begin{leanbox}
structure SignalStructure ($\Omega$ Msg : Type) where\\
\bd kernel : $\Omega \to$ PMF Msg\\[3pt]
noncomputable def joint (S : SignalStructure $\Omega$ Msg) (prior : PMF $\Omega$) : PMF ($\Omega \times$ Msg) :=\\
\bd prior.bind (fun $\omega$ $\Rightarrow$ (S.kernel $\omega$).map (fun m $\Rightarrow$ ($\omega$, m)))\\[3pt]
def HasPriorMarginal (prior : PMF $\Omega$) ($\nu$ : PMF ($\Omega \times$ Msg)) : Prop :=\\
\bd $\nu$.map Prod.fst = prior\\[3pt]
theorem joint\_hasPriorMarginal (S : SignalStructure $\Omega$ Msg) (prior : PMF $\Omega$) :\\
\bd HasPriorMarginal prior (S.joint prior)\\[3pt]
theorem messageMarginal\_apply [Fintype $\Omega$] [Finite Msg]\\
\bd (S : SignalStructure $\Omega$ Msg) (prior : PMF $\Omega$) (m : Msg) :\\
\bd S.messageMarginal prior m = $\sum\ \omega : \Omega,$ prior $\omega$ * S.kernel $\omega$ m\\[3pt]
structure PersuasionProblem ($\Omega$ Msg Act : Type) where\\
\bd prior : PMF $\Omega$ ; signal : SignalStructure $\Omega$ Msg\\
\bd senderUtility receiverUtility : $\Omega \to$ Act $\to \mathbb{R}$\\[3pt]
def IsPersuasive (P : PersuasionProblem $\Omega$ Msg Act) (r : Msg $\to$ Act) : Prop\\[3pt]
noncomputable def senderEU (P : PersuasionProblem $\Omega$ Msg Act) (r : Msg $\to$ Act) : $\mathbb{R}$\\[3pt]
theorem senderEU\_eq\_sum\_senderScore (P : PersuasionProblem $\Omega$ Msg Act) (r : Msg $\to$ Act) :\\
\bd P.senderEU r = $\sum\ m :$ Msg, P.senderScore m (r m)
\end{leanbox}

\Mathematical
A signal structure is a finite stochastic kernel from states to
messages.  With prior $\mu$, it induces the joint law
$\nu_S(\omega,m)=\mu(\omega)S(m\mid\omega)$ and the message marginal
$\nu_S^M(m)=\sum_\omega \mu(\omega)S(m\mid\omega)$.  Bayes
plausibility is the state-marginal identity $\nu_S^\Omega=\mu$.
A persuasion problem adds sender and receiver utility functions and
evaluates a message-contingent decision rule.  Receiver optimality
is expressed using unnormalized message weights, so
zero-probability messages do not require a special posterior
convention.

\Textbook
Kamenica--Gentzkow Bayesian persuasion
\citep{KamenicaGentzkow2011}; finite signal structures and
Bayes-plausible distributions are the discrete version of the
standard information-design primitives.

\Faithfulness
Faithful finite substrate.  The file does not prove concavification,
optimal signal existence, or signaling-game refinement theorems; it
provides the reusable finite definitions and the basic marginal
identity on which those results can be built.
...
\begin{leanbox}
structure GeneralMechanism ($\iota$ : Type) where\\
...
\subsection{Single-parameter Myerson payments}
\label{sec:supp:myerson}\label{def:withMyersonPayment_isDSIC_of_isMonotone}

\TT{Mechanism/Myerson.lean} specializes quasilinear mechanism design
to real single-parameter reports.
\begin{leanbox}
structure SingleParameterMechanism ($\iota$ : Type) where\\
\bd allocation : ($\iota \to \mathbb{R}$) $\to \iota \to \mathbb{R}$\\
\bd payment    : ($\iota \to \mathbb{R}$) $\to \iota \to \mathbb{R}$\\[3pt]
def AllocationIsMonotone (allocation : ($\iota \to \mathbb{R}$) $\to \iota \to \mathbb{R}$) : Prop\\[3pt]
noncomputable def myersonPayment (allocation : ($\iota \to \mathbb{R}$) $\to \iota \to \mathbb{R}$) :\\
\bd ($\iota \to \mathbb{R}$) $\to \iota \to \mathbb{R}$\\[3pt]
theorem isMonotone\_of\_isDSIC : M.IsDSIC $\to$ M.IsMonotone\\[3pt]
theorem withMyersonPayment\_isDSIC\_of\_isMonotone\\
\bd (allocation : ($\iota \to \mathbb{R}$) $\to \iota \to \mathbb{R}$)
   (hmono : AllocationIsMonotone allocation)
   (hint : IsEnvelopeIntegrable allocation) :\\
\bd (withMyersonPayment allocation).IsDSIC\\[3pt]
theorem existsUnique\_zeroNormalized\_payment\_of\_isMonotone $\ldots$
\end{leanbox}
The unique-payment theorem assumes monotonicity and continuity of
one-dimensional allocation slices; the DSIC construction needs only
monotonicity plus interval integrability.

\subsection{Fair division}
\label{sec:supp:fair-division}\label{def:envyCycleAllocation_isEF1}\label{def:stromquist_envyFree_exists}

\TT{Mechanism/FairDivision/Indivisible/} defines additive valuations
over finite indivisible goods and proves the EF/EF1/EFX/proportionality
and MMS facts used in the main chapter:
\begin{leanbox}
theorem envyCycleAllocation\_isEF1 $\ldots$\\
theorem roundRobinAllocation\_isEF1 $\ldots$\\
theorem exists\_efx\_two\_agents $\ldots$\\
theorem maximinShare\_le\_average $\ldots$
\end{leanbox}
\TT{Mechanism/FairDivision/Divisible/} works over measurable
allocations of the unit interval:
\begin{leanbox}
theorem cutAndChoose\_ef\_exists $\ldots$\\
theorem dubinsSpanierProportional $\ldots$\\
theorem stromquist\_envyFree\_exists $\ldots$\\
theorem ef\_and\_proportional\_exists $\ldots$
\end{leanbox}
...
noncomputable def vickreyPayoff (v : $\iota \to \mathbb{R}$) (bids : $\iota \to \mathbb{R}$) (who : $\iota$) : $\mathbb{R}$ :=\\
\bd if bids who > maxOtherBid bids who then v who - maxOtherBid bids who else 0\\[3pt]
theorem vickrey\_truthful\_dominant (v : $\iota \to \mathbb{R}$) (who : $\iota$) :\\
...
\TT{Auctions/ReserveVickrey.lean}, \TT{Auctions/VCG.lean}, and
\TT{Auctions/Knapsack/Basic.lean}.  First-price constructs the deterministic
...
and rent dissipation.  Reserve Vickrey proves truthfulness for the
second-price auction with a reserve.  VCG packages the efficient-allocation setup
and proves truthful reporting is dominant
(Clarke 1971, Groves 1973).  Knapsack auctions use the Myerson-payment
layer:
\begin{leanbox}
theorem welfareMaximizingAllocationRule\_isMonotone $\ldots$\\
theorem welfareMaximizingMechanism\_isDSIC $\ldots$\\
theorem dynamicProgrammingOptimalAllocation\_optimal $\ldots$\\
theorem integralGreedy\_halfApprox\_dpOptimal $\ldots$
\end{leanbox}
...
alternative.  \TT{Mechanism/Arrow.lean} provides the
strict-linear-order (\TT{IsRanking}) layer, collective rationality
(\TT{IsSWO}), the domain-restricted Pareto/IIA axioms, and proves
\emph{Arrow's impossibility theorem} via Geanakoplos's pivotal-voter
argument (with an exact biconditional-dictator companion).
\TT{Mechanism/GibbardSatterthwaite.lean} adds social choice
functions, strategy-proofness, and proves the
\emph{Gibbard–Satterthwaite theorem} by the reduction to Arrow
through a monotonicity-based induced social-welfare function.
\TT{Mechanism/SenParetianLiberal.lean} formalizes Sen's liberal
paradox as \TT{sen\_paretian\_liberal}.
...
\bd outcome := id\\
\bd utility := -- (H,H) = (1,-1), (H,T) = (-1,1), $\ldots$
\end{leanbox}
...
at which both players' EU is $0$.  Lean records both facts:\
\TT{matchingPennies\_no\_pure\_nash} lives in
\TT{Languages/NFG/Examples.lean}, while
\TT{matchingPennies\_mixed\_nash\_iff\_half} in
\TT{Languages/NFG/MatchingPenniesMixed.lean} instantiates the generic
\TT{Concepts/Mixed/BinaryMixed.lean} theorem by labeling heads as \TT{true}
and tails as \TT{false}; it states that a mixed profile is Nash iff
each player's probability of heads is $1/2$.
...
Rubinstein's \emph{electronic mail game}~\citep{Rubinstein1989}
(\TT{Languages/ElectronicMailGame.lean}) is a two-player
coordinated-attack game with Boolean players, a finite email chain of
endpoint worlds, and a uniform prior over them.  The tempting rule
\TT{attackOnMessage} --- attack as soon as any message is observed ---
is \emph{not} a Bayes-Nash equilibrium:\
\TT{attackOnMessage\_not\_bayesNash} refutes
\TT{game.BayesNash attackOnMessage} by unfolding
\TT{bayesNash\_iff\_exAnteEU} and exhibiting the profitable deviation
in which player \TT{true} never attacks.
...
worked subclass files inside \TT{Languages/NFG/} (PublicGoods,
Stackelberg) and the \TT{Congestion/} directory are useful as
