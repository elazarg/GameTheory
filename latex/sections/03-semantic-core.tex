...
\Cref{sec:solution-concepts:family} by stating monotonicity
results once on the schema.}

\leancorr{\TT{Core/GameForm.lean} for the record and derived
operations; supplement~§3.1 has the full Lean signatures.}
...
\begin{equation}
\label{eq:kernelgame}
  \KG \;=\; \bigl(\,\Strat,\;\Outcome,\;\K,\;\util : \Outcome \to \Payoff{\Players}\,\bigr),
\end{equation}
...
\end{equation}
a finite sum when \Outcome\ is finite.  At the core API level the
implementation is mathlib's unordered-sum expectation for
$\PMF{\cdot}$, well-defined on bounded utilities over an arbitrary
(countable) \Outcome; the finite-support lemmas recover the
displayed formula, and a parallel family of
\TT{\_of\_bounded} lemmas replays the same algebra without a
\TT{[Finite \Outcome]} hypothesis.  This is why expected utility is
the only API point where the utility field enters a numerical
solution-concept inequality.

The conversion $\toGF : \KG \mapsto \GF$ forgets utility; the
construction $\toKG_u : \GF \mapsto \KG$ adjoins a chosen utility
$u$.  They satisfy the expected one-sided identities:
$\toKG_{\KG.\util}(\KG.\toGF) = \KG$ and
$(\toKG_u(\GF)).\toGF = \GF$, up to record equality.
...
(\Cref{sec:theorems:nash-mixed}), where the existence proof
adds finiteness back as a hypothesis on $\Strat_i$ and a
bounded-utility hypothesis on outcomes.
...
round-trip lemmas); \TT{Math/PMFProduct/Basic.lean} (\TT{pmfPi});
supplement~§3.2--3.4.}
...
\item \emph{Preference-parameterized.} $\IsNashFor$,
  ...
  $\IsCorrelatedEqFor$, $\ldots$:\ predicates on $\GF$
...
transport and implication results — CE implies CCE under the
deviation-family schema, best-response/Nash reformulations, and
dominance-to-best-response lemmas — are \emph{purely structural}:
they hold whenever players have a preorder on outcome
distributions, regardless of whether that preorder comes from
expected utility, lexicographic preferences, or maximin attitudes.
Stating them at the $\GF.\IsNashFor$ layer keeps them generic and
stops them from masquerading as EU-specific theorems.  Existence
theorems such as CE/CCE existence are different: in this library
they use the finite, bounded-utility expected-utility machinery.
Second, the EU-specific layer keeps the common case ergonomic:\
users who care only about expected
...
\item The transport theorems of
  \Cref{sec:semantic-core:transport} relate preferences across
...
\item Strict-from-weak / weak-from-strict bridges of
  \Cref{sec:solution-concepts:dominance} relate a strict preference
  to a weak preference under a hypothesis $\sPref \Rightarrow \Pref$.
\end{itemize}

Each of these ties together two preferences at once.  A typeclass-based
...
level, a \emph{protocol map} from $\GF \to \GF'$ is a pair
\[
  (\stratMap, \outcomeMap)
  \quad\text{with}\quad
...
\]
together with a proof of the \emph{outcome-kernel preservation} law.
Writing
$(\stratMap\,\Profile)_i \coloneqq \stratMap_i(\Profile_i)$ for
componentwise profile translation, the law is
\begin{equation}
...
etc., under hypotheses on $\stratMap$).  When a protocol map also
satisfies the pointwise utility-preservation equation
$\util'(\outcomeMap(\omega))(i)=\util(\omega)(i)$, helper
constructors turn it into the code's distribution-level
\TT{KernelGame.Morphism}.  The \TT{KernelGame.Morphism} record
itself stores only a componentwise strategy map and the equation
that the translated profile has the same joint payoff law
\TT{udist}.  Its isomorphism/bisimulation variants require inverse
strategy translations preserving the same payoff law.  Thus
utility-distribution morphisms transport payoff-law preferences;
protocol maps transport preferences over protocol outcomes.  The
specialized \TT{EUMorphism} and \TT{EUGameIsomorphism} extend the
\TT{Morphism} and \TT{GameIsomorphism} records with an explicit
expected-utility-preservation field:\ payoff-law preservation already
entails EU preservation for bounded or finite-outcome games, so these
variants package that corollary as a ready-made input to the EU-level
transport theorems rather than a weaker mean-only notion.

\leancorr{The \TT{ProtocolMap} record is in \TT{Core/GameForm.lean};
\TT{KernelGame.Morphism} and its
\TT{id\_comp}/\TT{comp\_id}/\TT{comp\_assoc} laws are in \TT{Core/GameMorphism.lean};
...
\TT{Concepts/Foundations/UtilityInvariance.lean} package the standard
solution-concept transport results; \TT{Concepts/Foundations/EUProperties.lean}
and \TT{Concepts/Foundations/OfEUProperties.lean} record EU structural
properties and the \TT{ofEU} round-trip.  Supplement~§3.6.}
...
chapter writes $\KG[e]$ informally for the game reindexed along
$e$.

\subsection{Transport of solution concepts under bridges}
\label{sec:semantic-core:transport}

The use we make of protocol maps and utility-preserving isomorphisms in
\Cref{sec:languages:bridges} relies on transport theorems that
...
\;\Longleftrightarrow\;
\GF'.\IsNashFor\bigl(\Pref',\,\stratMap\,\Profile\bigr).
\]
...
\]
If $R(\Profile,\Profile')$ holds, then for every preference
$\Pref^\Omega$ on $\PMF{\Omega}$,
\[
...
\TT{DeviationFamily} equilibria, over any type $U$ of deviating units:
if every target-family deviation is matched by a source-family
deviation with the same observed law, then $\mathcal{D}$-equilibrium
transfers from the source presentation to the target presentation.  The
Nash theorem is the constant-deviation, point-mass, $U=\Players$
specialization.
\end{theorem}
...
\Cref{sec:introduction}:\ a bridge between two languages, viewed
as a protocol-preserving (resp.\ utility-distribution--preserving) map between the
compiled game forms (resp.\ kernel games), automatically
transports solution concepts.  Concrete bridges
(\Cref{sec:languages:bridges}) instantiate the theorem with
specific maps and inherit a Nash/dominance/correlated-eq
preservation law for free.

\leancorr{\TT{Concepts/Foundations/GameMorphism.lean},
\TT{Concepts/Foundations/UtilityInvariance.lean}, and
\TT{Core/GameSimulation.lean} for the map-based transport layer;
\TT{Concepts/Foundations/Transport/} for the observed-law layer
(\TT{ViewFamily} and \TT{observedPref} in \TT{Transport/View.lean};
\TT{Realization}, \TT{Realization.Simulates}, and \TT{Transport} in
\TT{Transport/Simulation.lean}; the generic transfer
\TT{Transport.target\_eq\_of\_source\_eq} in \TT{Transport/Transfer.lean}),
with the pure-profile Nash corner \TT{NashDeviationSimulation} and the
correlated/coarse-correlated corollaries in
\TT{Concepts/Foundations/DeviationSimulation.lean}.}
...
\leancorr{All three in \TT{Concepts/Equilibrium/GameFormSolutionConcepts.lean}
(\TT{dominant\_is\_nash\_for},
\TT{isNashFor\_iff\_bestResponseFor},
\TT{IsCorrelatedEqFor.}\allowbreak\TT{toCoarseCorrelatedEqFor}).}
