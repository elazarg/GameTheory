...
unordered-sum expectation for $\PMF{\cdot}$; finite-support lemmas
recover the displayed formula.  Expected utility is the only place
...
A kernel game over a finite outcome and finite-strategy domain
admits an explicit \emph{mixed extension}:\ each player's strategy
type is replaced by $\PMF{\Strat_i}$, and the outcome kernel
samples a pure profile from the independent product
$\bigotimes_i \Profile_i \in \PMF{\prod_i \Strat_i}$ and applies
the original kernel.  The mixed extension is itself a kernel game,
indexed by the same $\Players$, and serves as the carrier of mixed
strategies in Nash existence (\S\ref{sec:theorems:nash-mixed}).
...
\item The transport theorem of
  \S\ref{sec:semantic-core:transport} relates a preference on a
  source game form to its pullback along an outcome bijection — two
  preferences in the same statement, one a function of the other.
...
\S\ref{sec:languages:bridges} relies on a small family of
transport theorems we record here for reference.  Each is a
direct calculation against the preservation law
\eqref{eq:protocolmap} and the definition of the relevant
solution concept.
