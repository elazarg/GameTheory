...
\emph{kernel-game simulation} (the code's \TT{KernelGame.Morphism},
which preserves the full joint utility distribution \TT{udist}), and
call the morphism a \emph{utility-preserving isomorphism}
(\TT{KernelGame.GameIsomorphism} / \TT{Bisimulation}) when both
$\stratMap$ and $\outcomeMap$ are bijective and the inverse is also a
simulation.  Preserving the joint payoff law transports \emph{any}
preference over outcome distributions; expected-utility preservation
is the mean-functional corollary, not the definition.  The genuinely
EU-only variants — which preserve each player's mean but not the full
payoff law — are the explicitly specialized \TT{EUMorphism} and
\TT{EUGameIsomorphism}.
