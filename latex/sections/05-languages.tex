...
fragment), and Cournot duopoly with three quantities.  Each
defines an action type with \TT{deriving Fintype, DecidableEq},
a payoff table by pattern match, and one or two pure-Nash facts
discharged by case analysis (\TT{cases i <;> cases a' <;> simp;
try linarith}).  These serve both as reader-facing examples and
as smoke tests for the NFG$\to$\KG\ compilation pipeline.
...
and finite-horizon repeated-game examples.  The infinite discounted
repeated-game folk theorem is not built through this bounded
multi-round syntax; it is formalized directly on \TT{KernelGame}
streams in \TT{Concepts/FolkTheorem.lean}.  The theorem proves
that every strictly individually rational feasible payoff vector
is approximated by Nash payoffs of sufficiently patient discounted
