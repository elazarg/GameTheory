...
The library's NFG layer also formalizes several worked subclasses
as named structures:\ \emph{congestion games} with Rosenthal's
potential~\citep{Rosenthal1973}, \emph{TU coalitional games} with
the Shapley value~\citep{Shapley1953}, \emph{Nash bargaining}
\citep{Nash1950bargaining}, \emph{stable matching} (predicates and
structural lemmas)~\citep{GaleShapley1962}, \emph{public goods},
and \emph{Stackelberg leader-follower}.  Each subclass states the
content of its theorems at the level the library currently
reaches:\ a congestion game is an exact potential game; the
Shapley value comes with null-player, additivity, linearity, and
symmetry lemmas; for stable matching, the predicates
$\mathsf{IsMatching}$, $\mathsf{IsBlockingPair}$, $\mathsf{IsStable}$
and supporting structural lemmas — the Gale–Shapley algorithm and
existence of a stable matching are listed in \S\ref{sec:future} as
natural extensions.  Each subclass is a direct consumer of the
solution-concept catalogue (\S\ref{sec:solution-concepts}).

\leancorr{\TT{Languages/NFG/} contains \TT{Syntax}, \TT{Compile},
plus the worked-subclass files \TT{CongestionGame},
\TT{CoalitionalGame}, \TT{Bargaining}, \TT{Matching},
\TT{PublicGoods}, \TT{Stackelberg}.  Supplement~§4.1.}
...
\TT{EFGGame.IsSubgamePerfectEq}), \TT{Examples}, \TT{Tests}.
Supplement~§4.2.}
...
horizon ensures termination.  Two specializations are formalized
on top of the multi-round skeleton:
\begin{itemize}\itemsep2pt
\item \emph{Repeated games}:\ identical rounds, single payoff
aggregation function (sum, discounted sum).  This is the carrier
for the standard formalizations of trigger strategies, tit-for-tat,
and folk-theorem fragments.  (A \emph{folk theorem} says that any
feasible, individually rational payoff vector can be sustained as
an equilibrium average payoff of a sufficiently patient
infinitely-repeated game; the classical Friedman / Fudenberg–Maskin
versions \citep{Friedman1971, FudenbergMaskin1986} are
infinite-horizon and out of scope here.)
...
\emph{addresses} and information sets directly, and three strategy
types — \emph{mixed} (a distribution over a player's joint pure
strategy space $\prod_{a \in A_p} \Lambda_a$), \emph{product-mixed}
(independent randomization over each address), and
\emph{behavioral} (independent randomization at each information
set) — are defined and related propositionally.  The unconditional
product-mixed/behavioral equivalence is the intrinsic-form analogue
of one direction of Kuhn's theorem (\S\ref{sec:theorems:kuhn}).
...
\leancorr{\TT{Languages/Intrinsic/Strategies.lean} for the three
strategy types and the
\TT{productMixedToBehavioral}\,/\,\TT{behavioral\_realizes\_productMixed}
equivalence; \TT{PerfectRecall.lean} and \TT{Theorems.lean} for
the perfect-recall extension.  Supplement~§4.6.}
