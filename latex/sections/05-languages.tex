...
not packaged.  \Cref{sec:worked-example} revisits these examples in
the worked-example tour.
...
(\Cref{sec:theorems:kuhn}) is proved.
...
involving the Intrinsic layer is currently packaged in
...
EU-isomorphism (\Cref{sec:semantic-core:morphisms}) is the
...
library exposes the following packaged bridges; the Intrinsic layer
currently has a compiler but no packaged bridge row:
...
\noindent
The bridges of \Cref{sec:languages:bridges} are reductions in
...
Intrinsic currently an unbridged syntactic root) is itself a
contribution.}
