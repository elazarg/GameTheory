...
try linarith}). These serve both as reader-facing examples and as
...
and finite-horizon repeated-game examples.  The bounded
finite-horizon side records the forward direction of
\emph{finitely-repeated convergence}:\ if \TT{a} is a stage-game
Nash, the constant repetition profile is a Nash of the repeated
game (\TT{constStrategy\_isNash}) and its average payoff equals
the stage payoff (\TT{avgPayoff\_constStrategy}). The deeper
reverse direction (under a unique stage Nash, every SPE of the
repeated game plays the stage Nash every round) requires SPE on
repeated-game subgames and remains future work.

The infinite discounted repeated-game folk theorem is not built
through this bounded multi-round syntax; it is formalized
directly on \TT{KernelGame} streams in
\TT{Concepts/FolkTheorem.lean}.  The theorem proves that every
strictly individually rational feasible payoff vector is
approximated by Nash payoffs of sufficiently patient discounted
repeated games (\Cref{thm:folk-discounted}).
