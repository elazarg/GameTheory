...
The library exposes six languages — normal-form (NFG),
extensive-form (EFG), multi-agent influence diagrams (MAID),
multi-round games (MR, with stochastic and repeated specializations),
factored-observation stochastic games (FOSG), and intrinsic-form
games — and the relation-parameterized expressiveness framework
that relates them
(\Cref{sec:languages:expressiveness}).

\subsection{Normal-form games}
\label{sec:languages:nfg}

The normal-form layer is the simplest.  In the finite fragment used
by the main theorem statements, a normal-form game over a finite
player type $\Players$ and finite action family $\Act_i$ is the data
\[
...
extension (\Cref{sec:semantic-core:kernelgame}).

The library's NFG layer also formalizes a specialized
strategic-form structure whose form is a refinement of NFG and
whose theorems are about that refinement (congestion games,
formerly presented here, now occupy the dedicated top-level
\TT{Congestion/} directory with Rosenthal's potential and
...
strictly decreases when a dominant strategy is added).

The implementation idiom is uniform across the examples.  Each
...
discharged by small case-analysis scripts.  Matching Pennies
additionally has an exact mixed
characterization in \TT{Languages/NFG/MatchingPenniesMixed.lean};
that file instantiates the language-independent
\TT{Concepts/Mixed/BinaryMixed.lean} theorem for binary
matching-pennies-like \KG{}s, showing that both players put probability
...
The cooperative-flavored game-theoretic models in
\TT{Cooperative/} — TU coalitional games with the Shapley
...
hierarchy.  See \Cref{sec:future:cooperative} for what they
contain.
...
\TT{PublicGoods}, \TT{Stackelberg}; congestion games live in the
top-level \TT{Congestion/} directory.
...
(\Cref{sec:theorems:kuhn}) a purely structural identity
...
bridge (\Cref{sec:languages:bridges}) is most naturally stated.

The EFG \TT{Examples} file carries two canonical sequential games:
the 2-stage \emph{Centipede game}~\citep{Rosenthal1981} (payoffs
(1,0) / (0,2) / (3,1) with backward-induction prediction \TT{(p0 takes)}
leaving the welfare-max (3,1) on the table), and the \emph{Ultimatum
game}~\citep{GuthSchmittbergerSchwarze1982} (a proposer offers one of
three splits to a responder who
accepts/rejects). Each ships with the four \TT{evalPure} outcome
identities plus the textbook backward-induction comparison
inequalities; the full \TT{EFGGame.IsSubgamePerfectEq} witnesses are
not packaged.  The worked-example tour in \Cref{sec:worked-example}
uses Prisoner's Dilemma as its running trace and briefly points back
to these EFG examples.
...
once; this is the MAID-specific instance of the order-independent
evaluation problem for graphical games~\citep{KollerMilch2003}.

\tradeoff{We chose the order-independent frontier evaluator over
fixing a topological order at compilation time.  The price is the
diamond/commutation proof obligation above; the gain is that the
formal object does not depend on an arbitrary evaluation order.}
...
\TT{Kuhn}, \TT{Theorems}.  Supplement~§5.3.}
...
\TT{Concepts/Welfare/FolkTheorem.lean}.  In the main theorem the repeated
...
(\Cref{sec:theorems:kuhn}) is stated against.  This is the layer
...
The same file family contains a compact absent-minded-driver example:
two rounds produce the same player view, so the protocol violates
\TT{ViewDeterminesRound}, the MultiRound no-absent-mindedness
condition.  The example also records the standard payoff calculus:
the ex-ante optimal probability of continuing is $2/3$, while
re-optimizing at the indistinguishable decision point under those
arrival odds chooses $1/3$.
...
\TT{AbsentMindedDriver}, \TT{Kuhn}, \TT{Theorems}.
Supplement~§5.4.}
...
The finite-horizon FOSG kernel game is the canonical native carrier
that instantiates the representation-neutral observation-model Kuhn
theorem of \Cref{sec:theorems:kuhn}.
...
appear in sequential games, but without building a tree:\ (a) draw
one complete contingent plan, (b) randomize independently across a
player's decision addresses, or (c) randomize independently at the
information atoms the player can distinguish.  The address-to-information
comparison, (b) versus (c), is unconditional:\ independent
randomization by address and behavioral randomization by information
atom induce the same local choice laws.  The mixed-plan-to-behavioral
comparison, (a) versus the conditional strategy in (c), is the
intrinsic analogue of Kuhn's theorem.  Given a configuration ordering,
if the player has perfect recall along that order, then the conditional
behavioral strategy extracted from a mixed plan preserves the full
outcome law against every opponent profile.  The formal theorem takes
solvability directly; with that outcome map supplied, it does not require
a separate causality hypothesis.
The conclusion is equality of distributions over realized
decision profiles, not just equality of one-address marginals.

The intrinsic layer is a separate syntactic root from
EFG/MAID/MR/FOSG, used as a target by representations that prefer
order-independent reasoning.  It compiles pure strategy profiles to \KG\
when supplied with solvability and expected-utility data.  Mixed and
behavioral profiles remain in the layer's outcome-law semantics rather
than being transported by that constructor.  No cross-language bridge
involving the Intrinsic layer is part of
\TT{Languages/Bridges/}.

\leancorr{\TT{Languages/Intrinsic/} contains the strategy
carriers, causal-order and perfect-recall predicates, and the
formal outcome-law equivalence for solvable models.  The
definition and theorem anchors are listed in Supplement~§5.6.}
...
protocol equivalence preserves the game form; utility-distribution
equivalence or simulation preserves the joint payoff law; and
expected-utility simulation preserves only the mean utilities needed
for EU solution concepts.  The library exposes the following packaged
bridges; the Intrinsic layer has a compiler but no bridge row:

\begin{center}\small
\begin{tabular}{@{}llp{0.42\textwidth}@{}}
\toprule
Source & Target & Packaged relation \\
\midrule
NFG  & EFG  & kernel-game morphism from the NFG game to its depth-1 EFG image \\
NFG  & FOSG & kernel-game morphism from the NFG game to its one-shot FOSG image \\
EFG  & NFG  & kernel-game morphism for the deterministic strategic-form image \\
MAID & EFG  & utility-distribution equivalence for the generated EFG \\
EFG  & FOSG & profile-map utility-distribution simulation for serial execution \\
\bottomrule
\end{tabular}
\end{center}
...
(\TT{ObsModelCore}, \TT{AugmentedEFG}, \TT{SerialExec}); for example,
\TT{MAID\_EFG} packages the MAID$\to$EFG bridge as a reduction.
Supplement~§6.2.}
...
\TT{EquivalenceLens}.  ``Lens'' is the project's bundling vocabulary
and has nothing to do with optics or category-theoretic lenses.}
...
\noindent
Some bridges of \Cref{sec:languages:bridges} have also been
registered as reductions in this framework; others remain
bridge lemmas or \TT{KernelGame.Morphism} packages.  Context-aware
...
Concrete worked expressiveness theorems include the
utility-distribution equivalence for generated MAID/causal-context
compilations and the EFG-to-FOSG profile-map utility-distribution
simulation.  The finite NFG-to-FOSG theorem is exposed as
\TT{NFGGame.toFOSG\_morphism}; it is a bridge target for the
expressiveness layer rather than a registered \TT{Expressiveness}
reduction.
...
The resulting lattice of bridges (NFG, EFG, MAID, MR, FOSG, with
Intrinsic as an unbridged syntactic root) is itself a
contribution.}
