...
Giry monad — with \TT{pure} (the Dirac embedding
$\Dirac_x \in \PMF{X}$), \TT{bind}, and \TT{map}.  Expected value of
a function $f : X \to \RR$ under $\mu \in \PMF{X}$ is the unordered
sum $\sum_x \mu(x)\,f(x)$, formalized as
\TT{Math.Probability.expect}.  When $X$ is finite, this is a
finite sum.  On infinite discrete carriers the API uses a tsum;
algebraic and continuity lemmas assume the needed
boundedness/summability hypotheses, usually discharged in the
library by bounded utilities.
...
for some $n$.  Throughout the chapter, a \emph{finite game} means a
kernel game $\KG$ with $\Players$, each $\Strat_i$, and $\Outcome$
finite; the language layer adds further structural finiteness — a
finite extensive-form tree, finitely many information sets, a
bounded horizon — and each theorem states explicitly which subset
of these assumptions it uses.  The \emph{definitions} of the core objects —
...
pure strategies.  A parallel family of bounded-utility
\TT{\_of\_bounded} lemmas in \TT{Math.Probability} and
\TT{Concepts/Mixed/MixedExtension} removes finite outcome carriers while
retaining the finite strategy/tree hypotheses used by the proof, and
replays the algebra of expected utility under a $|u| \le C$ hypothesis
without \TT{[Finite \Outcome]}.  The dependent product
$\prod_i \Strat_i$ is defined for any index type, but finite
hypotheses on $\Players$ are nearly always present because
\TT{pmfPi}, finite products of simplices, and the main enumeration
arguments need a finite player index.  Some declarations also require
\TT{DecidableEq}, e.g.\ everything that uses
\TT{Function.update} for unilateral deviation.

These decidability requirements are discharged by
\TT{open Classical in}, which turns on the law of excluded middle for
the duration of a single declaration.  This is a benign use of
classical reasoning when it is invoked only to make
\TT{Function.update} compute on abstract index types.  On concrete
finite types like \TT{Fin n} the decision is constructive, so the
classical instance is definitionally redundant.  We collect these uses in
\Cref{sec:discussion:noncomputable} and explain why they do
...
\TT{utility} field of $\KG$ is $\util$'') reduces by computation; the
corresponding equality goals are discharged by \TT{rfl} and are freely
used by \TT{simp}/\TT{unfold}.
...
machinery), product topology, the discrete Giry monad \TT{PMF}, and a
substantial amount of $\sum$-and-tsum API\@.  Three non-trivial dependencies
deserve explicit mention:

\begin{itemize}
\item \emph{Brouwer's fixed-point theorem.}  Mixed Nash existence
uses Brouwer on the product of standard simplices; the proof shape
and nonconstructive tradeoff are discussed in
\Cref{sec:theorems:nash-mixed}.  For dependencies, the important
point is that the theorem itself comes from the standalone
\TT{fixed-point-theorems-lean4} package~\citep{harfeFixedPointTheorems};
...
A few small companion files add the discrete-probability
constructions mathlib does not already contain:\ \TT{Math.PMFProduct}
provides \TT{pmfPi}, the independent product of an indexed family of
$\PMF{\cdot}$'s over a finite index but otherwise arbitrary
per-coordinate types, used in the mixed extension of a kernel game;
and \TT{Math.Probability} provides the bounded-utility tsum
