...
The signal-structure layer is intentionally a substrate, not yet the
...
Clarke externality payments.  Scaling agent $i$'s utility by $w_i$
recovers the entire affine objective evaluated at the chosen
alternative — which the choice rule maximizes — so no misreport can
...
(\TT{affineMaximizer}, \TT{affineMaximizer\_isDSIC}, \TT{vcg},
\TT{vcg\_isDSIC}).}
...
(\TT{principalUtility\_le\_firstBest}), with the ``sell the firm'' case
($\alpha = 1$, the agent becomes residual claimant) attaining it.
