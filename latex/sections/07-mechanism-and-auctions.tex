...
payoff $u : \prod_i \Theta_i \times \prod_i \Act_i \to \Players \to
\RR$ depending on both the realized type vector and the chosen
joint action.  Behavioral strategies replace the pure version by
$\Theta_i \to \PMF{\Act_i}$.

The Bayesian game compiles to a kernel game via the \TT{ofEU}
adapter (\S\ref{sec:semantic-core:kernelgame}):\ the strategy type
is $\Theta_i \to \Act_i$, and the outcome kernel is the
deterministic point mass at the \emph{ex-ante} expected utility
\[
...
\S\ref{sec:prelim:pmf}, this falls outside the discrete-PMF
...
$M = (\Theta, \Act, g : \prod_i \Act_i \to \Players \to \RR)$;
a strategy profile is a tuple $\sigma_i : \Theta_i \to \Act_i$.
A \emph{direct mechanism} is a mechanism with $\Act_i = \Theta_i$;
the canonical strategy is the identity.  The current Lean definition
...
these are flagged in \S\ref{sec:future} as the natural next step.
...
The auction directory currently contains four formats at different
...
bidder pays their own bid regardless of winning.  The current
...
currently a collection of real-valued payoff lemmas.  Solution
concepts come from \S\ref{sec:solution-concepts} wherever a full
...
faithful to the strategic content it currently formalizes
