...
\[
  \IsStrongNash(\Profile) \;\Longleftrightarrow\;
    \neg \exists C, \tau.\;
    (\forall i \in C, \EU_C \ge \EU_\Profile) \land
    (\exists i \in C, \EU_C > \EU_\Profile)
\]
where $\EU_C$ is the EU under $\mathrm{coalitionDeviation}(C,
\Profile, \tau)$.  Specializing $C$ to a singleton recovers Nash,
and specializing $C$ to the grand coalition $\Players$ recovers
Pareto efficiency (\S\ref{sec:solution-concepts:welfare}):\ strong
...
The theorem is deliberately one-way.  It does not say that cheap talk
preserves all equilibria: observable messages may introduce additional
coordination equilibria.  Instead, it assumes only the local condition
needed for babbling: around an embedded base profile, every unilateral
enriched deviation projects to a base unilateral deviation with the
same preference comparison.  Therefore every base Nash equilibrium
embeds as a babbling Nash equilibrium of the cheap-talk game.
...
(\S\ref{sec:solution-concepts:team}), the welfare-maximizing
...
A separate, decision-theoretic regret notion (Savage 1951) lives
in the \TT{Savage} namespace of the same file. Given a finite
action set $A$ and a finite state space $\Omega$ with utility
$u : A \times \Omega \to \RR$, the *regret* of action $a$ at state
$\omega$ is $\mathrm{regret}(a, \omega) = \sup_b u(b, \omega) -
u(a, \omega)$; the *maximum regret* is the worst case over states;
a *minimax-regret action* minimizes the maximum regret. The
...
use.  The first is partition-based knowledge.  Each agent has an
\emph{information partition} $P$ of a finite state space, with
$P.\mathrm{cell}(s)$ the set of states $s$ considers possible at
$s$ (reflexive at $s$; partition-coherent).  The \emph{knowledge
operator}
\[
...
infrastructure is Aumann's (1976) full agreement: when two agents
...
\emph{Individual rationality}, the second concept, is a
participation constraint:\ a profile is individually rational for
player $i$ when $i$'s payoff is at least their security level
(\S\ref{sec:solution-concepts:security}).  Both are formal
scaffolding for the mechanism-design layer
