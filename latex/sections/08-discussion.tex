...
paper; the goal here is to make the global picture explicit so
...
used to evaluate it~\citep{Myerson1991}.  For preference-agnostic
theorems, keeping them below \TT{KernelGame} is the least repetitive
design.
...
backward-induction inequalities rather than full
...
\item \emph{Cooperative game theory beyond the core apparatus.}
Beyond the Shapley value and its uniqueness (\Cref{thm:shapley-unique}),
the Banzhaf and Shapley–Shubik indices, convex games, the core, and
the easy direction of Bondareva–Shapley, the classical landmarks are
not formalized:\ the \emph{converse} Bondareva–Shapley direction
(balancedness $\Rightarrow$ nonempty core, the LP-duality content),
the nucleolus~\citep{Schmeidler1969}, the kernel and bargaining set,
NTU games, and the Aumann–Shapley value for non-atomic
games~\citep{AumannShapley1974}.
