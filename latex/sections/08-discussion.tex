...
theorems (Nash, correlated equilibrium, Zermelo, Kuhn), where
the proof needs Brouwer, \TT{Finite.exists\_max}, or
...
Mixed-Nash and correlated-equilibrium existence use Brouwer on a
finite-dimensional product of simplices and finite-dimensional
continuity of expected utility.  Security levels and minimax values
use attained finite maxima/minima.  Backward induction and the
one-shot-deviation principle use finite trees and finite outcome
comparisons.  The Kuhn wrappers enumerate pure strategies, histories,
or reachable information/action carriers.  Finally, the theorem that a
mixed Nash distribution induces CE/CCE has bounded-utility variants
without finite outcomes, but its current product-deviation step still
uses a finite-coordinate \TT{pmfPi} update lemma; removing that last
finite strategy assumption would require a countable product-update
theorem for \TT{pmfPi}.
Some EU-specialized morphism wrappers also still use finite outcomes
to convert equality of utility distributions into equality of expected
utilities; removing that wrapper assumption requires an
\TT{expect\_pushforward} lemma whose boundedness hypothesis is only on
the image of the pushed-forward utility map.
