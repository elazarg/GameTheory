...
\paragraph{Choice.}\ Every distribution is a \TT{PMF}; no
$\sigma$-algebra is ever invoked.  Strategy and outcome carriers
may be countably infinite at the definitional level — the EU
operator is a tsum, the mixed extension and \TT{pmfPi} are
defined over a finite player index but arbitrary per-coordinate
types, and a parallel family of \TT{\_of\_bounded} lemmas
replays the algebra without \TT{[Finite \Outcome]}.  The
\TT{Fintype} hypotheses re-enter at the level of major existence
theorems, where the proof needs Brouwer, \TT{Finite.exists\_max},
or pure-strategy enumeration.  For Nash, CE/CCE existence, Zermelo,
OSD, minimax, and morphism EU wrappers, bounded-utility variants
remove finite outcome carriers while preserving the proof-relevant
finite strategy/tree hypotheses.

\paragraph{Remaining finite boundaries.}\ The finiteness assumptions
that remain are proof boundaries rather than definitional restrictions.
Mixed-Nash and correlated-equilibrium existence still use Brouwer on a
finite-dimensional product of simplices; dropping finite strategy
sets requires a topologized mixed-strategy space, compactness of
that space, and continuity of expected utility.  The library's
\TT{Math/SchauderFixedPoint.lean} supplies a suitable
fixed-point principle for compact convex subsets of real normed
spaces, but the game-theoretic compactness and continuity layer is
separate work.
Security levels have sup/inf definitions, but existence of an
attaining security strategy still needs finite or compact attainment.
Backward induction and the one-shot-deviation principle use finite
trees and finite action sets, but not finite outcomes in their
bounded-utility forms.  The Kuhn wrappers still enumerate pure
strategies, histories, or reachable information/action carriers.
...
\Cref{sec:prelim:pmf}; we do not repeat them here.  What this section adds
...
continuous version.  An honest reading of \Cref{sec:prelim:pmf}
...
predicates (\TT{*For}) on \TT{GameForm}.  The detailed API shape is
given in \Cref{sec:semantic-core:for-style}; here we record the
design tradeoff.

\paragraph{Gained.}\ Strategic results that do not depend on
cardinal utility — Kuhn-style structure, transport, and the
deviation hierarchy — can be stated once at the \TT{GameForm} layer
and reused for the EU preorder, ordinal preferences, maximin
attitudes, or other preference preorders.
...
explicitly.  The corresponding bridge lemmas and \TT{simp} rules
are extra library surface, but the EU-only interface stays short.

\paragraph{Why.}\ The split is the mechanized version of the
mechanism-design separation between a protocol and the preferences
used to evaluate it~\citep{Myerson1991}.  Once enough theorems were
preference-agnostic, keeping them below \TT{KernelGame} became the
least repetitive design.
...
transport theorems of \Cref{sec:semantic-core:transport}, which
...
representation (NFG, EFG, MAID, MultiRound, FOSG, Intrinsic);
concrete representations bridge to the central definitions via
$\toKG$.
...
into the definition.  The EFG examples instantiate evaluation and
backward-induction inequalities, but do not yet package full
\TT{EFG.IsSubgamePerfectEq} witnesses.
...
classical decidability used by \TT{Function.update} on abstract
index types, or fixed-point theorems.  We do not attempt to expose
computable witnesses for, e.g., \TT{mixed\_nash\_exists}.
...
(\Cref{sec:semantic-core:reindex}) makes the transport a
...
\item \emph{Language-level infinite-horizon stochastic games.}
The library proves an observable mixed-action approximate
discounted folk theorem directly on public \TT{KernelGame} profile
streams (\Cref{thm:folk-discounted}), but the concrete repeated-
and stochastic-game language layers remain bounded-horizon.  We
have not formalized infinite-horizon stochastic games with state
recursion, sampled action histories, or measure-theoretic
strategy/outcome spaces.

\item \emph{Continuous strategy / type / outcome spaces.}  See
\Cref{sec:discussion:finite}.
...
\item \emph{Cooperative game theory beyond the basic apparatus.}
The cooperative branch (\TT{Cooperative/}, parallel to the
non-cooperative core) defines the Shapley value with the four classical
axioms, with linearity represented in Lean by additivity plus scalar
homogeneity, the unanimity-basis decomposition, and the full Shapley
uniqueness theorem (\Cref{thm:shapley-unique}), and it also
includes the Banzhaf and Shapley–Shubik power indices, simple
games, convex (supermodular) games with the
$\mathrm{IsConvex} \Leftrightarrow$ monotone-marginals
characterization, the core (efficient $\land$ coalition-rational)
with convex-set closure, and additive games.  The remaining
classical cooperative landmarks are not formalized:\ the nucleolus
\citep{Schmeidler1969}, the Bondareva–Shapley balanced-collections
theorem~\citep{Bondareva1963, Shapley1967} characterizing
core-nonempty games, the kernel and bargaining set, NTU games, and
the Aumann–Shapley value for non-atomic games
\citep{AumannShapley1974}.

\item \emph{Impossibility theorems in social choice.}  Arrow's
impossibility theorem is formalized (\Cref{sec:mech:social}); the
strategic analogue Gibbard–Satterthwaite and Sen's liberal paradox
are formalizable in the same framework but are not formalized here.

\item \emph{Bayesian persuasion and information design.}
The finite signal-structure and persuasion predicates are present
(\Cref{sec:mech:info-design}), but the main economic theorems of
Kamenica–Gentzkow~\citep{KamenicaGentzkow2011} and successors
are not formalized.
