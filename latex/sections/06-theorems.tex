...
The topological route keeps the game-theoretic proof small: the
standalone \TT{fixed-point-theorems-lean4}
package~\citep{harfeFixedPointTheorems} proves Brouwer for
nonempty compact convex subsets of finite-dimensional normed
spaces via a cubical Sperner's-lemma route (Kuhn 1960), which has
the right shape for arbitrary $n$ players and lets the library
focus on the strategic-form content of the proof.  Algorithm-level
Nash existence is independent enough to live in a downstream
module.}
...
non-empty strategy spaces and bounded utilities.  There exist a
real value $v$ and a mixed Nash profile $\sigma$ of
...
\S\ref{sec:languages:multiround}.  It uses a public profile-history
...
is well-defined under finiteness, every team-game welfare
maximizer is Nash, the price of stability in a team game is $1$,
and the welfare ↔ Pareto bridge — Pareto dominance strictly
raises social welfare, so every welfare-maximizing profile is
Pareto-efficient.  In finite games, both a welfare-maximizing
and a Pareto-efficient profile exist
(\TT{exists\_welfareMax}, \TT{exists\_paretoEfficient}).  We do
not formalize the deeper welfare theorems of mechanism design
here — they live in \S\ref{sec:mechanism-auctions}.
...
the library (see \S\ref{sec:future:cooperative} for the framing).
...
combinatorics that powers \S\ref{sec:theorems:nash-mixed}
