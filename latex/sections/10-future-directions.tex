...
character.  We see it less as a single project and more as a
parallel library with shared infrastructure (the
...
The Shapley value definition and its basic null-player,
additivity, linearity, and symmetry lemmas
(\TT{NFG/CoalitionalGame.lean}) are in the library.  Natural
extensions:\ the full uniqueness axiomatization, the core, balanced
collections~\citep{Bondareva1963, Shapley1967}, the nucleolus
\citep{Schmeidler1969}, the Aumann–Shapley value for
non-atomic games~\citep{AumannShapley1974}, and bargaining
solutions beyond Nash's~\citep{KalaiSmorodinsky1975, Kalai1977}.
The TU coalitional structure is already there; only the
solution-concept layer needs to be filled in.
...
The library's NFG layer formalizes the matching predicates
(\TT{IsMatching}, \TT{IsBlockingPair}, \TT{IsStable}) and a few
structural lemmas, but not the Gale–Shapley deferred-acceptance
algorithm itself nor the existence of a stable matching from an
arbitrary preference profile~\citep{GaleShapley1962}.  The
algorithm is constructive, terminates in $O(n^2)$ steps, and is a
natural target for a Lean implementation paired with a
correctness proof.  Bertot's Coq formalization
\citep{Bertot2016} is a useful reference point.
