...
deferred-acceptance algorithm (\Cref{thm:gale-shapley}), proposer
optimality and receiver pessimality, the rural-hospitals invariants,
and the lattice structure of stable matchings
(\Cref{thm:stable-matching-lattice}).
...
rural-hospitals matched-agent invariants, and the lattice structure of
stable matchings ordered by optional partners
(\TT{Cooperative/GaleShapley/OptionalLattice.lean}).  The natural
extensions are now the strategy-proofness theorem for the proposing
side, many-to-one hospitals/residents matching, ties and weak
preferences, rotations, and a computable extraction of the $O(n^2)$
algorithm paired with the existing correctness proof.  Bertot's Coq
