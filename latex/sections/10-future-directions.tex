...
the agreement-with-Nash-on-symmetric-problems sanity check, and the
\emph{Kalai–Smorodinsky} solution~\citep{KalaiSmorodinsky1975} through
its proportionality characterization (\TT{IsKalaiSmorodinsky}) with IR,
Pareto, symmetry, affine invariance, and the ideal-point bound.
...
existence of a stable matching now proved via the Gale–Shapley
...
game has a nonempty core and that the Shapley value lies in it; and
further bargaining solutions.
...
(\Cref{sec:mech:info-design}) now adds the Bayes-plausibility
...
explicit finite signaling-game refinements, the converse (sufficiency)
direction of multi-receiver feasibility, and bridges from persuasive
decision rules to direct mechanisms.  Continuous type spaces and compact belief simplices
...
profile~\citep{GaleShapley1962} are now formalized
(\Cref{thm:gale-shapley}, \TT{Cooperative/GaleShapley.lean}).  The
natural extensions are the finer structural results:\ proposing-side
optimality (the proposer-optimal stable matching), the lattice
