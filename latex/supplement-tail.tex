...
\Mathematical
A normal-form game over a finite player set $\Players$ and finite
...
The main chapter's transport theorem
(\TT{Core/UtilityInvariance.lean})
states that a protocol map with bijective \TT{stratMap} and
bijective \TT{outcomeMap} transports the
...
(utility-preserving) yields the EU-specific transport laws.  These
are the operational content of the bridges:\ a Nash equilibrium of
a compiled NFG remains a Nash equilibrium of its FOSG image.

% =====================================================================
\section{Major theorems}
\label{sec:supp:theorems}

\subsection{Mixed Nash existence: \texttt{KernelGame.mixed\_nash\_exists}}
\label{sec:supp:nash-exists}\label{def:mixed_nash_exists}

\Anchor{mixed_nash_exists}
\begin{leanbox}
theorem KernelGame.mixed\_nash\_exists (G : KernelGame $\iota$)\\
\bd [Fintype $\iota$] [DecidableEq $\iota$]\\
\bd [$\forall$ i, Fintype (G.Strategy i)] [$\forall$ i, Nonempty (G.Strategy i)]\\
\bd [Fintype G.Outcome] :\\
\bd $\exists$ $\sigma$ : $\forall$ i, PMF (G.Strategy i),\ G.mixedExtension.IsNash $\sigma$
\end{leanbox}

\Mathematical
Every finite game has a mixed Nash equilibrium.  Hypotheses:\
finite player type, decidable equality on players, finite
non-empty strategy space per player, finite outcome.
...
\TT{mixed_nash_exists} is \TT{noncomputable} because Brouwer is.
...
for the alternative no-regret-learning route, not formalized.

\subsection{Minimax theorem: \texttt{KernelGame.von\_neumann\_minimax}}
\label{sec:supp:minimax}\label{def:von_neumann_minimax}

\Anchor{von_neumann_minimax}
\begin{leanbox}
theorem KernelGame.von\_neumann\_minimax (G : KernelGame (Fin 2))\\
\bd [Fintype G.Outcome] [$\forall$ i, Fintype (G.Strategy i)] [$\forall$ i, Nonempty (G.Strategy i)]\\
\bd (hzs : G.IsZeroSum) :\\
...
\Mathematical
Every finite two-player zero-sum game has a mixed Nash profile
whose player-0 payoff is a saddle value:\ player 1 cannot lower it
by deviating and player 0 cannot raise it by deviating.  A separate
lemma records that all mixed Nash equilibria yield the same
...
\paragraph{Proof structure.}\
Existence of a mixed Nash profile is \TT{mixed_nash_exists}.
Equality of EU at any two Nash profiles is the saddle-point
property of zero-sum Nash, recorded as
\TT{IsZeroSum.nash_eu_eq} in \TT{Theorems/Minimax.lean}.
...
theorem oneShotDeviation\_iff\_spe (G : EFGGame) (hpi : IsPerfectInfo G.tree) :\\
\bd $\forall$ $\sigma$, HasNoOneShotDeviation G $\sigma$ $\leftrightarrow$ G.IsSubgamePerfectEq $\sigma$\\[3pt]
theorem zermelo (G : EFGGame) (hpi : IsPerfectInfo G.tree) :\\
...
at which both players' EU is $0$.  The current Lean example records
