...
| Zero-sum and minimax | supported | matrix security, selected values, attained maximin--minimax equality, canonical external-regret cancellation to empirical-marginal approximate Nash, and a same-trace two-player Protocol learner with moving laws and exact nonzero controls | General finite Protocol schedule synthesis, security extensions, and measurable games remain separate. |
...
| FOSG and bridges | supported | EFG structural projection, explicit-order serialization, whole-round boundary/terminal/continuation support, policy inversion, external history utility, behavioral Nash equivalence in both player orders, canonical focal/counterfactual reach, local regret matching with finite/asymptotic bounds, generic no-revisit realization, bounded common-depth root bridge, exact topological decomposition, deviation-uniform root convergence from payoff-range certificates, canonical fixed-strategy/time-average external regret, and a moving same-trace two-player zero-sum learner reaching empirical Nash | General multi-site schedule synthesis, arbitrary behavioral replacements, and unequal-depth information fibers remain separate gates. |
