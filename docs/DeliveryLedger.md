...
| FOSG and bridges | supported | EFG structural projection, explicit-order serialization, whole-round boundary/terminal/continuation support, policy inversion, external history utility, behavioral Nash equivalence in both player orders, canonical focal/counterfactual reach, whole-policy/pure-action regret decomposition, local regret matching with finite/asymptotic bounds, generic no-revisit/all-nonterminal realization, and an exact hostile two-site identity that proves alternative reach is necessary | Generic bounded-evaluator single-site root bridge and topological decomposition, then root-regret or two-player zero-sum exploitability convergence. |
