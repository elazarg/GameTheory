...
| Zero-sum and minimax | supported | matrix security, selected values, attained maximin--minimax equality, and canonical external-regret cancellation to empirical-marginal approximate Nash with correlated and nonzero controls | Instantiate both players' regret bounds from one dynamic learning trace; general security and measurable extensions remain separate. |
...
| FOSG and bridges | supported | EFG structural projection, explicit-order serialization, whole-round boundary/terminal/continuation support, policy inversion, external history utility, behavioral Nash equivalence in both player orders, canonical focal/counterfactual reach, local regret matching with finite/asymptotic bounds, generic no-revisit realization, bounded common-depth root bridge, exact topological decomposition, deviation-uniform root convergence from payoff-range certificates, and canonical fixed-strategy/time-average external regret with positive and negative controls | Instantiate both players' D50 bounds on one zero-sum Protocol trace and consume the reusable D51 saddle-gap/Nash bridge; general schedule synthesis, arbitrary behavioral replacements, and unequal-depth information fibers remain separate gates. |
