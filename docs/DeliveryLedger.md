...
| FOSG and bridges | supported | EFG structural projection, explicit-order serialization, whole-round boundary/terminal/continuation support, policy inversion, external history utility, behavioral Nash equivalence in both player orders, canonical focal/counterfactual reach, whole-policy/pure-action regret decomposition, and local regret matching with finite/asymptotic bounds | Across-information-set perfect-recall decomposition, then root-regret or two-player zero-sum exploitability convergence. |
