...
| FOSG and bridges | supported | EFG structural projection, explicit-order serialization, whole-round boundary/terminal/continuation support, policy inversion, external history utility, behavioral Nash equivalence in both player orders, canonical focal/counterfactual reach, local regret matching with finite/asymptotic bounds, generic no-revisit realization, bounded common-depth root bridge, and an exact topological two-site decomposition with a live baseline-reach failure | Combine every local cumulative bound into root regret, then specialize to two-player zero-sum exploitability convergence; unequal-depth information fibers remain a separate evaluator gate. |
