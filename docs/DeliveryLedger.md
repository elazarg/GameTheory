...
| MAID | supported | typed acyclic execution; multi-owner Nash transfer; source-owner observation pruning with exact native/compiled laws; profile-local full-deviation coverage exactly characterizing safe Nash expansion; fair-signal safe/live controls; experimentally validated finite utility augmentation and one-site graphical-ignorability-to-coverage with a safe/live relay control | Prove the global safe-pruning fixpoint under sufficient recall; develop strategic reliance separately; then refinement and Kuhn-facing extensions. |
