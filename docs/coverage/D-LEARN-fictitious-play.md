...
Status: partial; topology-free trajectory package complete, two analytic theorems deferred
...
limit theorems remain named obligations of the opt-in Analysis consumer; they
are not weakened into finite combinatorics or duplicated over another law type.
...
| same | `frequently_play_eq_of_belief_converges` | theorem | deferred | D-LEARN analytic convergence gate in `GameTheory.Analysis.Learning` | dependency classification | Requires `Filter.atTop` and pointwise convergence; it does not belong in Core. |
| same | `isFictitiousPlay_limit_isNash` | theorem | deferred | D-LEARN analytic convergence gate in `GameTheory.Analysis.Learning` | dependency classification | The target will be the sole canonical mixed `IsNash`; no alternate convergence or equilibrium surface is licensed. |
...
```text
lake build GameTheory.Core.FictitiousPlay GameTheory.Tests.FictitiousPlay GameTheory.Core
...
best-response predicate can pass merely because the fixture is constant.
