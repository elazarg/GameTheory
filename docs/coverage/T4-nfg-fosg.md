...
|---|---|---|---|---|---|---|
| `Languages/NFG/Syntax.lean` | `NFGGame` | structure | adapt | EXP-042 NFG syntax | source comparison; D1/D4 | Keep action syntax and deterministic outcome. Compile to the canonical `GameForm`; keep utility separate and define no NFG-specific Nash predicate. |
...
| same | `toKernelGame_outcomeKernel` | theorem | adapt | named NFG play theorem | EXP-042 | This is the direct source-side equation used by T4. |
...
| `Languages/Bridges/NFG_FOSG.lean` | `actionsOfJoint` | definition | adapt privately if needed | EXP-042 action decoder | hostile slice | It may consume only a certified legal simultaneous joint action; no default action or padding value. |
| same | `toFOSG` | definition | adapt | named one-shot compiler | EXP-042 | Target state must retain the source profile or outcome; observations and menus compile through Protocol rather than redefining execution. |
| same | terminal/active/transition/bounded-horizon lemmas | theorem family | adapt minimally | named compiler facts | EXP-042 | Preserve only facts needed to establish the exact one-step law and public FOSG gate. |
...
| same | `liftBehavioral`, `jointActionOf`, `liftProfile` | definitions | adapt | source-profile-to-policy lift | EXP-042 | The lift must preserve source players and action types; all players act at the same target history. |
| same | `toFOSG_legalActionLaw_nil`, transition-support, `oneStepHistory`, `toFOSG_runDist_one` | laws/helpers | adapt minimally | one target runner lemma | EXP-042 | Use the actual Protocol/Information runner. |
...
| same | `toFOSG_udist_eq` | theorem | adapt | exact NFG/FOSG commuting theorem | D0/T4/EXP-042 | Required frozen semantic result. Equality is stated on canonical finite outcome laws. |
| same | `toFOSG_morphism` | certificate wrapper | retire unless measurement reverses this | direct theorem | D7 | D7 rejected a generic certificate hierarchy. T4 credits the named theorem, not a wrapper around it. |
