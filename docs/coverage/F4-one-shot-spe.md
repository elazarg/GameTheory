...
Canonical destinations: `GameTheory.Protocol.SubgamePerfect`;
`GameTheory.Languages.EFG.SubgamePerfect`
...
|---|---|---|---|---|---|---|
| `Languages/EFG/Refinements.lean` | `EFGGame.IsSubgamePerfectEq` | definition | adapt | `InformationModel.IsSubgamePerfect`; transparent `EFG.Game.IsSubgamePerfect` specialization | EXP-036; full build | Quantifies over every complete history and every whole information-local replacement policy; no subtree evaluator. |
| `Languages/EFG/OneShotDeviation.lean` | `HasNoOneShotDeviation` | definition | adapt | `InformationModel.HasNoProfitableOneShotDeviation`; transparent EFG specialization | EXP-036; off-path and two-decision EFG probes | The successor deviation is a typed legal choice followed by the original profile. |
...
| same | `oneShotDeviation_iff_spe_of_bounded` | theorem | adapt | `InformationModel.isSubgamePerfect_iff_hasNoProfitableOneShotDeviation`; EFG-facing theorem of the same name | EXP-036; phase 2/3 audits | The successor theorem is well-founded, information-local, and quantified over off-path histories. |
| same | `oneShotDeviation_iff_spe` | theorem | subsumed | Protocol theorem plus `EFG.Game.isSubgamePerfect_iff_hasNoProfitableOneShotDeviation` | EXP-036; full build and axiom audit | The finite-outcome convenience theorem is strictly weaker than the stable successor. |
...
lake build GameTheory.Languages.EFG.SubgamePerfect
lake build GameTheory.Tests.EFGSubgamePerfect
lake build
...
The EFG-facing integration test separately packages the hostile two-decision
perfect-recall protocol as a tree-shaped EFG, supplies a well-founded rank, and
checks that the transparent EFG predicates reduce through the named
one-shot/SPE equivalence. The wrapper adds no evaluator, subtree semantics,
solution predicate, transport token, or Analysis dependency.
