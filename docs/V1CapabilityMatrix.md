...
| Prove fictitious-play convergence and approachability/regret consequences | fictitious-play and approachability files in `Concepts/Learning/**` (D-LEARN) | [`GameTheory/Core/FictitiousPlay.lean`](../GameTheory/Core/FictitiousPlay.lean) and [`GameTheory/Core/FictitiousPlayPotential.lean`](../GameTheory/Core/FictitiousPlayPotential.lean) expose canonical gains, nonzero recurrences, and uniform one-step estimates; [`GameTheory/Analysis/Learning.lean`](../GameTheory/Analysis/Learning.lean) proves every pointwise belief limit is mixed Nash | critical gap | Complete the finite player sweep and summability chain, then recover the approachability bridge without duplicating regret semantics. |
