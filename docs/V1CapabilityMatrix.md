...
| Prove fictitious-play convergence and approachability/regret consequences | fictitious-play and approachability files in `Concepts/Learning/**` (D-LEARN) | [`GameTheory/Core/FictitiousPlay.lean`](../GameTheory/Core/FictitiousPlay.lean) now exposes canonical empirical beliefs, the successor law, and the mixed-best-response recurrence; no convergence or approachability theorem yet | critical gap | Recover the two base limit obligations in `Analysis.Learning`, prove potential-game convergence, then recover the approachability bridge without duplicating regret semantics. |
