...
| Prove fictitious-play convergence and approachability/regret consequences | fictitious-play and approachability files in `Concepts/Learning/**` (D-LEARN) | [`GameTheory/Core/FictitiousPlay.lean`](../GameTheory/Core/FictitiousPlay.lean) exposes canonical empirical beliefs and recurrence; [`GameTheory/Analysis/Learning.lean`](../GameTheory/Analysis/Learning.lean) proves every pointwise belief limit is mixed Nash | critical gap | Prove finite exact-potential fictitious-play convergence, then recover the approachability bridge without duplicating regret semantics. |
