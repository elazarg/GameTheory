...
The exact finite-forward theorem in this branch has zero internal seams, so
...
interface developed in PR #39.  That PR is a separate branch and is not an
import or theorem dependency of this branch.  Consequently the signed
extension here is conditional prose until that interface lands or is rebased
into the same dependency graph.
