...
| `LEAN-P0-4` | Discharge nondegeneracy of the germ quit family — that it is not eventually all-continue — or show it can fail and scope the order machinery accordingly. | `QuittingAnalyticGerm`. | same | `OPEN`; stated as a hypothesis, not discharged. | Without it the normalized direction is undefined and the leading-order package is vacuous on that germ. |
