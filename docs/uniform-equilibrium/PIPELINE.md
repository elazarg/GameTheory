...
| `PC-003` | The current P0 hinge is escaping-middle tightness/compactification plus a repair-or-fixed-debt-descent decoder. | Two endpoint charts, packet provenance, exact finite-block holonomy, and now the full fixed-cutoff provenance lift are compact/closed. A machine-checked length fence proves that literal unbounded game length cannot live in any compact `ℕ × X` lift. | Equate compact scalar coefficients—or fixed-cutoff closure—with a compact bounded-cost executable repair relation. | Prove uniform middle-length tightness; or add an infinity/stopping-law chart plus a separately bounded finite decoder; or exhibit a calibrated incompatibility family. | A simpler repair closes every plateau, or a decisive incompatibility chooses the necessary new state/route. |
| `PC-004` | Run a direct repair ladder and all-behavior counterexample CEGIS in parallel with the P0 compactness route. | Static/short repairs may close the plateau before general geometry; the conjecture is universal, so a certified barrier is decisive. | Wait for one grand positive proof before searching for refutations. | Stationary/full-set/short-word search exports exact violated inequalities to the descent or barrier lane. | One lane obtains an exhaustive certificate that subsumes the others. |
...
| --- | --- | --- | --- | --- | --- |
| `MATH-P0-1` | Resolve escaping middle length: first test an enriched absorption-mass/stopping-law chart carrying payoff, caps, owner clocks, and the marked packet; otherwise prove a uniform finite-length reduction or give a calibrated incompatible-splice family. | `QuittingBoundaryHolonomy`, `QuittingBoundaryHolonomyCompactness`, two-ended packet, calibrated minimizer provenance, corrected absorption-path endpoint convention. | [Realized anchored holonomy closedness](../../ideas/PositivePlateauBoundaryClosure/RealizedAnchoredHolonomyClosedness.md), [enriched absorption paths](../../ideas/PositivePlateauBoundaryClosure/EnrichedAbsorptionPathsMayCompactifyTheEscapingMiddle.md) | `ACTIVE`; every fixed cutoff is compact/closed with full provenance, while literal unbounded length is provably noncompact. Published absorption-mass compactification is the leading infinity-chart candidate, but its terminal-jump adapter and retained strategic data are incomplete. | An arbitrary-length limit decodes to a uniformly bounded executable repair state, or the counterexample identifies the missing scale/state and rules out this compactification. |
| `MATH-P0-2` | Decode an anchored close seam as a cap-certified repair and every certified exit/dead end as cutoff-independent root-debt descent. | `MATH-P0-1`, E40, E46, E47, exact root construction. | [Anchored repair or descent](../../ideas/PositivePlateauBoundaryClosure/AnchoredRepairOrUniformDebtDescent.md) | `ACTIVE`; `GreedyBufferedExitDecoder` is at its abstract stopping point, but anchor transport, bounded game-stage cost, and root-level exit semantics are missing. | Repair reaches terminal approximate existence, or fixed `L,c>0` contradicts the positive plateau. |
...
| `LEAN-P1-3` | Package Q132's exact behavioral nonattainment table. | behavioral hazards as quit-time/Never mixtures. | [Nonattainment fence](../../ideas/StationaryRepairExhaustion/NaiveStationaryCompactificationNeedNotAttainEquilibrium.md) | `READY/PARTIAL`; explicit game theorem absent. | Permanent regression for compactified cap/attainable-tail claims. |
...
fixed-cutoff resolved lift compact/closed; literal length fence -------- [L]
       ...
       MATH-P0-1: tightness OR infinity chart + bounded decoder
                ...
                |
       MATH-P0-2: anchored repair OR fixed root-debt descent
                |
