...
| `LEAN-P0-3` | Pin the matching scaling case in the germ bridge: expand the absorption product to first order so `t^q / absorption` is pinned rather than squeezed between `1/(n·Σa)` and `1/Σa`. | `QuittingAnalyticGerm`. | same | `READY`; the only missing input is the first-order expansion, and analyticity of absorption is already proved. | Completes the three-way scaling comparison on absorption itself, which the vanishing-branch argument consumes. |
