...
The joint carrier is now production.  It is the compact metrizable closure of
one finite semantic encoding containing the completed absorption law with
`Never`, holonomy, exact-D endpoints, packet, debt, and the marked stage graph.
Finite points are sequentially dense and every sequence has a convergent
subsequence.  Completed composition is retained as the closed ternary relation
generated by correlated finite exact seams.  This supplies exact-seam
approximants for points already in the relation, but does not assert total
lifting from arbitrary equal-anchor pairs or amalgamation of independently
chosen relation witnesses.

The consumer interface is also landed.  The completion exports its metric
topology, retracts extended holonomy coordinates continuously to the uniform
reward-dependent real coefficient box, and recovers the literal holonomy of
every finite coherent cylinder.  Legal chronological splices remain in that
box by weighted intercept and tail estimates.  Consequently the physical
all-behavior tail-repair value and playerwise obstacle cap extend continuously
to completed paths, and arbitrary finite coherent sequences admit a
subsequence on which the completed path and joint repair state converge.
...
### `MATH-P0-1` — compact metrizable completion of marked semantic paths

- **Status:** COMPLETED (semantic-graph scope; ordered provenance and surgery
  pullbacks remain separate obligations)
- **Lane:** P0
...
**Objective.** Prove a compactness theorem for the **joint marked semantic
graph** with finite calibrated blocks sequentially dense in it, carrying
exit-or-Never mass, anchors, the conditional packet, payoff, the completed
marked stopping-obstacle and deleted-clock graph, debt, and a continuous
projection to boundary holonomy.  The current physical-tail branch needs the
extensional graph, not an ordered history.  Treat ordered provenance and total
exact-seam/self-seam finite pullbacks separately for decoders that perform
complementary-cylinder surgery.

**State.** Completed at the semantic-graph scope required by prefix
consumption.  The source-free finite cylinder's joint stage graph carries all
player clocks, coalition absorption, affine prefix/suffix marks, holonomy,
anchors, debt, and the marked packet.  Finite composition is associative, the
complete realized encoder is a homomorphism for adjacent block concatenation,
and strong finite semantic coherence is closed under exact-anchor splicing.
The production metrizable completion jointly retains those coordinates plus
the absorption law with `Never`; finite semantic points are sequentially
dense, every sequence has a convergent subsequence, and the maximal
Stone--Cech envelope maps continuously onto it.  Continuous projections expose
holonomy, packet, debt, anchors, stage graph, coalition masses, and `Never`.
The exported bounded real decoder recovers each finite holonomy exactly and
makes the all-tail repair value and obstacle cap continuous; its generic
subsequence capstone packages joint convergence of completed paths and repair
states without imposing zero-boundary provenance.

Completed exact-seam composition is the closed ternary relation generated by
finite correlated triples.  Each witness is sequentially approximable by
literal exact seams and has equal limiting anchors.  A compact six-coordinate
relation records coherent limits of finite associative diagrams.  These are
not a total lifting theorem for arbitrary equal-anchor pairs and not an
amalgamation theorem for independently selected splice witnesses; those remain
optional surgery obligations rather than part of the physical-tail repair
branch.
...
The earlier full-carrier claim was too strong.  A hypograph over mass retains
...
debt, and packet.  Ordered chronological provenance is a separate carrier
refinement if a later surgery decoder observes it.  See `PC-012` and
...
approximate anchor match into an exact complementary seam.  The marked graph,
packet, and debt remain available for the unresolved prefix-consumption branch.

**Acceptance.** Met: the joint semantic graph has continuous
holonomy/packet/debt projections, compactness, ordinary finite sequential
density, subsequence compactness, a bounded real holonomy decoder, and a
continuous physical all-tail repair observable.  Continuous mark transport and total
exact-seam/self-seam pullback remain additional surgery obligations rather than
consequences silently derived from compactness.  Coordinates may be removed
only after a fibre-constancy theorem.
...
Two exact selection tests fence this route.  First, lowering a cap stack from
debt `D` to a prescribed target forces absorption at least `(D-target)/D`.
The stack payoff has an exact conditional-delivery decomposition, so a large
descent can still be hidden only by cancellation between the absorbed
delivery and the surviving terminal cap.  The local/global period-one
regression shows that an admissible all-Continue cap root may stall instead.
Second, along the auxiliary homotopy `cap-t*debt`, every root with `t<1` is
all-Continue at a positive minimum.  A nontrivial endpoint at `t=1` is a solo
debt gate with a strict joiner or punishment moat, but exact Nash selection
need not choose it: the same regression remains all-Continue on the entire
closed homotopy.  Renewal now needs a root-selection or cancellation theorem,
not another compactness argument.
...
Allowing a fractional costate on the whole maximum-debt face does not improve
any separable row objective: linear optimization purifies it to one maximal
debtor.  Fractional weights matter only after coupling reset transfer and
quitting incidence.  The finite coupled model has exact forward Hall cut
inequalities; any strict cut excludes all fractional flows and yields a
quantitative maximal-debtor/singleton-surplus witness, while any feasible flow
contains a pure positive label path.  Neither output is yet chronological.
The weighted Hall converse still requires a threshold/layer-cake decoder from
real Farkas potentials to a player-subset cut, and a positive label path does
not carry co-realized semantic states or exact edges.

The local defect itself now has a complementary coalition grading.  Its exact
played-action formula expands through the Boolean Möbius singleton, pair, and
higher coefficients.  Positive-part subadditivity gives a nonnegative upper
charge by honest played coalition cylinders.  Over any finite cutoff, positive
defect occupation selects one fixed player, coalition, and sign orientation
carrying at least the reciprocal of the finite label count.  This is not yet a
deviation: the cylinder is generated by opponents, may occur at scattered
incompatible states, and the positive-part upper bound is irreversible without
a no-cancellation or tightness theorem.

Two sharp tests further restrict this proposal.  First, the genuine
state-matched minimum semantic spine conserves every debt coordinate.  Its
consecutive debt-transfer matrix is therefore identically zero, and no
positive matched reset/incidence flow can live on it.  Positive transfer is
co-realized only on a best-response reset excursion whose cluster may leave
the minimum fiber.  A useful transport theorem must account for that
off-minimum excursion and return, rather than attach abstract flow weights to
the minimum spine.  Second, positive raw Möbius incidence is not net gain: a
two-player row has positive singleton and oppositely oriented pair incidence
while its endpoint difference and coordinate defect are both exactly zero.
Legal finite policy interpolation sees the net defect, not the raw charge,
and its gain is already bounded by the player's initial best-response debt.

There is nevertheless one same-profile first-order gain in the collision
branch.  At the selected pure-time Quit date, the persistent collision atom
...
local defects is uniformly at least half the marked collision mass times the
positive minimum debt; the marked player's own defect tends to zero.  One
cannot Nashify those other coordinates abstractly while retaining the exit
face.  A two-player regression starts with sure collision and exact marked
optimality, yet `epsilon`-Nashifying the other coordinate bounds its Quit mass
by `epsilon` and reverses the marked inequality for `epsilon < 1/2`.  Minimum
...
in excursion excess, while the full opposite-face transfer identity is
retained.  A singleton reward above the cluster cap forces an absorbing
