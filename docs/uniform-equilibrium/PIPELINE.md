...
| --- | --- | --- | --- | --- | --- |
| `MATH-P0-1` | Resolve escaping middle length: prove a uniform finite-length reduction; or construct an infinity/stopping-law chart with a bounded finite decoder; or give a calibrated incompatible-splice family. | `QuittingBoundaryHolonomy`, `QuittingBoundaryHolonomyCompactness`, two-ended packet, calibrated minimizer provenance. | [Realized anchored holonomy closedness](../../ideas/PositivePlateauBoundaryClosure/RealizedAnchoredHolonomyClosedness.md) | `ACTIVE`; every fixed cutoff is compact/closed with full provenance, while literal unbounded length is provably noncompact. | An arbitrary-length limit decodes to a uniformly bounded executable repair state, or the counterexample identifies the missing scale/state and rules out this compactification. |
