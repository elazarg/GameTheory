...
| --- | --- | --- | --- | --- | --- |
| `MATH-P0-1` | Encode every calibrated exact-D block as a marked subprobability absorption cylinder, then prove sequential compactness/closedness for an enriched path carrying exit-or-Never mass, anchors, the conditional packet, payoff, full stopping obstacles, owner clocks, and debt. | `QuittingBoundaryHolonomy`, `QuittingBoundaryHolonomyCompactness`, two-ended packet, calibrated minimizer provenance. | [Realized anchored holonomy closedness](../../ideas/PositivePlateauBoundaryClosure/RealizedAnchoredHolonomyClosedness.md), [enriched absorption paths](../../ideas/PositivePlateauBoundaryClosure/EnrichedAbsorptionPathsMayCompactifyTheEscapingMiddle.md) | `ACTIVE`; finite semantics and fixed-cutoff closure are landed. Ordinary AP compactness is published, but the marked subprobability/stopping-obstacle enrichment may fail to be closed. | Exact finite encoding and concatenation identities; then either a compact closed strategic graph or a same-ordinary-AP/incompatible-obstacle counterexample identifying missing state. |
| `MATH-P0-2` | Prove the robust pointwise alternative with two separate consumers: a corrected augmented-AP-to-terminal-profile compiler, or a bounded finite surgery giving strict cutoff-independent debt descent at the original root; uniformize by sequential compactness. | `MATH-P0-1`, corrected full-jump continuation semantics, E40, E46, E47, exact root construction. | [Anchored repair or descent](../../ideas/PositivePlateauBoundaryClosure/AnchoredRepairOrUniformDebtDescent.md), [enriched absorption paths](../../ideas/PositivePlateauBoundaryClosure/EnrichedAbsorptionPathsMayCompactifyTheEscapingMiddle.md) | `ACTIVE`; the abstract buffered-path trichotomy is complete, but neither game-facing decoder nor its local stability theorem is proved. | For fixed accuracy and positive debt threshold, every limit state has one stable finite repair certificate or fixed local `L_z,c_z>0`; sequential contradiction produces uniform `L,c`, terminal approximate profiles, or contradicts the plateau. |
