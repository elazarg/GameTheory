...
This branch formalizes the finite certificate-facing part of the essential APS
...
`QuittingEssentialAPSAll.lean` is the umbrella import. `GameTheoryTest.lean`
imports that umbrella, so every implementation module is reachable from a
default Lake target and covered by the repository import audit.
...
the explicit positive hazard and opponent-contraction fields are the
executable progress witnesses that rule out the operator's `p = 0`
