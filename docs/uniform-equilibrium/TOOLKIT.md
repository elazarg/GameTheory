...
| Two-owner tangent support | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerSupport.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerExactRoot.lean` | After Bellman elimination, the reduced two-owner Jacobian is `[[0,D12],[D21,0]]` with determinant `-D12*D21`. Exact compatibility forces both effects and the whole Jacobian to vanish, but the singular chart integrates explicitly: for every nondegenerate two-owner hazard pair, rational Bellman continuation closes both active gains. Inactive Nash signs and floor/upper-box bounds then package one exact edge; return and reachability are not supplied. |
