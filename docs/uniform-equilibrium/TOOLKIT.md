...
| Literal terminal semantic carrier | `UniformEquilibrium/Quitting/Root/TerminalSemanticPair.lean`, `TerminalSemanticMoment.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticEqualityStratum.lean`, `TerminalSemanticSoloOwnerRefinement.lean`, `TerminalSemanticMinimumSpine.lean`, `TerminalSemanticSoloSpineOccupation.lean`, `TerminalSemanticSupportEntry.lean`, `TerminalSemanticAtomicSupportBoundary.lean`, `TerminalSemanticAllContinuePlateau.lean`, `TerminalSemanticPlateauTightness.lean` | The closure of executable `(terminal payoff, all-behavior envelope)` pairs is compact, sequentially generated by actual profiles, invariant under root prefixing, and its prescribed coordinate lies in the finite convex hull of zero and the terminal reward vectors. Total nonnegative debt attains a positive minimum in a counterexample and is fixed coordinatewise by exact Nash prefixing. Its compact predecessor-serial edge graph gives an infinite state-matched exact-Nash chronology. Exact owner-occupation accounting eliminates the non-atomic solo spine: vanishing survival gives an isolated-negative atomic endpoint, while positive survival forces an all-Continue minimum cluster point. Thus every counterexample has either a positive minimum all-Continue semantic plateau or a positive-rate atomic solo endpoint with `solo <= -terminalGap` and `solo < punishmentValue`. The atomic edge retains minimum-tail provenance and exposes a first feasible support boundary: below rate one its owner collision derivative is nonzero or another outsider is cotight; at rate one it is strictly negative or another outsider is cotight. In the negative rate-one branch the tight entrant is quantitatively not a punisher. Plateau best responses retain a profitable atom of positive mass and distinguish literal Never, at-stop collision, and opponent absorption before the stop. Replacing the debtor by that asymptotic best response resets its debt and transfers the full counterexample gap to a fixed different player along a subsequence. The residual is time-local charging of that atom or a nonlinear multi-outsider support-or-punishment theorem. |
