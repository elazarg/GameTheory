...
| Literal terminal semantic carrier | `UniformEquilibrium/Quitting/Root/TerminalSemanticPair.lean`, `TerminalSemanticMoment.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticEqualityStratum.lean`, `TerminalSemanticMinimumSpine.lean`, `TerminalSemanticAuxiliaryNashBudget.lean`, `TerminalSemanticAllContinuePlateau.lean`, `TerminalSemanticPlateauTightness.lean`, `TerminalSemanticPlateauQuantitativePassport.lean`, `TerminalSemanticPlateauTimeDisintegration.lean` | The closure of executable `(terminal payoff, all-behavior envelope)` pairs is compact, sequentially generated by actual profiles, invariant under root prefixing, and its prescribed coordinate lies in the finite convex hull of zero and the terminal reward vectors. Total nonnegative best-response debt attains a positive minimum in a counterexample. Selecting a finite mixed-Nash root against an arbitrary auxiliary continuation `b-h` and prefixing it to the actual minimum pair gives the exact budget `D*kappa + sum_i alpha_i*(D-h_i) <= 0`. Consequently every positive minimum pair is an all-Continue Nash self-loop; the former provenance atomic branch is impossible. At most one coordinate can be singleton-tight, and any exact Nash root against the prescribed minimum value is collision-free and either all-Continue or a solo root whose owner carries all debt. Plateau best responses retain a quantitatively profitable atom of positive mass and distinguish literal Never, at-stop collision, and opponent absorption before the stop. The atom's terminal mass now has an exact survival-weighted time disintegration on the same actual profile, with concentrated-stage versus diffuse-charge alternatives. The residual is strategic incidence: those actual live rows are not yet identified with exact minimum-semantic prefix rows, so their support/charge is not yet a legal debt-utilization edge. |
