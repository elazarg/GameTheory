...
| Phantom-boundary conditioning and diffuse compilation | `UniformEquilibrium/Quitting/Cycles/PhantomBoundaryConditioning.lean`, `ConditionedProductPurification.lean`, `ConditionedSingletonStrategicPurification.lean`, `ConditionedTangentSeam.lean`, `ConditionedDiffuseChronology.lean`, `ConditionedDiffuseProductRescaling.lean`, `ConditionedDiffuseStrategicRescaling.lean`, `ConditionedDiffuseCompiler.lean`, `ConditionedDiffuseUniform.lean`, `UniformEquilibrium/Quitting/Paths/HazardScaledResidualCompiler.lean`, `JointPolicySeparatedErrorCompiler.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeConditionedFloorViability.lean`, `CounterexampleRegimeConditionedSlackThreshold.lean`, `CounterexampleRegimeAtomicOwner.lean` | Conditions a positive-survival exact tail on eventual absorption without losing consecutive chronology. Tight phantom-floor coordinates remain viable; strict-floor failures are paid by plateau slack, while an affine repair recreates Never mass and cannot finitely rejoin scale one. Singleton rows purify exactly. Multi-owner conditional laws need not be product laws, but diffuse hazard rescaling is legal and approximates them quadratically. Exact source complementarity retains player-deleted absorption in the Continue error, and the two-clock compiler turns uniformly small mesh plus completeness of every conditioned deleted clock into an explicit asymptotic Nash profile with linear-in-mesh error. Arbitrarily fine certificates select a uniform payoff. A positive-rate sole-owner limit closes by punishment completion. The remaining conditioned regimes are strict plateau slack and failure of deleted-clock completeness; the latter still needs an asymptotic solo-chronology producer before the approximate solo compiler applies. |
