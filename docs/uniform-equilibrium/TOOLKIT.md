...
| Literal terminal semantic carrier | `UniformEquilibrium/Quitting/Root/TerminalSemanticPair.lean`, `TerminalSemanticMoment.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticEqualityStratum.lean`, `TerminalSemanticSoloOwnerRefinement.lean`, `TerminalSemanticMinimumSpine.lean`, `TerminalSemanticAllContinuePlateau.lean`, `TerminalSemanticPlateauTightness.lean` | The closure of executable `(terminal payoff, all-behavior envelope)` pairs is compact, sequentially generated by actual profiles, invariant under root prefixing, and its prescribed coordinate lies in the finite convex hull of zero and the terminal reward vectors. Total nonnegative debt attains a positive minimum in a counterexample and is fixed coordinatewise by exact Nash prefixing. The minimum fiber has a compact predecessor-serial edge graph, hence an infinite state-matched exact-Nash chronology. It is either a positive-debt all-Continue semantic fixed plateau, or one fixed positive-debt owner quits alone at every date, is singleton-tight, and deters an attractive outsider through exit/collision risk. The latter row yields the quantitative isolated-negative obstruction or a concrete blocker and strict collision cliff. On the plateau, pure-time best-response terminal laws converge to the exact envelope moment and retain positive mass on a profitable Never or non-own-singleton atom. The residual is strategic co-realization with one common chronology, not state matching or probability tightness. |
