...
| Literal terminal semantic carrier | `UniformEquilibrium/Quitting/Root/TerminalSemanticPair.lean`, `TerminalSemanticMoment.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticAuxiliaryNashBudget.lean`, `TerminalSemanticMinimumAggregateSurplus.lean`, `TerminalSemanticMinimumAggregateSurplusConsumer.lean`, `TerminalSemanticMinimumPlateauPacket.lean`, `TerminalSemanticPlateauTightness.lean`, `TerminalSemanticPlateauTimeDisintegration.lean`, `TerminalSemanticPlateauIncidence.lean`, `TerminalSemanticPlateauDefectCharge.lean`, `TerminalDebtLiteralStackAllContinueRegression.lean` | Executable `(terminal payoff, all-behavior envelope)` pairs generate a compact prefix-invariant carrier. Total best-response debt attains a positive minimum in a counterexample. Auxiliary Nash selection gives `D*kappa + sum_i alpha_i*(D-h_i) <= 0`, forcing every positive minimum pair to be an all-Continue self-loop and eliminating the former atomic branch. The open auxiliary cube is inert; its critical face is collision-free and singleton mass can occur only at a full-debt coordinate. Every player subset has one positive-mass reward-moment outcome carrying its exact aggregate singleton surplus; in a counterexample this is Never or a strict-toggle-blocked coalition. Plateau deviations retain quantitative marked mass with exact time disintegration and carrier-owned live-row prefix incidence. Arbitrary-root defect charging proves `(1-c_-i)d_i(tail) <= d_i(tail)-d_i(current)+delta_i`; pure-time survival-weighted defects are bounded by the initial debt. The remaining pointwise term—positive tail-debt drift—is sharp even when the local defect is zero. Deep exact stacks may remain all-Continue, and the static aggregate alternative is saturated by the negative control. The residual is a stopping-time-respecting global drift telescope or a negative/Never vertex-germ consumer. |
