...
| Aggregate prefix-consumption gate | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeAggregatePrefixConsumption.lean` | Sandwiches the terminal gap below the canonical full-prefix behavioral repair value and that value below the minimum aggregate exact-`D` objective. At every cutoff a marked aggregate anchor therefore carries the gap with an explicit reward/cardinality packet-mass bound. A conditional theorem consumes half the gap through next-cutoff objective drop or a legal reachable predecessor charge; literal state attachment and the exploitability-versus-drop/charge comparison remain hypotheses. |
