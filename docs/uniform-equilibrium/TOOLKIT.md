...
| Literal terminal-debt prefix descent | `UniformEquilibrium/Quitting/Root/TerminalDebtPrefix.lean`, `UniformEquilibrium/Quitting/Boundary/Repair/TerminalDebtSingletonDescent.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalDebtPrefixDescent.lean` | Prepending an exact mixed Nash root against an actual suffix payoff weakly decreases every all-behavior terminal-debt coordinate. A positive singleton gap yields a uniform strict decrease in that coordinate and in total debt, while maximum exploitability remains nonincreasing. Lexicographic near-minimizers exist at every accuracy, and the secondary objective handles arbitrary ties. The gap-carrying player may vary with the accuracy. A small-opponent-absorption endpoint gain converts quantitatively to a singleton gap against the literal suffix. The consumer requires such a gap at a positive-debt coordinate of actual near-minimizers; conditioned values and stored Bellman boundaries require a separate co-realization theorem. |
