...
| Projective-gauge scalar obstruction | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentProjectiveGaugeDefect.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentProjectiveGaugeScalarClosure.lean` | Deleting one active mixing row is not locally recoverable: its exact missing scalar can remain nonzero arbitrarily near the packet. Nor is first-order sign change automatic. For three active owners, a nonzero radial minor gives a fixed one-sided defect sign; a zero minor leaves an explicit higher-order singular direction. A different structural identity or nonlinear resolution is required. |
| Two-owner tangent support | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerSupport.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerExactRoot.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerPacketEdge.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerPacketDichotomy.lean` | Exact two-owner compatibility has an explicit rational root manifold. Along the packet ray, strict physical slack gives positive-charge exact edges at every sufficiently small scale. Otherwise a punishment boundary, inactive singleton row, or upper-box coordinate is tight; active positive tangent at the punishment floor blocks the ray, while tight inactive singleton equality is a finite outsider regression. No return is supplied. |
...
| Reachable carried-debt telescope | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeReachableCarryTelescope.lean` | Scaled remaining prefix capacity pays every diagonal seam and telescopes carried debt to one far boundary. With nonpositive punishment, each finite zero-boundary chain is intrinsically floor-admissible and handled independently. Its terminal payoff is zero and terminal debt is the sum of positive singleton caps: cap-zero closes automatically, while a sufficiently charged admissible path ending there funds the boundary. The intrinsic reversed chain has the wrong orientation; `CounterexampleRegimeNonpositiveFloorTerminalCapRegression.lean` shows nonpositive punishment need not make the cap zero. |
