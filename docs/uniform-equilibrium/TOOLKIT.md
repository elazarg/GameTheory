...
| Projective-gauge row defect | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentProjectiveGaugeDefect.lean` | Shows that deleting one active mixing row is not locally recoverable in general. The missing scalar is `Bellman_i+(1-t*a_i)*Mixing_i`, and an exact sum-one regression can annihilate all Bellman and retained mixing rows while the omitted row remains nonzero arbitrarily near the packet. Compatibility controls only the mass direction, so a different structural identity or nonlinear resolution is required. |
| Two-owner tangent support | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerSupport.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerExactRoot.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerPacketEdge.lean` | After Bellman elimination, exact two-owner compatibility forces the reduced Jacobian to zero, but the singular chart integrates explicitly. Along the packet ray, positive subunit hazards are automatic; nonnegative tangent controls the active upper bound, strict floor and outsider singleton slack are stable, while tight active floor plus positive tangent fails at every positive scale. Remaining inactive/tight finite gates package one exact edge, not a return. |
...
| Reachable carried-debt telescope | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeReachableCarryTelescope.lean` | On any coherent chronology of literal punishment-floor predecessor edges with matching exact dynamic-debt states, scaled remaining prefix capacity pays every diagonal seam and all carried terms telescope to one far-debt remainder. With nonpositive punishment, each finite zero-boundary chain is intrinsically floor-admissible and the global admissible potential telescopes it independently, eliminating cross-cutoff nesting; its own terminal state need not be the floor anchor, and same-state terminal debt versus capacity remains the exact gate. |
