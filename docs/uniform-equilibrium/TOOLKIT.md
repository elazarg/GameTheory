...
| Phantom-boundary conditioning and product purification | `UniformEquilibrium/Quitting/Cycles/PhantomBoundaryConditioning.lean`, `ConditionedProductPurification.lean` | Conditions a positive-survival exact tail on eventual absorption without losing consecutive chronology. The conditioned values obey an exact convex recursion and active-face error bound. A singleton-support row is exactly purifiable to an ordinary solo product root at the conditioned hazard. With positive Continue mass and two active marginals, the conditional coalition law rigidly determines the original product root, so genuine phantom rescaling is impossible rowwise. The remaining diffuse route needs singleton chattering or another approximate chronological construction. |
