...
| Combined quitting counterexample regime | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeAll.lean` | An exact normal form combining a positive terminal gap with finite canonical punishment-floor prefix capacity. It derives `η ≤ inf_N D_N ≤ D_N ≤ K ≤ M`, zero-charge recurrence, a forced singleton packet with a strict refusal defect, and an optimized exact-D tail converging to a positive-debt all-Continue self-loop. `CounterexampleRegimeSeam.lean` packages both objects and exposes every periodic tail window through the exact pure-time/`Never` deviation cap. |
