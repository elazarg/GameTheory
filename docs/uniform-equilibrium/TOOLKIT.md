...
| Counterexample tangent packet | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentPacket.lean` | Uses the exact finite charge-tangent identity to extract a coherent normalized owner occupation, boundary, and signed endpoint tangent from the actual optimized tail. The tail is eventually all-Continue or yields a nonzero packet; every such counterexample packet has a negative coordinate or an active positive coordinate. Phase repair and support enlargement remain the two separate consumers. |
| Period-one tangent atlas | `UniformEquilibrium/Quitting/Cycles/PeriodOneTangentAtlas.lean` | Identifies the exact one-root joint/opponent survival split with normalized singleton-owner mass, then rewrites the phase and proper-mass refusal evaluators as signed endpoint-tangent terms with their finite strategic slacks retained. Full owner mass is the separate zero-denominator branch in which every opponent continues surely. |
