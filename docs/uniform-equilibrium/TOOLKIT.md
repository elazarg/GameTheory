...
| Phantom-boundary conditioning and product purification | `UniformEquilibrium/Quitting/Cycles/PhantomBoundaryConditioning.lean`, `ConditionedProductPurification.lean`, `ConditionedSingletonStrategicPurification.lean`, `ConditionedTangentSeam.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeConditionedFloorViability.lean`, `CounterexampleRegimeConditionedSlackThreshold.lean`, `CounterexampleRegimeAtomicOwner.lean` | Conditions a positive-survival exact tail on eventual absorption without losing consecutive chronology. Tight phantom-floor coordinates remain viable; strict-floor failures are quantitatively paid by plateau slack, while transporting a pointwise affine repair recreates positive Never mass and cannot finitely rejoin scale one. A singleton-support row is exactly purifiable to an ordinary solo product root at the conditioned hazard, and a coordinatewise singleton-tight interior solo path retains exact Bellman and endpoint Nash. Two persistent owners close by deleted-clock contraction; a positive-rate sole-owner limit closes by punishment completion, even with negative singleton payoff. Negative tangent either remains a negative conditioned delivery gap or is funded by strict phantom slack; insufficient slack makes the diagnostic delivery both phase-exploitable and underfloor. With positive Continue mass and two active marginals, the conditional coalition law rigidly determines the original product root, so genuine phantom rescaling is impossible rowwise. The remaining tight-singleton route is diffuse and needs a deleted-clock approximation; non-tight coordinates require a support- or delivery-changing reset. |
