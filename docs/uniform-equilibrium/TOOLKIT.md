...
| Regular tangent arc lift | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentRegularArcLift.lean` | Removes the common radial factor from the full Bellman and active-mixing equations under `hazard=t*leading`, `continuation=boundary+t*drift`, retaining all coalition terms. A compatible packet lies on the exceptional-divisor equality locus. Surjective derivative plus an outward kernel direction gives a positive analytic equality arc by the existing implicit-function theorem; strict physical cell signs let its nonzero points decode to exact Nash--Bellman roots. Singular derivatives and boundary sign cells remain open. |
| Two-owner tangent support | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeTangentTwoOwnerSupport.lean` | After Bellman elimination, the reduced two-owner Jacobian is `[[0,D12],[D21,0]]` with determinant `-D12*D21`; a signed pivot is regular iff its reciprocal directed effect is nonzero, with an explicit outward solve. If exactly two positive-mass owners are compatible, both effects and the whole Jacobian vanish. Negative reciprocal collision then equals positive reciprocal singleton effect but leaves a genuinely higher-order singular chart. |
...
| Reachable carried-debt telescope | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeReachableCarryTelescope.lean` | On any coherent chronology of literal punishment-floor predecessor edges with matching exact dynamic-debt states, scaled remaining prefix capacity pays every diagonal seam and is killed-excessive for aggregate debt. All carried terms telescope to one survival-weighted far-boundary comparison. Constructing such a chronology from calibrated minimizers and dominating that boundary remain explicit premises. |
