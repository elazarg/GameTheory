...
| Aggregate prefix-consumption gate | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeAggregatePrefixConsumption.lean` | Sandwiches the terminal gap below the canonical full-prefix behavioral repair value and that value below the minimum aggregate exact-`D` objective. At every cutoff a marked aggregate anchor therefore carries the gap with an explicit reward/cardinality packet-mass bound. Immediate `Never` exploitability is exactly maximum dynamic debt; for a literally attached reachable edge, bounding the admissible prepended chain's residual aggregate debt by scaled legal charge consumes half the gap through objective drop or charge. Sure caps and positive internal cutoffs still need boundary reinsertion. |
