...
| Phantom-boundary conditioning and product purification | `UniformEquilibrium/Quitting/Cycles/PhantomBoundaryConditioning.lean`, `ConditionedProductPurification.lean`, `ConditionedSingletonStrategicPurification.lean`, `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeConditionedFloorViability.lean` | Conditions a positive-survival exact tail on eventual absorption without losing consecutive chronology. Tight phantom-floor coordinates remain viable; strict-floor failures are quantitatively paid by plateau slack, while transporting a pointwise affine repair recreates positive Never mass. A singleton-support row is exactly purifiable to an ordinary solo product root at the conditioned hazard, and a coordinatewise singleton-tight interior solo path retains exact Bellman and endpoint Nash. With positive Continue mass and two active marginals, the conditional coalition law rigidly determines the original product root, so genuine phantom rescaling is impossible rowwise. A counterexample on the purified singleton stratum must retain an opponent-survival obstruction; an atomic one-owner limit must have negative singleton self-payoff. The remaining diffuse route needs a deleted-clock approximation or another chronological construction. |
