...
| Literal terminal semantic carrier | `UniformEquilibrium/Quitting/Root/TerminalSemanticPair.lean`, `TerminalSemanticMoment.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticAuxiliaryNashBudget.lean`, `TerminalSemanticMinimumDebtSimplex.lean`, `TerminalSemanticWeightedAuxiliaryNashBudget.lean`, `TerminalSemanticMinimumAggregateSurplusConsumer.lean`, `TerminalSemanticMinimumPlateauPacket.lean`, `TerminalSemanticPlateauTightness.lean`, `TerminalSemanticPlateauTimeDisintegration.lean`, `TerminalSemanticPlateauIncidence.lean`, `TerminalSemanticPlateauPositivePartSplit.lean`, `TerminalSemanticPlateauDefectCharge.lean`, `TerminalSemanticPlateauDefectTelescope.lean` | Executable `(terminal payoff, all-behavior envelope)` pairs generate a compact prefix-invariant carrier. Auxiliary Nash selection forces every positive minimum-debt pair to be an all-Continue self-loop and eliminates the former atomic branch. Normalized debt lies in the player simplex; singleton slack is nonnegative, and nontrivial exact minimum roots occur only as solo roots at unique full-debt/zero-slack vertex gates. Positive weighted objectives have the same anisotropic moat. Every player subset has one supported reward-moment outcome carrying its exact aggregate surplus; no-UE makes it Never or strict-toggle blocked. One actual pure-time law pays debt either through quantitative harmonic Never mass or through outcome-count-free opponent-containing chronological charge. Live-row prefix incidence is exact. Arbitrary-root defect charging and the stopped Abel telescope consume marked mass up to tail excess and survival-weighted local-defect occupation; uniformly near-minimum tails cost only one epsilon in excess. The local-defect occupation is not automatically one player's regret, and the negative control saturates all static alternatives. The residual is a matched reset/debt-transfer consumer or a negative/Never vertex germ. |
