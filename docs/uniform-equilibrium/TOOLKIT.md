...
| Reachable carried-debt telescope | `UniformEquilibrium/Diagnostics/Quitting/CounterexampleRegimeReachableCarryTelescope.lean`, `CounterexampleRegimeTerminalIncomingPathAlternative.lean` | Scaled remaining prefix capacity pays every seam and telescopes carried debt to the terminal singleton cap. Cap-zero closes automatically. For `0<debt<|I|M`, a canonical funding hazard yields either an incoming admissible edge and the carry bound or finite Farkas multipliers. At saturation, the pure-singleton zero-target predecessor is Bellman-infeasible and more complicated full-absorption roots remain. Nonpositive punishment alone need not make the cap zero. |
