...
| Literal terminal semantic carrier | `UniformEquilibrium/Quitting/Root/TerminalSemanticPair.lean`, `TerminalSemanticMoment.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticEqualityStratum.lean`, `TerminalSemanticSoloOwnerRefinement.lean`, `TerminalSemanticMinimumSpine.lean`, `TerminalSemanticAllContinuePlateau.lean` | The closure of executable `(terminal payoff, all-behavior envelope)` pairs is compact, sequentially generated by actual profiles, invariant under root prefixing, and its prescribed coordinate lies in the finite convex hull of zero and the terminal reward vectors. Total nonnegative debt attains a positive minimum in a counterexample and is fixed coordinatewise by exact Nash prefixing. The minimum fiber has a compact predecessor-serial edge graph, hence an infinite state-matched exact-Nash chronology. It is either a positive-debt all-Continue semantic fixed plateau, or one fixed positive-debt owner quits alone at every date, is singleton-tight, and deters an attractive outsider through exit/collision risk. The latter row either closes against the owner's singleton vector, forcing the quantitative isolated-negative obstruction, or has a concrete blocker and strict collision cliff. Its owner-deleted survival is identically one, so the residual is an own-hazard occupation/terminal-realization problem. On the plateau, the prescribed vector dominates all own-singleton rewards but each positive debtor has a strictly better Never or nonsingleton-coalition atom; converting the convergent executable semantic sequence into one tight terminal law remains the live seam. |
