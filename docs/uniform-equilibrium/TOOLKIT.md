...
| Literal terminal semantic carrier | `UniformEquilibrium/Quitting/Root/TerminalSemanticPair.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticEqualityStratum.lean` | The closure of executable `(terminal payoff, all-behavior envelope)` pairs is compact, sequentially generated by actual profiles, and invariant under every root prefix. Total nonnegative debt attains a minimum. In a counterexample the minimum has positive debt, and every exact Nash prefix fixes every debt coordinate. Each positive debtor has unit opponent Continue mass and zero exercise premium; one debtor leaves only its own marginal free, while two debtors force every Quit mass to zero. The remaining bridge is a triangular conditioned extraction on a realizing profile sequence, or a compiler for the exact solo-owner/all-Continue equality face. |
