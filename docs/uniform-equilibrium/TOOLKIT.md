...
| Literal terminal semantic carrier | `UniformEquilibrium/Quitting/Root/TerminalSemanticPair.lean`, `TerminalSemanticMoment.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticEqualityStratum.lean`, `TerminalSemanticMinimumSpine.lean`, `TerminalSemanticAuxiliaryNashBudget.lean`, `TerminalSemanticMinimumAggregateSurplus.lean`, `TerminalSemanticMinimumPlateauPacket.lean`, `TerminalSemanticAllContinuePlateau.lean`, `TerminalSemanticPlateauTightness.lean`, `TerminalSemanticPlateauQuantitativePassport.lean`, `TerminalSemanticPlateauTimeDisintegration.lean`, `TerminalSemanticPlateauIncidence.lean`, `TerminalDebtLiteralStackAllContinueRegression.lean` | The closure of executable `(terminal payoff, all-behavior envelope)` pairs is compact, sequentially generated by actual profiles, invariant under root prefixing, and its prescribed coordinate lies in the finite convex hull of zero and the terminal reward vectors. Total nonnegative best-response debt attains a positive minimum in a counterexample. Selecting a finite mixed-Nash root against an arbitrary continuation `b-h` and prefixing it to the actual minimum pair gives `D*kappa + sum_i alpha_i*(D-h_i) <= 0`. Consequently every positive minimum pair is an all-Continue Nash self-loop; the former provenance atomic branch is impossible. For every player subset `J`, one common terminal outcome has aggregate surplus above the members' singleton rewards at least `(|J|-1)D`. At most one coordinate can be singleton-tight, and any exact Nash root against the minimum prescribed value is collision-free and either all-Continue or solo with the owner carrying all debt. A boundary-matched charge packet is therefore Dirac and underfunds a distinct outsider. Plateau best responses retain a quantitatively profitable Never, at-stop collision, or opponent-before-stop atom. Its mass has an exact survival-weighted time disintegration. Every exposed actual live row also has an exact carrier-owned semantic prefix factorization, so profile provenance is retained; a positive collision atom over an exact minimum tail certifies that the row is not Nash. Deep exact stacks can nevertheless remain entirely all-Continue. The residual is quantitative: charge the actual row's Nash defect or the shifted tail's excess debt to obtain a nontrivial legal minimum-prefix edge. |
