...
| Literal terminal semantic carrier | `UniformEquilibrium/Quitting/Root/TerminalSemanticPair.lean`, `TerminalSemanticMoment.lean`, `UniformEquilibrium/Diagnostics/Quitting/TerminalSemanticEqualityStratum.lean`, `TerminalSemanticSoloOwnerRefinement.lean` | The closure of executable `(terminal payoff, all-behavior envelope)` pairs is compact, sequentially generated by actual profiles, and invariant under every root prefix. Its prescribed coordinate lies in the finite convex hull of zero and the terminal reward vectors, giving an exact rational carrier-realizability screen. Total nonnegative debt attains a minimum. In a counterexample the minimum has positive debt, and every exact Nash prefix fixes every debt coordinate. The minimum stratum is either an exact all-Continue semantic self-loop, or a unique positive-debt owner quits with positive mass, is singleton-tight, and deters an otherwise-profitable outsider through its possible exit and collision rewards. The latter row either closes against the owner's singleton vector, forcing the quantitative isolated-negative obstruction, or has a concrete blocker with owner-exit payoff below the current continuation; if blocker and attractive outsider coincide, collision is strictly worse still. The remaining bridge is a state-matched inverse limit/conditioned extraction or a direct compiler for this finite solo-owner deterrence pattern. |
