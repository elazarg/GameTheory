...
| Three-player quitting existence (2026-08-10 update) | The existence theorem is known from Solan's 1999 three-player absorbing-game result. | The repository's analytic-germ / singleton-packet / dimension-three-dispatch proof is an independent formal reconstruction, not a claim of mathematical priority. | **High** within the audited proof-assistant sample; exact theorem is sealed in `ThreePlayerBaseCaseExhaustion`. | `high` that existence is known mathematics; `medium` that this is the first formalization until a broader proof-assistant search is completed. |
...
For authoritative status, use [`FRONTIER.md`](../FRONTIER.md),
[`PIPELINE.md`](../PIPELINE.md), the linked claim file,
