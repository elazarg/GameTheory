...
| Periodic-window and phantom-boundary evaluation | `M+L` | The full behavioral reply against periodic opponents is exactly a finite phase-stop/refusal maximum. Conditioning on eventual absorption removes the phantom boundary while retaining exact consecutive chronology. Tight phantom-floor coordinates remain viable; strict-floor affine repair recreates Never mass. Singleton rows purify exactly. On the diffuse singleton-tight stratum, hazard rescaling has quadratic coalition-law error, preserves nonsummable deleted clocks, and feeds a two-clock compiler with explicit error linear in the mesh; arbitrarily fine certificates now compile directly to a uniform payoff. Hence every counterexample on this stratum has a summable player-deleted conditioned clock. A positive-rate sole-owner limit is enforced by punishment completion. The residual is strict plateau slack, extraction of the deficient clock's solo chronology, support-changing reset, or a state-matched collision-aware return. |
