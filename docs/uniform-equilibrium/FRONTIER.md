...
| Literal terminal semantic equality stratum | `M+L` | The compact prefix-invariant semantic carrier has positive minimum total and maximum best-response debt in every counterexample. Auxiliary selection forces every positive minimizer of either objective to be an all-Continue self-loop. One actual pure-time law pays debt through quantitative harmonic Never mass or chronological charge. In the persistent collision branch, atom mass times the marked quitter's local defect is bounded by reset debt, so the same executable rows have vanishing marked-player defect. Their tail cluster either lies strictly above the minimum fiber or, on the fiber, other players carry a uniform positive defect sum. General exit-face Nashification is false even approximately: a two-player regression loses all collision support and reverses the marked inequality. Coupled reset/incidence flows cannot obtain provenance from the coordinate-conserving minimum spine. Cap--Nash prefixing of an off-minimum reset cluster instead has an exact excursion account: absorption pays excess while preserving the reset and transfer. Minimizing on the reset face gives either a minimum-fiber return or a canonical off-minimum point whose only cap--Nash root is all-Continue. Boolean Möbius labels remain upper charges and can cancel to zero net defect. The live gap is now other-player defect consumption on the localized collision row, or elimination/compilation of the constrained all-Continue reset plateau while retaining the original incidence law. |
