...
| Literal terminal semantic equality stratum | `M+L` | The closure of attainable literal prescribed/envelope pairs is compact, sequentially generated by executable profiles, and invariant under every root prefix. Total debt attains a positive minimum in a putative counterexample. Every exact Nash prefix fixes the full debt vector; a positive debtor has unit deleted survival and zero endpoint premium. One positive debtor leaves only its own marginal free, while two positive debtors force every Quit mass to zero. The unresolved step is triangular conditioned extraction on a realizing sequence, or direct consumption of this exact solo-owner/all-Continue face. |
