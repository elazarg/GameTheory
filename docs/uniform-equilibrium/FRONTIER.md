...
| Periodic-window and phantom-boundary evaluation | `M+L` | The full behavioral reply against periodic opponents is exactly a finite phase-stop/refusal maximum. Conditioning on eventual absorption removes the phantom boundary while retaining exact consecutive chronology and compact convex weights. Tight phantom-floor coordinates remain viable, while any strict-floor repair transported affinely recreates Never mass. Singleton rows purify exactly, and a singleton-tight interior solo path remains exact Nash; a counterexample there must retain an opponent-survival obstruction, with any atomic sole-owner limit forced to have negative self-payoff. Multi-owner rows with genuine phantom mass cannot be purified by exact rowwise product rescaling. The remaining route is a deleted-clock diffuse approximation, a monopoly exit, or a state-matched collision-aware return. |
