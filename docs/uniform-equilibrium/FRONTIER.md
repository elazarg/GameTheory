...
| Periodic-window and phantom-boundary evaluation | `M+L` | The full behavioral reply against periodic opponents is exactly a finite phase-stop/refusal maximum. Conditioning on eventual absorption removes the phantom boundary while retaining exact consecutive chronology and compact convex weights. Tight phantom-floor coordinates remain viable, while any strict-floor repair transported affinely recreates Never mass and cannot finitely return to the unshrunk chronology. Singleton rows purify exactly, and a singleton-tight interior solo path remains exact Nash. Two persistent owners contract every deleted clock; a positive-rate sole-owner limit is enforced by punishment completion, so only the diffuse mesh remains there. Negative tangent becomes either negative conditioned motion or a quantitatively slack-funded branch; insufficient slack makes the repeated-root delivery both exploitable and underfloor. Multi-owner rows with genuine phantom mass cannot be purified by exact rowwise product rescaling. The remaining route is a deleted-clock diffuse approximation, a support-changing slack reset, or a state-matched collision-aware return. |
