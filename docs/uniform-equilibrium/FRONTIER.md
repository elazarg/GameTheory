...
| Literal terminal-debt prefix action | `M+L` conditional | The global infimum of literal maximum terminal exploitability is positive in every putative counterexample. Exact Nash prefixing acts coordinatewise by `d ↦ (c*d-a)₊`; chronological blocks compose exactly and the summed debt loss is opponent-killed debt plus consumed positive endpoint premium. Arbitrarily deep finite exact-root stacks exist over lexicographic near-minimizers, with every displayed suffix pair literally co-realized and every local total drop below the common accuracy. A macroscopic debtor sees vanishing opponent hazards and premium; two macroscopic debtors force the whole root toward all-Continue. The remaining hypothesis is a positive literal premium or singleton gap on a positive-debt coordinate of the triangular stacks; conditioned and stored boundary values do not supply it without a semantic extraction bridge. |
| Literal terminal semantic equality stratum | `M+L` | The closure of attainable literal prescribed/envelope pairs is compact, sequentially generated by executable profiles, invariant under every root prefix, and its prescribed coordinate lies in the finite reward-moment polytope. Total debt attains a positive minimum in a putative counterexample. Every exact Nash prefix fixes the full debt vector. The minimum fiber's exact-Nash prefix graph is compact and predecessor-serial, producing an infinite state-matched chronology. It is either a positive-debt all-Continue semantic fixed plateau, or one fixed positive-debt owner quits alone at every date, is singleton-tight, and deters an attractive outsider through exit/collision risk. The solo row yields the quantitative isolated-negative obstruction or a concrete affine blocker and collision cliff; its owner-deleted survival is identically one. On the plateau, the prescribed vector dominates all own singleton rewards while each positive debtor has a strictly better Never or nonsingleton-coalition atom. The residual is own-hazard occupation/terminal realization or terminal-law tightness, not state matching. |
