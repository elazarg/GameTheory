...
| Literal terminal semantic equality stratum | `M+L` | The compact prefix-invariant semantic carrier has a positive minimum best-response debt in every counterexample. The auxiliary budget `D*kappa + sum_i alpha_i*(D-h_i) <= 0` forces every positive minimum pair to be an all-Continue self-loop and eliminates the former atomic branch. The open auxiliary cube is inert; the critical face is collision-free and admits singleton mass only at a full-debt coordinate. Every player subset has a positive-mass reward-moment outcome carrying its exact aggregate singleton surplus; no-UE makes it Never or a strict-toggle-blocked coalition. A pure-time deviation retains quantitative marked mass with exact time disintegration and carrier-owned live-row incidence. Arbitrary-root defect charging gives `(1-c_-i)d_i(tail) <= d_i(tail)-d_i(current)+delta_i`, and pure-time survival-weighted defects are bounded by the initial response debt. The remaining positive tail-debt drift is pointwise sharp even at zero local defect. Deep exact stacks can remain all-Continue and the finite aggregate alternative is saturated by the negative control. The exact residual is a stopping-time-respecting drift telescope or a negative/Never vertex-germ consumer. |
