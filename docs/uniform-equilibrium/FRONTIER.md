...
| Literal terminal semantic equality stratum | `M+L` | The compact prefix-invariant semantic carrier has positive minimum total and maximum best-response debt in every counterexample. Auxiliary selection forces every positive minimizer of either objective to be an all-Continue self-loop. Normalized total-minimum debts form a player simplex with nonnegative singleton slack; nontrivial exact minimum roots occur only as solo roots at unique full-debt/zero-slack vertex gates. Positive weighted objectives have an analogous anisotropic moat, while a near-minimax auxiliary root at distance `rho` has absorption at most `epsilon/(rho+epsilon)`. One actual pure-time law pays debt either through quantitative harmonic Never mass or through outcome-count-free opponent-containing chronological charge. Actual live rows retain exact carrier prefix incidence. A finite stopped Abel telescope consumes marked mass up to tail excess and local-defect occupation. Every simplex-valued costate supported on the maximum-debt face has nonpositive switching cost, but separable objectives purify to a single maximal debtor. Coupled reset/incidence flows satisfy exact Hall cut inequalities; a strict cut has a quantitative max-debt/surplus consumer, while a feasible flow yields only a player-label skeleton until semantic states and edges are co-realized. Boolean Möbius decomposition charges local defect to explicit played coalition cylinders and selects one fixed singleton/pair/higher label over a finite cutoff. The remaining gap is not label selection: it is a weighted Hall converse plus chronological provenance, or a no-cancellation theorem turning that occupied cylinder into a legal deviation/cycle. |
