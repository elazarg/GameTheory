...
| Literal terminal semantic equality stratum | `M+L` | The compact prefix-invariant semantic carrier has a positive minimum best-response debt in every counterexample. The auxiliary budget forces every positive minimum pair to be an all-Continue self-loop and eliminates the former atomic branch. Normalized debts form a player simplex with nonnegative singleton slack; nontrivial exact minimum roots occur only as solo roots at unique full-debt/zero-slack vertex gates. Every positive weighted objective has an analogous anisotropic moat. One actual pure-time law pays debt either through quantitative harmonic Never mass or through outcome-count-free opponent-containing chronological charge. Actual live rows retain exact carrier prefix incidence. Arbitrary-root defect charging and a finite stopped Abel telescope consume marked mass up to tail excess and survival-weighted local-defect occupation; uniform near-minimum attachment costs only one epsilon in excess. The occupation sum is not automatically a single player's collectible regret, and the static aggregate alternative is saturated by the negative control. The exact residual is a matched reset/debt-transfer consumer or a negative/Never vertex germ. |
