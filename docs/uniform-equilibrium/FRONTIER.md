...
| Literal terminal semantic equality stratum | `M+L` | The closure of attainable literal prescribed/envelope pairs is compact, sequentially generated by executable profiles, invariant under every root prefix, and its prescribed coordinate lies in the finite reward-moment polytope. Total debt attains a positive minimum in a putative counterexample. Every exact Nash prefix fixes the full debt vector. The minimum fiber's exact-Nash prefix graph is compact and predecessor-serial, producing an infinite state-matched chronology. It is either a positive-debt all-Continue semantic fixed plateau, or one fixed positive-debt owner quits alone at every date, is singleton-tight, and deters an attractive outsider through exit/collision risk. This solo row either yields the quantitative isolated-negative obstruction or a concrete affine blocker and collision cliff. The owner's deleted survival is identically one, so the unresolved step is own-hazard occupation/terminal realization or direct elimination of the all-Continue plateau, not state matching. |
