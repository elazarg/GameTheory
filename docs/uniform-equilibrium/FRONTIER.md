...
| Literal terminal semantic equality stratum | `M+L` | The closure of attainable literal prescribed/envelope pairs is compact, sequentially generated by executable profiles, invariant under every root prefix, and its prescribed coordinate lies in the finite reward-moment polytope. Total debt attains a positive minimum in a putative counterexample. Every exact Nash prefix fixes the full debt vector. The minimum stratum is either an exact all-Continue semantic self-loop, or a unique positive-debt owner has positive hazard, is singleton-tight, and deters an otherwise-profitable outsider through exit/collision risk. This solo row either yields the quantitative isolated-negative obstruction or a concrete affine blocker; a coincident attractive blocker forces a strict collision/owner-solo/prescribed/outsider-solo staircase. The unresolved step is a state-matched inverse limit/conditioned extraction or direct consumption of this finite deterrence pattern. |
