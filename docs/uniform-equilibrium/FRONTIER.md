...
| Literal terminal-debt prefix descent | `M+L` conditional | The global infimum of literal maximum terminal exploitability is positive in every putative counterexample. Exact Nash prefixing against an actual suffix payoff weakly decreases every debt coordinate; a positive singleton gap strictly decreases total debt, so a lexicographic maximum/sum objective eliminates tied maximal debtors. Such lexicographic near-minimizers exist at every accuracy, and the gap-carrying player may vary. A root endpoint gain with sufficiently small opponent absorption supplies the literal singleton gap. The remaining hypothesis is a uniform positive literal gap at a positive-debt coordinate of those profiles; conditioned and stored boundary values do not provide literal co-realization. |
