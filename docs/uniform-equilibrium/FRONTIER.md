...
| Periodic-window and phantom-boundary evaluation | `M+L` | The full behavioral reply against periodic opponents is exactly a finite phase-stop/refusal maximum. Conditioning on eventual absorption removes the phantom boundary while retaining exact consecutive chronology and compact convex weights. Singleton rows purify exactly; multi-owner rows with genuine phantom mass cannot be purified by exact rowwise product rescaling. The remaining route is a floor-safe singleton chattering or a state-matched collision-aware return, not ordinary compact recurrence. |
