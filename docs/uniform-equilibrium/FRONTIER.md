...
| Literal terminal semantic equality stratum | `M+L` | The closure of attainable literal prescribed/envelope pairs is compact, sequentially generated by executable profiles, invariant under every root prefix, and its prescribed coordinate lies in the finite reward-moment polytope. Total best-response debt attains a positive minimum in a putative counterexample. The auxiliary-target budget `D*kappa + sum_i alpha_i*(D-h_i) <= 0`, obtained by selecting a Nash root against `b-h` and prefixing it to the actual minimum pair, forces every such minimum pair to be an all-Continue Nash self-loop. Hence the former atomic alternative is globally impossible: every counterexample has a positive minimum all-Continue semantic plateau. Every player subset `J` has one common terminal outcome with aggregate singleton surplus at least `(|J|-1)D`. At most one coordinate is singleton-tight, and exact Nash roots against the minimum prescribed value are collision-free and either all-Continue or solo with the owner carrying all debt. A boundary-matched charge packet would be Dirac at one owner and underfund a distinct outsider. A pure-time best-response law retains a quantitatively profitable Never, at-stop collision, or opponent-before-stop atom. Its mass admits an exact survival-weighted time disintegration. Each actual live row has an exact carrier-owned semantic prefix factorization, but positive collision mass over a minimum tail proves that row is not exact Nash. Deep exact stacks may remain entirely all-Continue. Thus profile incidence is closed; the remaining seam is a quantitative Nash-defect/excess-debt charge producing a nontrivial legal minimum-prefix row. |
