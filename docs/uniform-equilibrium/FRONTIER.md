...
| Concentrated reprojection consumers | `M+L` | The other-defect branch freezes one non-owner player, one best endpoint, and a joint root/tail limit with positive coalition mass and quantitative defect; the limit is non-Nash, so it still needs Nashification/transport. Singleton concentration forces the marked owner's positive Quit advantage to vanish and yields a third-player strict joiner, an owner-side collision cancellation, or a punishment moat. Strict tail escape is compatible with orientation: cap--Nash return occurs exactly when absorption spends the tail excess, and an all-Continue cap face can stall. |
