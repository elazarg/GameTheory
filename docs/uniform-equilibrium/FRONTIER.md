...
| Three-core elimination | `M+L+A+C` | Every finite quitting game whose corrected normal core has exactly three players has an ordinary uniform-equilibrium payoff. Consequently every four-player counterexample has full corrected core; the live four-player algebraic and barrier searches may be restricted to full-core tables. |
| Combined counterexample normal form and finite screens | `M+L+A+C` | Nonexistence is exactly a positive all-behavior terminal gap; finite canonical floor-prefix capacity is derived from it by the finite-prefix closing dichotomy and is not a second field. The regime is empty below four players and has canonical minimal `Fin n` form; toggle/stationary ceilings, supported packet atoms, closed weak-preference walks, quantitative floor-clock budgets, and zero-charge recurrence are landed. Proper restriction and packet-support cycles are explicitly not ambient extensions or chronological Bellman orbits. |
| Periodic-window and phantom-boundary evaluation | `M+L` | The full behavioral reply against periodic opponents is exactly a finite phase-stop/refusal maximum. Conditioning on eventual absorption removes the phantom boundary while retaining exact consecutive chronology. Singleton rows purify exactly. On the diffuse singleton-tight stratum, hazard rescaling has quadratic coalition-law error. Complete deleted clocks feed a two-clock compiler. A deficient clock forces a unique nonsummable owner, summable non-owner hazards, and literal terminal convergence to the owner's singleton vector; direct conditioned solo extraction and punishment completion compile the owner whenever the rescaled pure-Quit defect vanishes. Exact source Nash charges every singleton-floor deficit to the source deleted clock, so no source-floor premise is required. Hence a diffuse positive-absorption counterexample seam has a strict phantom coordinate `solo_i < boundary_i`; every such player is eventually literal `Never`, and the quitter support lies on a proper singleton-tight face beyond a common cutoff. One fixed outsider recurs cofinally with a uniform rescaled Quit defect, a conditioned payoff uniformly below its singleton reward, and a uniform positive endpoint gap against the next conditioned target. Unmodified targets split into floor-admissible reset targets or a fixed underfloor coordinate funded by phantom survival. Coordinatewise floor clipping gives a sharper scalar alternative: a distinct negative singleton or exact punishment/singleton equality, or cofinally many exact Nash--Bellman reset edges with both endpoints above every punishment floor and uniformly positive absorption. These displayed edges cannot compose because every tail stores the all-Continue simplex. Own-marginal approximate repair causes an order-one predecessor displacement, while full-root tail stability consumes any supplied full-vector match. Approximate root/continuation splicing needs no sure-absorption premise. The residual is multi-player chronological matching with a nontrivial stored tail root, or exclusion of the scalar punishment/singleton branch. |
| Conditioned periodic renewal | `M+L` | Longer finite windows of the canonical source tail can be repeated as ordinary periodic product profiles. Their literal payoffs are exactly the normalized restart deliveries and converge to the source value conditioned on eventual absorption. Their full caps are the exact refusal/first-pass phase-stop maxima. This closes payoff provenance but not renewal tightness: maximum- or total-debt minimality supplies no upper bound on the cyclic cap, whose stabilized exercise or passive-refusal excess is the exact residual. |
| Cap--Nash endpoint transport and chronology | `M+L` | Prefix an actual profile by an exact mixed Nash root selected against its coordinatewise behavioral cap. Every literal best-response debt and the uniform random-deviation audit loss scale exactly by joint Continue mass, so positive total-debt infimum charges absorption and survival odds to relative excess above the infimum. A sharp joining-loss constant transports singleton endpoints to the cap. Against `cap-q`, one auxiliary root simultaneously gives a near-minimum absorption moat and a prescribed/comparison singleton floor; this excludes a persistent conditioned endpoint gap on any co-realized total-debt near-minimizing family. Under an explicit strict comparison of the moat errors, every exact Nash root against the unshifted cap is literally all-Continue. Arbitrary-depth finite backward cap chronologies still exist, but `D_*` times their unweighted total absorption is at most the terminal debt excess, each finite block's survival stays uniformly positive, and payoff/cap return is only linear in absorption. A distant debt target forces macroscopic absorption, yet the exact conditional-delivery split leaves cancellation or root selection as the renewal obstruction. The cap-to-prescribed homotopy is all-Continue before its endpoint at a positive minimum; its endpoint is either still all-Continue or a solo debt gate with a joiner/punishment certificate, and an honest regression stalls on the whole closed homotopy. Stacks at different depths need not be compatible, so no infinite Never path, tangent, or renewal-tightness conclusion follows. |
...
| Literal terminal-debt prefix action | `M+L` conditional | Here debt means the best-response debt, equivalently the all-behavior unilateral regret or exploitability gap, not online-learning regret. The global infimum of literal maximum terminal exploitability is positive in every putative counterexample. Exact Nash prefixing acts coordinatewise by `d ↦ (c*d-a)₊`; chronological blocks compose exactly and the summed debt loss is opponent-killed debt plus consumed positive endpoint premium. Arbitrarily deep finite exact-root stacks exist over lexicographic near-minimizers, with every displayed suffix pair literally co-realized and every local total drop below the common accuracy. A macroscopic debtor sees vanishing opponent hazards and premium; two macroscopic debtors force the whole root toward all-Continue. On one actual profile, affine deconditioning transports a same-player positive singleton and conditioned-endpoint gap to literal debt uniformly over every absorption regime. The projective conditioned tail does not co-realize those inputs automatically, and plateau-realizing profiles provably have the opposite conditioned-endpoint sign. The remaining live use is on a different marked actual family, not the plateau sequence itself. |
| Literal terminal semantic equality stratum | `M+L` | The compact prefix-invariant semantic carrier has positive minimum total and maximum best-response debt in every counterexample. Auxiliary selection forces every positive minimizer of either objective to be an all-Continue self-loop. One actual pure-time law pays debt through quantitative harmonic Never mass or chronological charge. In the persistent collision branch, atom mass times the marked quitter's local defect is bounded by reset debt, so the same executable rows have vanishing marked-player defect. Their tail cluster either lies strictly above the minimum fiber or, on the fiber, other players carry a uniform positive defect sum. A reached-row better-endpoint switch is a legal unilateral deviation with exact gain `live mass * local defect`. Its marked coalition is routed with no loss into exactly one of four same/erase/insert/same Boolean cells. Fractional switching reduces the same debt exactly and retains at least `1-lambda` of every incidence atom; its transfer account loses the actual prefixed source's exact excess over the minimum, because localization only puts the continuation tail on that fiber. If an arbitrary finite endpoint word reaches zero total root defect from positive collision mass, some full member-Continue move carries positive mass from an exact pair to its singleton. This crossing is unsigned: a two-player regression reaches all-Continue although the dropout player strictly prefers remaining in the pair. The singleton either lies in the isolated-negative punishment branch or admits a distinct strict joiner whose pure-Quit update creates an overlapping positive-mass pair; this is a static root transition, not a Bellman edge. General exit-face Nashification is false even approximately, and iterated legal resets can destroy the collision and end at zero-absorption all-Continue. The counterexample has a finite closed strict-toggle orbit on the coalition cube, but not a Bellman chronology. Coupled reset/incidence flows cannot obtain provenance from the coordinate-conserving minimum spine. Cap--Nash prefixing of an off-minimum reset cluster instead has an exact excursion account. The compact joint semantic/full-outcome-law carrier permits fixed-law reset minimization, retains transfer and incidence, and transports incidence as `root incidence + survival * old incidence`. At that minimizer a singleton-cap violation yields strict absorbing descent with positive survival; otherwise all-Continue is a cap fixed point. Positive incidence and a supported strict toggle do not eliminate this face, by a literal exact regression. On the positive-incidence reset face, minimizing debt first gives an exact dichotomy: contact with the global minimum fiber, or positive separation. In the separated case the surface tension `(D-D_*)/I` attains, and its division-free maximum principle forces every exact cap--Nash root to all-Continue. For each fixed positive incidence threshold this unique cap root has a robust positive total-defect moat. A fixed normalized three-player table keeps incidence at least one half while cap-local total defect tends to zero, so no linear moat persists uniformly into the diffuse-incidence scale. The harmonic and finite reservoirs can be retained on one co-realized reset law and fed to the fixed-law dispatch. The live gap is a compiler from the separated all-Continue joint-law plateau to a signed finite reset word or state-matched chronology. Boolean Möbius labels remain upper charges and can cancel to zero net defect. |
| Fixed-opponent root/tail transport and residual-discharge no-go | `M+L` | Arbitrarily changing one player's root and continuation while keeping every opponent fixed preserves that player's all-behavior best-response envelope. Payoff gain is therefore exactly the decrease of a bounded own-debt potential, up to the transported profile's explicit residual error. This closes the fixed-opponent root/tail operation class. For total debt at a global minimum, any scalar residual discharge is at most the explicit Nash error created; exact positive discharge is impossible, and a selected-subset debt decrease is transferred to complementary players. These invariants rule out residual-discharge-only scalar accounting but do not control opponent-created continuation surcharge or close opponent-changing transport. |
| Signed routed dropout | `M+L` conditional | Recomputed best-endpoint provenance makes the floor-forced full Continue overwrite weakly profitable, with exact gain equal to the mover's local Nash defect. Positive defect gives a strict pair-to-singleton dropout; zero defect gives exact endpoint indifference. A literal regression retains positive incidence and a negative singleton while reaching zero defect entirely through best-endpoint moves with a neutral dropout. Its starting pair profile is even a global zero-debt semantic minimizer, but deleting the indifferent player raises another player's debt by one. Global chronology must control cross-player debt or consume the exact-tie branch. |
| Reset-face reprojection | `M+L` | At a separated surface-tension plateau, exact support on the reset face extends uniformly with arbitrary additive error to points of sufficiently small reset debt. The quantitative extension has an exact alternative: either one linear face penalty controls the whole joint carrier, or positive off-face violations have unbounded violation-to-reset-debt ratio. In the linear branch, a finite endpoint word satisfies `retention*initialExcess ≤ finalExcess + penalty*finalResetDebt`; exact return to the minimum reset face forces a full overwrite, but not its payoff sign. The unbounded branch concentrates at a joint semantic/law contact with zero reset debt and tension, strictly positive opponent incidence, and exact reward-moment provenance. Exact temporal splitting gives either a recurring positive stage atom or a finite unit coalition clock with mesh tending uniformly to zero. In the concentrated branch the owner defect vanishes unweighted; the shifted tail either escapes above the minimum fiber, is a singleton cylinder, or other players carry a uniform positive defect and one supplies a legal routed best-endpoint gain. The diffuse clock has neither an exact-Nash policy nor a common shifted-tail state. In the other surface-tension branch, minimum-face contact need not retain positive incidence. |
| Concentrated reprojection consumers | `M+L` | The other-defect branch freezes one non-owner player, one best endpoint, and a joint root/tail limit with positive coalition mass and quantitative defect; the limit is non-Nash. A same-profile partial reset gives exact gain, debt transfer, and marked-incidence retention, but reaches the mover's zero face only when that gain exhausts its debt and need not preserve the old zero face. Singleton concentration forces the marked owner's positive Quit advantage to vanish and yields a third-player strict joiner, an owner-side cancellation, or a punishment moat. Cancellation further yields strict shifted-tail escape or one fixed probability-weighted opponent-coalition join loss. Strict tail escape returns only when cap--Nash absorption spends the tail excess. |
| Harmonic reservoir obstruction | `M+L` | At a minimum debt gate with no strict singleton joiner, the harmonic/Never branch produces a fully absorbing sure-solo row which is locally exact. Its unique noncontracting owner has stationary unilateral cap zero and regret at least the selected debt share, while `solo < punishment ≤ 0` reverses the punishment-admissibility inequality. Thus neither a period-one completed cycle nor an arbitrary instant-punishment continuation consumes this branch. |
| Rare punishment scaling | `M+L` no-go | Independent rare-hazard scaling does not preserve collision-supported punishment. In one fixed normalized three-player table the punishment value is `-1`, attained by a sure two-opponent collision, yet shrinking both opponent marginals to zero sends the owner's stationary cap to `1`. A viable rare route must retain coalition phases or assume singleton-mixture support. |
...
| Augmented-cap carrier, terminal funding, and splice | `M+L` | Finite exact-D caps lie in a bounded punishment-floor carrier. The aggregate terminal singleton cap is at most the reward bound and cannot saturate the older cardinality bound in a counterexample. The canonical one-owner zero-target funding root is always Farkas; any physical funding root must enlarge support and use negative collision payoff, and compatible two-owner zero-pair-join support is insufficient. Cap-minus-value transports exactly by survival, but common terminal realization and a strategic return remain open. |
