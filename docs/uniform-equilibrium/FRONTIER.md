...
| Literal terminal semantic equality stratum | `M+L` | The closure of attainable literal prescribed/envelope pairs is compact, sequentially generated by executable profiles, invariant under every root prefix, and its prescribed coordinate lies in the finite reward-moment polytope. Total best-response debt attains a positive minimum in a putative counterexample. The auxiliary-target budget `D*kappa + sum_i alpha_i*(D-h_i) <= 0`, obtained by selecting a Nash root against `b-h` and prefixing it to the actual minimum pair, forces every such minimum pair to be an all-Continue Nash self-loop. Hence the former atomic alternative is globally impossible: every counterexample has a positive minimum all-Continue semantic plateau. At most one coordinate is singleton-tight, and exact Nash roots against the minimum prescribed value are collision-free and either all-Continue or solo with the owner carrying all debt. A pure-time best-response law retains a quantitatively profitable Never, at-stop collision, or opponent-before-stop atom. Its mass admits an exact survival-weighted time disintegration on the same executable chronology and a concentrated-stage/diffuse-charge alternative. The remaining seam is strategic incidence between those actual live rows and exact minimum-semantic prefix/debt-utilization edges. |
