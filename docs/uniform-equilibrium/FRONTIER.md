...
| Literal terminal semantic equality stratum | `M+L` | The closure of attainable literal prescribed/envelope pairs is compact, sequentially generated by executable profiles, invariant under every root prefix, and its prescribed coordinate lies in the finite reward-moment polytope. Total debt attains a positive minimum in a putative counterexample and exact Nash prefixing fixes the full debt vector. The minimum fiber's compact predecessor-serial graph produces an infinite state-matched chronology. Exact solo-owner occupation accounting eliminates its non-atomic branch: vanishing survival is an isolated-negative atomic endpoint, while positive survival converges to a forbidden all-Continue minimum point. Therefore every counterexample has either a positive minimum all-Continue semantic plateau or a positive-rate atomic solo endpoint with `solo <= -terminalGap` and `solo < punishmentValue`. The atomic edge retains minimum-tail provenance and has a sharp first support boundary: below rate one its owner collision derivative is nonzero or another outsider is cotight; at rate one it is strictly negative or another outsider is cotight, and the tight entrant is quantitatively not a punisher. Plateau best responses retain positive mass on literal Never, an at-stop collision, or opponent absorption before the stop; replacing the selected debtor resets its debt and transfers the full gap to a fixed different player along a subsequence. The residual is time-local debt charging of that marked atom or a nonlinear multi-outsider support-or-punishment theorem. |
