...
| L-INTR | `Languages/Intrinsic.lean`, `Languages/Intrinsic/**` | 8 | native branch only if it retains theorem-relevant data | EXP-062 active; hostile closed-loop solvability and future-information causality slice reserved | not started; 0/158 credited | decide whether one theorem is unavailable from bare Protocol before porting any mixed/behavioral, Kuhn, PMF, or KernelGame hierarchy |
