...
| L-INTR | `Languages/Intrinsic.lean`, `Languages/Intrinsic/**` | 8 | capability-light native product/closed-loop root before any named Protocol compiler | native ownership and configuration-dependent causality validated by EXP-062/D31; stable root and native Examples/Tests leaf promoted | partial; 54/158 reviewed, 104 queued | [exact declaration ledger](coverage/L-INTR-broad.md); recover the derived solution-selection theorem micro-family next, while temporal compilation, perfect recall, mixed/behavioral, PMF/utility, equilibrium, and Kuhn retain separate gates |
