...
| E-06 | Epistemic root advertised a common-prior object it did not define. | closed | Theorems use an explicit shared PMF prior on arbitrary carriers; there is no separate common-prior structure. |
...
| E-08 | Raw epistemic posteriors and conditioned laws lack an equality bridge. | closed | `posterior_eq_fiberPosterior_expect` identifies scalar posteriors with guarded event-indicator expectation under the canonical PMF fiber posterior on every positive information cell. `Tests.Agreement` checks revealing and coarse cells and proves that a zero-mass cell supplies no conditioning witness. |
