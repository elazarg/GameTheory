...
named, packaged, and audited, including discrete probability and its measure
bridges?
...
while `GameTheory.Math.Probability.OnlineLearning` is the sole PMF
...
- Use Mathlib PMFs and ordinary measures, with reusable operations and guarded
  expectations under `GameTheory.Math.Probability`; finite support belongs on
  the operations that need it (D62).
