...
| FOSG | **passed for one round (EXP-042/T4; EXP-059/D30):** real source players act simultaneously through Protocol; explicit EFG microsteps retain the full prefix while phase-only information hides within-round choices; literal target-history erasure equals the canonical source-history law for every target profile and both orders agree | **Native history/Kuhn, reachable observation-model, and Compile batches classified (EXP-057/D28, EXP-058/D29, EXP-059/D30);** next a two-round stochastic signal-replay gate must cover source public/private updates, own-action memory, inactive slots, and scaled history laws before generic bridge promotion; counterfactual reach, CFR, and ordinary continuation coefficients remain separate gates |
