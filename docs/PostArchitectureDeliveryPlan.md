...
| intrinsic games | **EXP-062 active:** test whether configuration-dependent causality plus unique closed-loop solvability is unavailable from bare Protocol before choosing a temporal compiler | only if the hostile slice passes: a capability-light native configuration/information root, then separately gated compilation, perfect recall, and native theorems |
