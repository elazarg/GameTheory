...
| FOSG | **two-round gate passed (EXP-042/T4; EXP-059/060, D30):** real source players act simultaneously through Protocol; explicit EFG microsteps retain exact source histories while hiding within-round choices; resolver-only public/private/own-action replay, inactive slots, arbitrary target projection, and literal six-step/two-round history laws pass in both orders | **Native history/Kuhn, reachable observation-model, and Compile batches classified (EXP-057/D28, EXP-058/D29, EXP-059/D30);** next implement and validate the generic explicit-order adapter, then account the pinned live bridge chain; counterfactual reach, CFR, and ordinary continuation coefficients remain separate gates |
