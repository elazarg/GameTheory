...
|---|---|---|---|
| finite stochastic/Markov games | one pinned `MultiRound/StochasticGame.lean`, no general value or stationary-equilibrium theory | **domain gate passed (EXP-050/D22, EXP-051/D23, and EXP-072/D39):** native finite-horizon/uniform semantics, an exact uniform deviation-cap certificate equivalence, normalized two-player zero-sum Shapley values and saddles, and general-sum finite Fink stationary Bellman-equilibrium existence | public opt-in `Stochastic` root using `FinDist` transitions and a named Protocol bridge; all fixed-point existence remains in the one-way `Analysis.Stochastic` bridge; general uniform existence remains an excluded open conjecture |
