...
private def IsAllocationOn [Fintype ι] [DecidableEq G]
    (allGoods : Finset G) (A : Allocation ι G) : Prop :=
...
private lemma roundRobinAux_biUnion [DecidableEq G]
  ...
  · rw [roundRobinAux_step w turn s A hne]
    ...
    have hrem' : ∀ g' ∈ s.erase g, ∀ i : Fin n,
        g' ∉ Function.update A turn (insert g (A turn)) i := by
...
/-- Recursive core of round-robin. -/
noncomputable def roundRobinAux [DecidableEq G]
    (v : AdditiveValuation (Fin n) G)
    (turn : Fin n) (remaining : Finset G) (A : Allocation (Fin n) G) :
    Allocation (Fin n) G :=
  rawRoundRobinAux v turn remaining A

/-- The complete choice round-robin allocation over all goods. -/
noncomputable def roundRobinAllocation [Fintype G] [DecidableEq G]
    (v : AdditiveValuation (Fin n) G) : Allocation (Fin n) G :=
  roundRobinAux v ⟨0, NeZero.pos n⟩ Finset.univ (fun _ => ∅)

omit [NeZero n] in
private theorem isAllocation_of_isAllocationOn_univ [Fintype G] [DecidableEq G]
    {A : Allocation (Fin n) G} (hA : IsAllocationOn (Finset.univ : Finset G) A) :
    IsAllocation A := by
  constructor
  · exact hA.1
  · intro g
...
/-- Round-robin as a certified feasible-allocation rule. -/
noncomputable def roundRobinRule [Fintype G] [DecidableEq G]
    (v : AdditiveValuation (Fin n) G) :
    {A : Allocation (Fin n) G // IsAllocation A} :=
  ⟨roundRobinAllocation v, by
    dsimp [roundRobinAllocation, roundRobinAux]
    exact isAllocation_of_isAllocationOn_univ
      (rawRoundRobinAlloc_isAllocation v (Finset.univ : Finset G))⟩

/-- `roundRobinAllocation` produces a complete partition of the goods. -/
theorem roundRobinAllocation_isAllocation [Fintype G] [DecidableEq G]
    (v : AdditiveValuation (Fin n) G) :
    IsAllocation (roundRobinAllocation v) :=
  (roundRobinRule v).2

omit [NeZero n] in
private theorem isEF1_of_isEF1OnNonempty [DecidableEq G]
    {v : AdditiveValuation (Fin n) G} {A : Allocation (Fin n) G}
...
theorem roundRobinAllocation_isEF1 [Fintype G] [DecidableEq G]
  ...
  dsimp [roundRobinAllocation, roundRobinAux]
  exact isEF1_of_isEF1OnNonempty hnn
