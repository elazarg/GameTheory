...
* `nashSolution_invariant` — invariance under affine transformations
-/
...
structure BargainingProblem where
  /-- Feasible utility pairs: `feasible (u₁, u₂)` means `(u₁, u₂)` is achievable. -/
  feasible : ℝ × ℝ → Prop
...
/-- The Nash bargaining solution is Pareto optimal among IR-feasible outcomes.
    If there were a strictly Pareto-dominating IR-feasible outcome `v`,
    then `v` would have a strictly higher Nash product, contradicting maximality. -/
...
/-- A symmetric bargaining problem: the feasible set is symmetric and
    the disagreement utilities are equal. -/
...
theorem nashSolution_symmetric (u : ℝ × ℝ) (hsym : B.IsSymmetric)
    ...
    u.1 - B.d₁ = u.2 - B.d₂ := by
  -- The swapped outcome (u₂, u₁) is also a Nash solution
...
/-- Scale invariance: if we apply positive affine transformations
    `u₁ ↦ α₁ * u₁ + β₁` and `u₂ ↦ α₂ * u₂ + β₂` to both the
    feasible set and disagreement point, the Nash solution transforms
    accordingly. This follows because the Nash product is maximized
    at the same point after the transformation. -/
theorem nashSolution_affine_invariant
  ...
  · -- Maximality
    ...
    -- The Nash product transforms: (α₁*u₁+β₁ - (α₁*d₁+β₁))*(α₂*u₂+β₂ - (α₂*d₂+β₂))
    -- = α₁*(u₁-d₁) * α₂*(u₂-d₂) = α₁*α₂*(u₁-d₁)*(u₂-d₂)
    -- Similarly for v
