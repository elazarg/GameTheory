...
theorem restrictedBehavioralToMixed_apply_prod
    ...
    [∀ i, Fintype (Option (M.Act i))]
    (σ : RestrictedBehavioralProfile (I := I) H) (i : ι)
    (fi : RestrictedLocalCoord (I := I) H i → Option (M.Act i)) :
    (restrictedBehavioralToMixed (I := I) H σ i) fi =
      ∏ p : RestrictedLocalCoord (I := I) H i, (σ i p) (fi p) := by
...
theorem restrictedMixedJointRaw_behavioralToMixed_apply_prod
    ...
    (restrictedMixedJointRaw (I := I) H
      (restrictedBehavioralToMixed (I := I) H σ)) π =
      ∏ i, ∏ p : RestrictedLocalCoord (I := I) H i, (σ i p) (π i p) := by
...
theorem restrictedMixedJointRaw_behavioralToMixed_eq_map_reassemble
    ...
    (κ : ∀ i, RestrictedLocalCoord (I := I) H i → PMF (Option (M.Act i))) :
    restrictedMixedJointRaw (I := I) H
      (restrictedBehavioralToMixed (I := I) H κ) =
      PMF.map
        (fun ω : (∀ k : Σ i, I.RestrictedLocalCoord H i, Option (M.Act k.1)) =>
          fun i v => ω ⟨i, v⟩)
        (pmfPi (fun k : Σ i, I.RestrictedLocalCoord H i => κ k.1 k.2)) := by
  classical
  let τ : ∀ k : Σ i, I.RestrictedLocalCoord H i, PMF (Option (M.Act k.1)) :=
    fun k => κ k.1 k.2
  let e :
      (∀ k : Σ i, I.RestrictedLocalCoord H i, Option (M.Act k.1)) ≃
