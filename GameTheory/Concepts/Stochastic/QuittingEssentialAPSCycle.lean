...
/-!
# Executable finite essential-APS cycles
...
* `isUniformEquilibriumPayoff` uses positive mass and the supplied contraction
  witness to compile the selected value to one fixed-target uniform payoff.
...
structure QuittingEssentialAPSCycleCertificate
  ...
  opponentContracts : ∀ who,
    (∏ block : Fin L,
      if who = owner block then 1 else 1 - hazard block) < 1
  owner_changes : ∀ block,
