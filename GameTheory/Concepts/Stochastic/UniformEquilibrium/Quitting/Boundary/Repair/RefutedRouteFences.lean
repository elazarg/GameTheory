...
`hp2_pos` is consumed by `nlinarith` when deriving `hineq2` below (needed to
turn `0 ≤ p2 * (1 - V1 true)` into `V1 true ≤ 1`); the unused-variable linter
does not see through that hint-list usage, so it is suppressed locally. -/
