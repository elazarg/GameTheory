...
*Migration candidate: `Basic.lean`.*  This is a general fact about
`StochasticGame.Hist`/`histDist`, not specific to the Big Match; it is kept
here (rather than in `Basic.lean`) to avoid rebuilding the whole library. -/
