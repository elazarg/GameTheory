...
theorem quittingMeshIntensity_mono
    {p pStar : ℝ} (hp0 : 0 ≤ p) (hpLe : p ≤ pStar)
    (hpStar1 : pStar < 1) :
...
@[simp] theorem quittingUniformMeshValue_block
  ...
  rw [quittingUniformMeshCoarseTime_block_add hm (Nat.zero_lt_of_lt hm),
    quittingUniformMeshOffset_block_add hm (Nat.zero_lt_of_lt hm)]
