...
`GameTheory.Concepts.Stochastic.Equilibrium.Discounted`, these ingredients prove that the
selected stationary strategies guarantee the normalized discounted Shapley
value against every behavioral strategy.
-/
