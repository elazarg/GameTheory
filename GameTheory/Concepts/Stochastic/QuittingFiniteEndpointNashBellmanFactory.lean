...
The existing finite factory is anchored at the zero payoff.  Target-tail
reinsertion needs the same predecessor construction from an arbitrary bounded
payoff vector.  This file supplies that generalization without changing the
compact Nash--Bellman relation: the root coordinate of the terminal point is
chosen to be all-Continue, while its payoff coordinate is the prescribed
endpoint.
-/
