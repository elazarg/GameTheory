...
import GameTheory.Concepts.Stochastic.QuittingEssentialAPSUniformHazard
...
theorem
  ...
  obtain ⟨pStar, hpStar0, hpStar1, hmassCeiling⟩ :=
  ...
  apply isUniformEquilibriumPayoff_of_arbitrarily_small_infinitePath_quitError
    reward initial hbound.le hreward
  intro error herror
  ...
  have hlocalError : ∀ time,
      2 * bound * quittingMeshHazard (mass time) mesh ≤ error := by
    intro time
    exact hmeshError (mass time) (hmass0 time) (hmassCeiling time)
  have hcertificate :=
    quittingUniformMesh_quitErrorCertificate
      reward owner mass value hmesh hK hmass0 hmassLt harc hactive
        hviable hD hcollision hlocalError hvalueBound hrho0 hrho1 hblock
  exact ⟨by simpa only [owner, hrun.1] using hcertificate⟩
