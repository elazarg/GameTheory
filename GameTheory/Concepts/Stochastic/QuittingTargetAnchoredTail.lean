...
/-!
# Target-closed tails for finite quitting prefixes

A target-closed suffix is already a best reply for one selected player.  An
exact finite Nash--Bellman prefix ending at the suffix's actual payoff then
leaves no debt for that player.  Every other player's tail debt is bounded by
twice the reward bound and is multiplied by opponent survival through the
prefix.  If the target's own planned survival is small, it is one factor of
every non-target player's opponent-survival product.
-/
...
from the cap-attainment theorem; only the opponents remain constant. -/
