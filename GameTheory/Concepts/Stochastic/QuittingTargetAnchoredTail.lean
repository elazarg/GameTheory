...
The second half of the file compiles an exact finite Nash--Bellman prefix whose
boundary is the actual payoff of such a suffix. The selected target has zero
suffix debt; every other player's debt is bounded by twice the reward bound
and weighted by opponent survival through the prefix.
-/
...
the target uses the already-landed cap-attaining behavioral response. -/
