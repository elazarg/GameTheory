...
/-!
# Self-play: from per-player no-regret to coarse correlated equilibrium
...
equilibrium. (The welfare of such an ε-CCE is then bounded by smoothness — the **price of anarchy
of learning** — via `IsSmooth.epsilonCoarseCorrelated_bound` in `Concepts/Welfare/Smoothness`.)

`selfPlay_timeAverage_isεCCE` takes the per-player cumulative regret bound `B` as a hypothesis. The
per-player gain sequence a no-regret learner consumes is
`g_t a = mixedExtension.eu (update (p t) who (pure a)) who`; since `mw_externalRegret_le` requires
`[0,1]` gains, this file also provides the affine normalization (`normGain` into the band
`[lo i, lo i + W]`) and the scaling identity relating a normalized regret bound back to actual
payoffs (multiplying it by `W`). The coupled construction that actually discharges the hypothesis —
multiplicative-weights self-play — is built on top of these pieces in `MWSelfPlay`.
...
/-- **Independent no-regret play yields a coarse correlated equilibrium.** If every player's
    ...
    resulting `(B/T)` shrinks under a horizon-dependent η-tuning (not formalized here). -/
