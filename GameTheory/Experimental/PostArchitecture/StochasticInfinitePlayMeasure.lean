...
/-- The canonical product measurable space on complete exact-length plays. -/
@[reducible]
instance playMeasurableSpace (initial : G.State)
    [∀ i, Nonempty (G.Action i)] :
    MeasurableSpace (∀ n, PathHistory G initial n) := by
  letI : (n : ℕ) → MeasurableSpace (PathHistory G initial n) :=
    fun n => pathHistoryMeasurableSpace G initial n
...
def pathStepLaw (n : ℕ) (history : PathHistory G initial n) :
    PMF (PathHistory G initial (n + 1)) :=
  ((G.perfectMonitoring initial).runBehavioralFrom profile 1 history.1).bindOnSupport
    fun result hresult =>
      PMF.pure ⟨result, step_length G initial profile n history result hresult⟩

/-- The canonical horizon law with its exact trace length retained. -/
def finitePathHistoryLaw (n : ℕ) : PMF (PathHistory G initial n) :=
...
private theorem finitePathHistoryLaw_zero :
    finitePathHistoryLaw G initial profile 0 =
      PMF.pure (initialPathHistory G initial) := by
  apply pmf_map_injective (f := Subtype.val) Subtype.val_injective
  rw [map_val_finitePathHistoryLaw G initial profile 0, PMF.pure_map]
...
omit [Fintype ι] in
private theorem measurable_lastPathHistory
...
private theorem pathStepKernel_comp_finiteLaw (n : ℕ) :
    pathStepKernel G initial profile n ∘ₘ
        (finitePathHistoryLaw G initial profile n).toMeasure =
      ((finitePathHistoryLaw G initial profile n).bind
        (pathStepLaw G initial profile n)).toMeasure := by
  exact toMeasure_bind
...
private theorem coordinate_finitePathHistoryLaw (n : ℕ) :
    (infinitePlayMeasure G initial profile).map (fun play => play n) =
      (finitePathHistoryLaw G initial profile n).toMeasure := by
  induction n with
  | zero =>
      rw [coordinate_zero G initial profile,
        finitePathHistoryLaw_zero G initial profile, PMF.toMeasure_pure]
  | succ n ih =>
...
private theorem map_chronologicalAt_finitePathHistoryLaw (n : ℕ) :
    PMF.map (chronologicalAt G initial n)
        (finitePathHistoryLaw G initial profile n) =
...
theorem map_chronologicalProjection_infinitePlayMeasure (n : ℕ) :
    (infinitePlayMeasure G initial profile).map
      ...
      (G.chronologicalHistoryLaw initial profile n).toMeasure := by
  ...
  rw [PMF.toMeasure_map
    (f := chronologicalAt G initial n)
    (p := finitePathHistoryLaw G initial profile n)
    (measurable_chronologicalAt G initial n)]
