/-
# EXP-054: explicit-list executable knapsack boundary

This hostile slice tests a computable `0/1` knapsack solver whose allocation
is a finite set and whose agent enumeration is an explicit duplicate-free
list.  The algorithm therefore needs no hidden `Fintype`, classical choice,
real arithmetic, or coordinate update.  Its proof semantics quantify over any
finite allocation supported by the supplied list.
-/

import Mathlib.Data.Finset.Basic
import Mathlib.Algebra.BigOperators.Ring.Finset

namespace GameTheory.Experimental.KnapsackBoundary

variable {Agent : Type} [DecidableEq Agent]

/-- Total weight of a finite selected set. -/
def load (weight : Agent → ℕ) (selected : Finset Agent) : ℕ :=
  ∑ agent ∈ selected, weight agent

/-- Total value of a finite selected set. -/
def welfare (value : Agent → ℕ) (selected : Finset Agent) : ℕ :=
  ∑ agent ∈ selected, value agent

/-- Explicit-list skip/take solver.  Ties choose the take branch. -/
