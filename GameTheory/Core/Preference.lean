...
compare outcome laws. Only the definitions that inspect laws—convexity under
mixing and pullback along an outcome relabeling—belong here.
...
def Convex (weaklyPrefers : WeakPreference Agent Outcome) : Prop :=
  ∀ (agent : Agent) (t : ℝ) (h0 : 0 ≤ t) (h1 : t ≤ 1)
    ...
    weaklyPrefers agent secondPreferred secondAlternative →
      weaklyPrefers agent
        (mix t h0 h1 firstPreferred secondPreferred)
        (mix t h0 h1 firstAlternative secondAlternative)
...
theorem comapOutcome_apply
    (relabel : Outcome → Outcome')
    (weaklyPrefers : WeakPreference Agent Outcome')
    (agent : Agent) (preferred alternative : PMF Outcome) :
