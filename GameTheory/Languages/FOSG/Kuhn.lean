...
private theorem reachableInfoLegalMove_eq_none_of_terminal
...
private theorem reachableHistory_stepActionDeterminism
    (hLeg : G.LegalObservable) :
...
private theorem playerViewFrom_append_singleton_ne_nil
...
private theorem playerViewFrom_append_singleton_eq
...
private theorem publicViewFrom_length (es : List G.Step) :
...
private theorem publicView_length (h : G.History) :
    h.publicView.length = h.steps.length := by
  simpa [History.publicView] using publicViewFrom_length (G := G) h.steps

private theorem reachableInfoLegalMove_eq_none_of_terminal_view_eq
...
private theorem reachableInfoLegalMove_subsingleton_of_terminal
    (hLeg : G.LegalObservable) {h : G.History} (hterm : G.terminal h.lastState)
    (i : ι) :
...
private theorem reachableHistory_projectStates_eq_last
...
private theorem reachableInfoLegalMove_cast_val
...
private theorem reachableInfoLegalMove_cast_currentObs_val
...
private theorem subst_heq_fosg {α : Type} {P : α → Type} {a b : α}
    (h : a = b) (x : P b) : HEq (h ▸ x : P a) x := by
  subst h
  rfl

private theorem reachableHistory_castJointAction_val
...
private theorem projectActions_snoc_eq
...
private theorem sourceView_and_ownAction_eq_of_target_view_eq
    {h h' t t' : G.History}
...
private theorem reachableHistory_pureStep_snoc
    [Fintype ι] [∀ i, Fintype (Option (Act i))]
    (hLeg : G.LegalObservable)
    {π : (toReachableHistoryObsModelCore G hLeg).PureProfile}
...
private theorem reachableHistory_source_nonterminal_of_target_nonterminal
    [Fintype ι] [∀ i, Fintype (Option (Act i))]
    (hLeg : G.LegalObservable)
    {π : (toReachableHistoryObsModelCore G hLeg).PureProfile}
    {ss : List G.History} {t : G.History}
...
private theorem reachableHistory_pureRun_update_obs_local_nonterminal
    [Fintype ι] [∀ i, Fintype (Option (Act i))]
...
private theorem reachableHistory_pureRun_nonterminal_last_steps_length
    [Fintype ι] [∀ i, Fintype (Option (Act i))]
    (hLeg : G.LegalObservable) (n : Nat)
    {π : (toReachableHistoryObsModelCore G hLeg).PureProfile}
    {ss : List G.History}
...
private theorem ownAction_eq_of_source_target_view_eq
    {h h' t t' : G.History}
    {raw : JointAction Act} {raw' : JointAction Act}
    {hraw : G.legal h.lastState raw} {hraw' : G.legal h'.lastState raw'}
    {dst dst' : W}
...
noncomputable def reachableKuhnCoarseCorrelatedDeviationSimulation
    ...
    GameForm.DeviationFamilySimulation
      ...
      (reachableBehavioralGameFormAtHorizon (G := G) k).constantDeviationProfileFamily where
  viewG := { observe := id }
  viewH := { observe := id }
  rel := fun μG μH =>
    μH = PMF.map
  ...
  law_eq := by
    intro μG μH hrel
    rw [hrel]
    simp [GameForm.OutcomeView.correlatedLaw, GameForm.correlatedOutcome,
...
noncomputable def reachableKuhnCorrelatedDeviationSimulation
    ...
    GameForm.DeviationFamilySimulation
      ...
      (reachableBehavioralGameFormAtHorizon (G := G) k).recommendationDeviationFamily where
  viewG := { observe := id }
  viewH := { observe := id }
  rel := fun μG μH =>
    μH = PMF.map
