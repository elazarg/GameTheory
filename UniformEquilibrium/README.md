...
- `StochasticGame.exists_uniformDeviationCapConstructor` in
  [`UniformExistenceConjecture.lean`](Conjecture/UniformExistenceConjecture.lean) — the
  general problem above, in its quantitative form;
- `quittingGame_exists_uniformEquilibriumPayoff` in
  [`UniformEquilibrium/Quitting/Conjecture/Basic.lean`](Quitting/Conjecture/Basic.lean) — the finite-quitting
...
[`Uniform.lean`](../GameTheory/Concepts/Stochastic/Equilibrium/Uniform.lean)
holds only the definitions and their proved
...
[`UniformEquilibrium/Quitting/Terminal/TargetTail/TerminalUniformization.lean`](Quitting/Terminal/TargetTail/TerminalUniformization.lean)
with fixed-payoff selection in
[`UniformEquilibrium/Quitting/Terminal/TargetTail/TerminalUniformPayoffSelection.lean`](Quitting/Terminal/TargetTail/TerminalUniformPayoffSelection.lean).
...
- **Verification waist**: the adaptive certificates (`UniformEquilibrium/Certificates/Adaptive/Certificate.lean`) —
...
`UniformEquilibrium/Architectures/PublicResponse/CredibilityCriterion.lean` and its support-pruned companion
...
`UniformEquilibrium/Quitting/Examples/FTV/CyclicCredibility.lean` is the actual-data acceptance test for this
...
`UniformEquilibrium/Quitting/Examples/FTV/CyclicFiniteHorizon.lean` instantiates the arbitrary-start payoff
...
`UniformEquilibrium/Quitting/Examples/FTV/CyclicMinimality.lean` supplies Question 97's exact finite-algebra layer.
...
`UniformEquilibrium/Quitting/Examples/FTV/CyclicSemanticBridge.lean` closes the concrete duplication between those
...
`UniformEquilibrium/Certificates/Public/VariableStoppingAdaptiveDispatcher.lean` composes child systems at a
bounded causal stopping time, while
`UniformEquilibrium/Certificates/Public/VariableStoppingPrefixLawCompiler.lean` derives the prefix laws and an
...
`Models/Quitting/Asymptotic.lean` identifies every fixed behavior profile's limiting
finite-average payoff with its expected terminal reward.  The root modules
make the missing continuation datum explicit: `Models/Quitting/RootContinuation.lean`
proves the exact first-stage disintegration, and
`UniformEquilibrium/Quitting/Root/FirstBranch.lean` proves that a surely absorbing root/continuation
...
`Models/Quitting/SimpleBranches.lean` gives the sharp all-continue (`Never`) test,
including the value `max 0 qᵢ` against all-continuing opponents.
`Models/Quitting/RootPerturbation.lean` proves the one-coordinate `2 M d` payoff and
`4 M d` other-player regret bounds, including the near-sure-quitter
specialization.  `UniformEquilibrium/Quitting/Bellman/Finite/HorizonBridge.lean` proves that common
...
player-indexed closed tails.  `UniformEquilibrium/Quitting/Terminal/TargetTail/TargetAnchoredTail.lean` constructs a
tail that is optimal for one target against any stationary opponent row;
`UniformEquilibrium/Quitting/Bellman/Finite/EndpointNashBellmanFactory.lean` supplies exact finite prefixes
...
`UniformEquilibrium/Quitting/Terminal/TargetTail/DiagonalTargetTail.lean` is conditional on accuracy-indexed exact
...
`UniformEquilibrium/Quitting/Root/FleschSuccessor.lean` derives its asymmetric successor graph from two
consecutive proper arcs; `UniformEquilibrium/Quitting/EssentialAPS/Basic.lean` distinguishes executable
one-continuation segments from the larger full-convex-hull operator; and
`UniformEquilibrium/Quitting/EssentialAPS/FixedPoint.lean` constructs the carrier-restricted
greatest algebraic fixed family. `UniformEquilibrium/Quitting/EssentialAPS/Cycle.lean` proves that
...
`UniformEquilibrium/Quitting/Projective/LassoAll.lean` is the public projective boundary.  Matching
...
`UniformEquilibrium/Quitting/Punishment/CompletedCycle.lean` strengthens the exact absorbing-cycle
...
| Zero-sum single-controller (full finite generality) | `exists_uniformEquilibriumPayoff_of_isZeroSumBoolGame_of_isSingleController` | `UniformEquilibrium/SpecialCases/SingleController/PrimalExistence.lean` |
...
`UniformEquilibrium/SpecialCases/SingleController/RankCompletion.lean` then selects least-distance-decreasing
...
`UniformEquilibrium/SpecialCases/SingleController/FlowCompletion.lean` supplies the stronger same-policy route
...
`UniformEquilibrium/SpecialCases/SingleController/FlowHarmonicity.lean` then combines complementary slackness
...
`UniformEquilibrium/SpecialCases/SingleController/FlowReward.lean` proves exact reward--bias compatibility on
...
`UniformEquilibrium/SpecialCases/SingleController/PrimalExistence.lean` proves feasibility, bounds every
...
(`shapleyBehaviorProfile_isDiscountedNash`, `ZeroSum/Basic.lean`), and a **conditional
...
`UniformEquilibrium/SpecialCases/ZeroSum/MertensNeyman/Criterion.lean`).

`UniformEquilibrium/Examples/BigMatch/SelfSimilarity.lean` checks the finite structural core of Question 80
...
- `UniformEquilibrium/Examples/BigMatch/FinkEndpoint.lean` — refutes the calendar-Markov Fink endpoint.
- `UniformEquilibrium/Examples/BigMatch/DeficitIndexNoGo.lean` — the linear running-deficit index is not a
...
- `UniformEquilibrium/VanishingDiscount/Fink/TangentCounterexample.lean`, `UniformEquilibrium/VanishingDiscount/Fink/SelectionCounterexample.lean` — the
  supported-harmonic-adjustment route is selection-resistant.
- `UniformEquilibrium/Examples/PureExternality/Cycle.lean` — analytic provenance cannot supply the routing
  gluing invariant.
- `UniformEquilibrium/Diagnostics/Uniform/NonexistenceCertificate.lean` — quantitative late-horizon and
