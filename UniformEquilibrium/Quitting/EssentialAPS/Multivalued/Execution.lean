...
This module is deliberately independent of quitting games.  A local relation
may have several successors; finite reachability records only witnessed edges.
Classical choice then yields exactly one of three chronological objects:

* a finite witnessed path to a terminal state;
* one coherent infinite witnessed path; or
* a finite witnessed path to a typed no-successor obstruction.

No occupation measure, convex combination, or balance equation is interpreted
as a path edge.
-/
...
/-- A finite path in a supplied relation. -/
...
/-- A finite path whose vertices remain in a displayed finite carrier. -/
