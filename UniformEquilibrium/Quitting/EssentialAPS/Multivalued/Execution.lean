...
reachability below records only witnessed edges.  The main theorem then yields
one of three chronological objects: a finite terminal path, one coherent
infinite path, or a finite path to a typed no-successor obstruction.

Occupation measures and balance equations do not occur in the theorem and
