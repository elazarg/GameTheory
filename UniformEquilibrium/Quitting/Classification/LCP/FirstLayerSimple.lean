...
gives an exact terminal Nash profile.  The canonical all-errors terminal
selection theorem supplies a uniform-equilibrium payoff.
...
theorem strict_cross_of_normalLayer_one_eq_empty
  ...
  have hnotle : ¬normalizedSoloMatrix reward other owner ≤ 0 := by
    ...
    have hmem : other ∈ normalLayer (normalizedSoloMatrix reward) 1 := by
      ...
      simpa using hmem'
    rw [hlayer] at hmem
    exact Finset.not_mem_empty other hmem
...
theorem exists_exact_stationary_terminalNash_of_normalLayer_one_eq_empty
  ...
  · refine ⟨fun _ => PMF.pure false, ?_⟩
    simpa [quittingStationaryProfile, quittingAlwaysContinueProfile] using
...
/-- The canonical terminal-selection waist turns the concrete first-layer
producer into target-free uniform-payoff existence. -/
theorem exists_uniformEquilibriumPayoff_of_normalLayer_one_eq_empty
  ...
  quittingGame_exists_uniformEquilibriumPayoff_of_terminalNash_all_errors
    reward
    (terminalNash_all_errors_of_normalLayer_one_eq_empty reward hlayer)
